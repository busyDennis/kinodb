...
class PhoneNumber extends AbstractValidator
{
    ...
    public function isValid ($value = null, $context = null)
    {
        ...
        if ($countryPattern['code'] ==
                 substr($value, 0, strlen($countryPattern['code']))) {
        ...
        foreach ($countryPattern['patterns']['national'] as $type => $pattern) {
            if (in_array($type, $this->allowedTypes)) {
                ...
                if (preg_match($pattern, $value)) {
                    return true;
                } elseif (isset($valueNoCountry) &&
                         preg_match($pattern, $valueNoCountry)) {
                    // this handles conditions where the country code and prefix
                    // are the same
                    return true;
                }
            }
                ...
                if (in_array($type, $this->allowedTypes)) {
                    ...
                    if (preg_match($pattern, $value)) {
                        return true;
                             ...
                             preg_match($pattern, $valueNoCountry)) {
                        // this handles conditions where the country code and
                        // prefix are the same
                        return true;
                    }
                }
