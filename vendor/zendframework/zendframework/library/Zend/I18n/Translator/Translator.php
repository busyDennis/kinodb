...
class Translator
{
    ...
    public static function factory ($options)
    {
        ...
        if (isset($options['translation_file_patterns'])) {
            ...
            foreach ($options['translation_file_patterns'] as $pattern) {
                ...
                $translator->addTranslationFilePattern($pattern['type'], 
                        $pattern['base_dir'], $pattern['pattern'], 
        ...
        if (isset($options['translation_files'])) {
            ...
            foreach ($options['translation_files'] as $file) {
                ...
                $translator->addTranslationFile($file['type'], 
                        $file['filename'], 
        ...
        if (isset($options['remote_translation'])) {
            ...
            foreach ($options['remote_translation'] as $remote) {
                ...
                $translator->addRemoteTranslations($remote['type'], 
        ...
        if (isset($options['cache'])) {
            if ($options['cache'] instanceof CacheStorage) {
                ...
                $translator->setCache(
                        Cache\StorageFactory::factory($options['cache']));
            }
        }
        
        // event manager enabled
        if (isset($options['event_manager_enabled']) &&
                 $options['event_manager_enabled']) {
        ...
        if ($this->locale === null) {
            if (! extension_loaded('intl')) {
                throw new Exception\ExtensionNotLoadedException(
                        sprintf('%s component requires the intl PHP extension', 
                                __NAMESPACE__));
            }
    ...
    public function translate ($message, $textDomain = 'default', $locale = null)
    {
        ...
        $translation = $this->getTranslatedMessage($message, $locale, 
                $textDomain);
    ...
    {
        $locale = $locale ?  : $this->getLocale();
        $translation = $this->getTranslatedMessage($singular, $locale, 
                $textDomain);
        ...
        if ($translation === null || $translation === '') {
            if (null !== ($fallbackLocale = $this->getFallbackLocale()) &&
                ...
                return $this->translatePlural($singular, $plural, $number, 
                        $textDomain, $fallbackLocale);
            }
            
            return ($number == 1 ? $singular : $plural);
        }
        
        $index = $this->messages[$textDomain][$locale]->getPluralRule()->evaluate(
                $number);
        
        if (! isset($translation[$index])) {
            throw new Exception\OutOfBoundsException(
                    sprintf('Provided index %d does not exist in plural array', 
                            $index));
        }
     ...
     * @return string|null
     */
    protected function getTranslatedMessage ($message, $locale, 
            $textDomain = 'default')
    {
        if ($message === '') {
            return '';
     ...
     * @return Translator
     */
    public function addTranslationFilePattern ($type, $baseDir, $pattern, 
            $textDomain = 'default')
    {
    ...
    protected function loadMessages ($textDomain, $locale)
    {
        ...
        if (! $messagesLoaded) {
            ...
            if ($this->isEventManagerEnabled()) {
                $results = $this->getEventManager()->trigger(
                        self::EVENT_NO_MESSAGES_LOADED, $this, 
                        array(
                                'locale' => $locale,
                                'text_domain' => $textDomain
                        ), 
    ...
    protected function loadMessagesFromRemote ($textDomain, $locale)
    {
        ...
        if (isset($this->remote[$textDomain])) {
            foreach ($this->remote[$textDomain] as $loaderType) {
                ...
                if (isset($this->messages[$textDomain][$locale])) {
                    $this->messages[$textDomain][$locale]->merge(
                            $loader->load($locale, $textDomain));
                } else {
                    $this->messages[$textDomain][$locale] = $loader->load(
                            $locale, $textDomain);
                }
