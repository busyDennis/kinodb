...
abstract class AbstractObject implements ObjectInterface
{
    ...
    public function __construct ($options = null)
    {
        ...
        $this->type = strtolower(
                substr(get_class($this), strlen($this->barcodeNamespace) + 1));
    ...
    {
        $this->orientation = floatval($value) -
     ...
     */
    protected function addText ($text, $size, $position, $font, $color, 
    ...
    {
        return (int) $this->withBorder + $this->calculateBarcodeWidth() +
                 (int) $this->withBorder;
    }
    ...
    {
        return (int) $this->withBorder * 2 + $this->calculateBarcodeHeight() +
...
if ($this->width === null || $recalculate) {
    $this->width = abs(
            $this->calculateWidth() * cos($this->orientation / 180 * pi())) + abs(
            $this->calculateHeight() * sin($this->orientation / 180 * pi()));
}
return $this->width;
}
...
if ($this->offsetLeft === null || $recalculate) {
    $this->offsetLeft = - min(
            array(
                    0 * cos($this->orientation / 180 * pi()) - 0 * sin(
                            $this->orientation / 180 * pi()),
                    0 * cos($this->orientation / 180 * pi()) - $this->calculateBarcodeHeight() * sin(
                            $this->orientation / 180 * pi()),
                    $this->calculateBarcodeWidth() *
                             cos($this->orientation / 180 * pi()) - $this->calculateBarcodeHeight() * sin(
                                    $this->orientation / 180 * pi()),
    ...
    $this->offsetTop = - min(
            array(
                    ...
                    $this->calculateBarcodeHeight() *
                             cos($this->orientation / 180 * pi()) + $this->calculateBarcodeWidth() * sin(
                                    $this->orientation / 180 * pi()),
                            0 * cos($this->orientation / 180 * pi()) + $this->calculateBarcodeWidth() * sin(
                                    $this->orientation / 180 * pi())
            ));
}
...
$x2 = $x1 * cos($this->orientation / 180 * pi()) -
         $y1 * sin($this->orientation / 180 * pi()) + $this->getOffsetLeft();
$y2 = $y1 * cos($this->orientation / 180 * pi()) +
         $x1 * sin($this->orientation / 180 * pi()) + $this->getOffsetTop();
return array(
        intval($x2),
        intval($y2)
);
...
$point3 = $this->rotate($this->calculateWidth() - 1, 
...
foreach ($barcodeTable as $bar) {
    ...
    if ($bar[0]) {
        ...
        $point3 = $this->rotate($xpos + $width - 1, 
                $ypos + $bar[3] * $barLength);
        $point4 = $this->rotate($xpos + $width - 1, 
...
if ($this->drawText) {
    ...
    if ($this->stretchText) {
        ...
        $space = ($this->calculateWidth() - 2 * $this->getQuietZone()) /
                 $textLength;
        for ($i = 0; $i < $textLength; $i ++) {
            $leftPosition = $this->getQuietZone() + $space * ($i + 0.5);
            $this->addText($text{$i}, $this->fontSize * $this->factor, 
                    $this->rotate($leftPosition, 
                            (int) $this->withBorder * 2 +
                                     $this->factor *
                                     ($this->barHeight + $this->fontSize) + 1), 
                    $this->font, $this->foreColor, 'center', 
                    - $this->orientation);
        }
    } else {
        $this->addText($text, $this->fontSize * $this->factor, 
                $this->rotate($this->calculateWidth() / 2, 
                        (int) $this->withBorder * 2 + $this->factor *
                                 ($this->barHeight + $this->fontSize) + 1), 
                $this->font, $this->foreColor, 'center', - $this->orientation);
    }
}
}
