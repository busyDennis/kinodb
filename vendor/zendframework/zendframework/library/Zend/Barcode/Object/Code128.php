...
class Code128 extends AbstractObject
{
    ...
    protected function calculateBarcodeWidth ()
    {
        ...
        $encodedData += $characterLength +
                 2 * $this->barThinWidth * $this->factor;
    ...
    protected function convertToBarcodeChars ($string)
    {
        ...
        for ($pos = 0; $pos < strlen($string); $pos ++) {
            ...
            if (static::_isDigit($string, $pos, 4) && $currentCharset != 'C' ||
                ...
                if ($currentCharset != 'C') {
                    if ($pos == 0) {
                        ...
                        $code = array_search("Code C", 
                                $this->charSets[$currentCharset]);
                    }
                    $result[] = $code;
                    $currentCharset = 'C';
                }
            } elseif (in_array($char, $this->charSets['B']) &&
                     $currentCharset != 'B' && ! (in_array($char, 
                            $this->charSets['A']) && $currentCharset == 'A')) {
                /**
                 * Switch to B as B contains the char and B is not the current
                 * charset.
                 */
                if ($pos == 0) {
                ...
                $result[] = $code;
                $currentCharset = 'B';
            } elseif (array_key_exists($char, $this->charSets['A']) &&
                     $currentCharset != 'A' && ! (array_key_exists($char, 
                            $this->charSets['B']) && $currentCharset == 'B')) {
                /**
                 * Switch to C as C contains the char and C is not the current
                 * charset.
                 */
                if ($pos == 0) {
