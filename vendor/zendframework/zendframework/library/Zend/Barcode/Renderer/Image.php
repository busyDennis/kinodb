...
class Image extends AbstractRenderer
{
    ...
    public function __construct ($options = null)
    {
        if (! function_exists('gd_info')) {
            throw new RendererCreationException(
                    __CLASS__ . ' requires the GD extension');
        }
    ...
    protected function initRenderer ()
    {
        ...
        if ($this->resource !== null) {
            ...
            $this->imageBackgroundColor = imagecolorallocate($this->resource, 
                    ($backgroundColor & 0xFF0000) >> 16, 
                    ($backgroundColor & 0x00FF00) >> 8, 
                    $backgroundColor & 0x0000FF);
            $this->imageForeColor = imagecolorallocate($this->resource, 
            ...
            imagefilledrectangle($this->resource, 0, 0, $width - 1, $height - 1, 
                    $white);
        }
        $this->adjustPosition(imagesy($this->resource), 
                imagesx($this->resource));
        imagefilledrectangle($this->resource, $this->leftOffset, 
                $this->topOffset, $this->leftOffset + $barcodeWidth - 1, 
     ...
     * @throws Exception\RuntimeException
     */
    protected function drawText ($text, $size, $position, $font, $color, 
            $alignment = 'center', $orientation = 0)
    {
        ...
        if (is_numeric($font)) {
            ...
            imagestring($this->resource, $font, $positionX, $positionY, $text, 
                    $color);
        } else {
            ...
            imagettftext($this->resource, $size, $orientation, 
