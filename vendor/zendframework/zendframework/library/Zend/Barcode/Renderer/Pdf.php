...
class Pdf extends AbstractRenderer
{
    ...
    protected function drawPolygon ($points, $color, $filled = true)
    {
        ...
        foreach ($points as $point) {
            ...
            $y[] = $page->getHeight() - $point[1] * $this->moduleSize -
                     $this->topOffset;
        }
        if (count($y) == 4) {
            if ($x[0] != $x[3] && $y[0] == $y[3]) {
        ...
        $color = new Color\Rgb((($color & 0xFF0000) >> 16) / 255.0, 
            ...
            $alignment = 'center', $orientation = 0)
    {
        ...
        $page->setLineColor($color);
        $page->setFillColor($color);
        $page->setFont(Font::fontWithPath($font), 
                $size * $this->moduleSize * 1.2);
        
        $width = $this->widthForStringUsingFontSize($text, 
                Font::fontWithPath($font), $size * $this->moduleSize);
        
        $angle = pi() * $orientation / 180;
        $left = $position[0] * $this->moduleSize + $this->leftOffset;
        $top = $page->getHeight() - $position[1] * $this->moduleSize -
                 $this->topOffset;
