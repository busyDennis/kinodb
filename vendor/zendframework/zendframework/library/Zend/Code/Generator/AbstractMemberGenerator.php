...
abstract class AbstractMemberGenerator extends AbstractGenerator
{
    ...
    {
        return (($isStatic) ? $this->addFlag(self::FLAG_STATIC) : $this->removeFlag(
                self::FLAG_STATIC));
    }
    ...
    {
        return (bool) ($this->flags & self::FLAG_STATIC); // is FLAG_STATIC in
                                                              // flags
    }
    ...
    public function setVisibility ($visibility)
    {
        switch ($visibility) {
            case self::VISIBILITY_PUBLIC:
                $this->removeFlag(self::FLAG_PRIVATE | self::FLAG_PROTECTED); // remove
                                                                              // both
                $this->addFlag(self::FLAG_PUBLIC);
                break;
            case self::VISIBILITY_PROTECTED:
                $this->removeFlag(self::FLAG_PUBLIC | self::FLAG_PRIVATE); // remove
                                                                           // both
                $this->addFlag(self::FLAG_PROTECTED);
                break;
            case self::VISIBILITY_PRIVATE:
                $this->removeFlag(self::FLAG_PUBLIC | self::FLAG_PROTECTED); // remove
                                                                             // both
