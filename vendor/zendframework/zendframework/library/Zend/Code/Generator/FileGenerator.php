...
class FileGenerator extends AbstractGenerator
{
     ...
     *         @throes Exception\RuntimeException If file exists but is not
            ...
            $includeIfNotAlreadyIncluded = true)
    {
        ...
        if (! $realpath) {
            throw new Exception\InvalidArgumentException(
                    sprintf('No file for %s was found.', $filePath));
        }
            ...
            switch (strtolower(
                ...
                case 'class':
                    $fileGenerator->setClass(
                            ($value instanceof ClassGenerator) ? $value : ClassGenerator::fromArray(
                                    $value));
                    continue;
                case 'requiredfiles':
     ...
     * as part.
     * If $withResolvedAs is set to true, there will be a third element that is
     * the
    ...
    public function getUses ($withResolvedAs = false)
    {
        ...
        if ($withResolvedAs) {
            for ($useIndex = 0, $count = count($uses); $useIndex < $count; $useIndex ++) {
                if ($uses[$useIndex][1] == '') {
                    if (($lastSeparator = strrpos($uses[$useIndex][0], '\\')) !==
                             false) {
                        $uses[$useIndex][2] = substr($uses[$useIndex][0], 
                                $lastSeparator + 1);
                    } else {
    ...
    public function generate ()
    {
        ...
        if (preg_match('#/\* Zend_Code_Generator_FileGenerator-(.*?)Marker:#', 
                $body)) {
        ...
        if ($namespace) {
            $output .= sprintf('namespace %s;%s', $namespace, 
                    str_repeat(self::LINE_FEED, 2));
        }
        ...
        if (! empty($uses)) {
            foreach ($uses as $use) {
                ...
                if (null === $alias) {
                    ...
                    $output .= sprintf('use %s as %s;%s', $import, $alias, 
                            self::LINE_FEED);
                }
            }
            $output .= self::LINE_FEED;
