...
class DocBlockReflection implements ReflectionInterface
{
    ...
    {
        $this->tagManager = $tagManager ?  : new DocBlockTagManager(
                DocBlockTagManager::USE_DEFAULT_PROTOTYPES);
    ...
    protected function reflect ()
    {
        ...
        $this->cleanDocComment = preg_replace(
                "#[ \t]*(?:/\*\*|\*/|\*)[ ]{0,1}(.*)?#", '$1', $docComment);
        ...
        foreach ($scanner->getTags() as $tag) {
            $this->tags[] = $this->tagManager->createTag(
                    ltrim($tag['name'], '@'), ltrim($tag['value']));
        }
