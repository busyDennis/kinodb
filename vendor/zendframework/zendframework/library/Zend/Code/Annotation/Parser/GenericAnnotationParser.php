...
class GenericAnnotationParser implements ParserInterface
{
    ...
    public function registerAnnotation ($annotation)
    {
        ...
        if (! $annotation instanceof AnnotationInterface) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            (is_object($annotation) ? get_class($annotation) : gettype(
                                    $annotation))));
        }
        ...
        if (in_array($class, $this->annotationNames)) {
            throw new Exception\InvalidArgumentException(
                    sprintf('An annotation for this class %s already exists', 
                            $class));
        }
        ...
        if (! is_array($annotations) && ! $annotations instanceof Traversable) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, 
                            (is_object($annotations) ? get_class($annotations) : gettype(
                                    $annotations))));
        }
