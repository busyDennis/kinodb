...
class MethodScanner implements ScannerInterface
{
     ...
     */
    public function __construct (array $methodTokens, 
        ...
        if (is_int($parameterNameOrInfoIndex)) {
            ...
            if ($info['type'] != 'parameter') {
                throw new Exception\InvalidArgumentException(
                        'Index of info offset is not about a parameter');
            }
        } elseif (is_string($parameterNameOrInfoIndex)) {
            foreach ($this->infos as $info) {
                if ($info['type'] === 'parameter' &&
                         $info['name'] === $parameterNameOrInfoIndex) {
                    break;
                }
                unset($info);
            }
    ...
    protected function scan ()
    {
        ...
        $MACRO_INFO_START = function  () use( &$infoIndex, &$infos, &$tokenIndex, 
        &$tokenLine)
        {
