...
class DocBlockScanner implements ScannerInterface
{
    ...
    protected function tokenize ()
    {
        ...
        $MACRO_TOKEN_APPEND_WORD_PARTIAL = function  ($length) use( 
        &$currentWord, &$tokens, &$tokenIndex)
        {
        ...
        if ($currentChar === ' ' || $currentChar === "\t") {
            $MACRO_TOKEN_SET_TYPE(
                    ($context & $CONTEXT_INSIDE_ASTERISK) ? 'DOCBLOCK_WHITESPACE' : 'DOCBLOCK_WHITESPACE_INDENT');
            $MACRO_TOKEN_APPEND_WORD();
            $MACRO_TOKEN_ADVANCE();
            if ($MACRO_STREAM_ADVANCE_WORD() === false) {
                goto TOKENIZER_END;
        ...
        if ($currentChar === '*') {
            if (($context & $CONTEXT_INSIDE_DOCBLOCK) &&
                     ($context & $CONTEXT_INSIDE_ASTERISK)) {
