...
class Util
{
     ...
     */
    public static function resolveImports (&$value, $key = null, 
            stdClass $data = null)
    {
        if (! is_object($data) || ! property_exists($data, 'uses') ||
        ...
        if ($data->namespace && ! $data->uses && strlen($value) > 0 &&
                 $value{0} != '\\') {
        ...
        if ($data->namespace || $data->uses) {
            ...
            if (array_key_exists($firstPart, $data->uses)) {
                $value = substr_replace($value, $data->uses[$firstPart], 0, 
                        $firstPartEnd);
