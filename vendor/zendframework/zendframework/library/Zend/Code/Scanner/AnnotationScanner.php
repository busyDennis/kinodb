...
class AnnotationScanner extends AnnotationCollection implements ScannerInterface
{
    ...
    public function __construct (AnnotationManager $annotationManager, 
            $docComment, NameInformation $nameInformation = null)
    {
    ...
    protected function scan (array $tokens)
    {
        ...
        switch ($token[0]) {
            ...
            case 'ANNOTATION_NEWLINE':
                ...
                if (! $contentEnd && isset($annotations[$annotationIndex]) &&
        ...
        foreach ($annotations as $annotation) {
            ...
            $annotationObject = $this->annotationManager->createAnnotation(
                    $annotation);
            if ($annotationObject) {
    ...
    protected function tokenize ()
    {
        ...
        &$stream, &$streamIndex, &$currentChar, &$currentWord, &$currentLine)
        {
            $positionsForward = ($positionsForward > 0) ? $positionsForward : 1;
            $streamIndex = ($streamIndex === null) ? 0 : $streamIndex +
                     $positionsForward;
            ...
            $currentLine = (preg_match('#(.*)\n#', $stream, $matches, null, 
                    $streamIndex) === 1) ? $matches[1] : substr($stream, 
                    $streamIndex);
            if ($currentChar === ' ') {
                $currentWord = (preg_match('#( +)#', $currentLine, $matches) ===
                         1) ? $matches[1] : $currentLine;
            } else {
                $currentWord = (($matches = strpos($currentLine, ' ')) !== false) ? substr(
                        $currentLine, 0, $matches) : $currentLine;
            }
            
            return $currentChar;
        };
        $MACRO_STREAM_ADVANCE_WORD = function  () use( &$currentWord, 
        &$MACRO_STREAM_ADVANCE_CHAR)
        {
            return $MACRO_STREAM_ADVANCE_CHAR(strlen($currentWord));
        };
        $MACRO_STREAM_ADVANCE_LINE = function  () use( &$currentLine, 
        &$MACRO_STREAM_ADVANCE_CHAR)
        {
        ...
        $MACRO_TOKEN_APPEND_CHAR = function  () use( &$currentChar, &$tokens, 
        &$tokenIndex)
        {
            $tokens[$tokenIndex][1] .= $currentChar;
        };
        $MACRO_TOKEN_APPEND_WORD = function  () use( &$currentWord, &$tokens, 
        &$tokenIndex)
        {
            $tokens[$tokenIndex][1] .= $currentWord;
        };
        $MACRO_TOKEN_APPEND_LINE = function  () use( &$currentLine, &$tokens, 
        &$tokenIndex)
        {
        ...
        if ($currentChar === ' ') {
            $MACRO_TOKEN_SET_TYPE(
                    ($MACRO_HAS_CONTEXT($CONTEXT_ASTERISK)) ? 'ANNOTATION_WHITESPACE' : 'ANNOTATION_WHITESPACE_INDENT');
            $MACRO_TOKEN_APPEND_WORD();
            $MACRO_TOKEN_ADVANCE();
            if ($MACRO_STREAM_ADVANCE_WORD() === false) {
                goto TOKENIZER_END;
        ...
        if ($MACRO_HAS_CONTEXT($CONTEXT_CONTENT) &&
        ...
        if ($currentChar === '(' &&
                 $tokens[$tokenIndex - 1][0] === 'ANNOTATION_CLASS') {
            $context |= $CONTEXT_CONTENT;
        ...
        if ($currentChar === '*') {
            if ($MACRO_HAS_CONTEXT($CONTEXT_DOCBLOCK) &&
                     ($MACRO_HAS_CONTEXT($CONTEXT_ASTERISK))) {
