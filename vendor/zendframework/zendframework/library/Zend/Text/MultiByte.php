...
class MultiByte
{
    ...
    {
        trigger_error(
                sprintf("This method is deprecated, please use '%s' instead", 
                        'Zend\Stdlib\StringUtils::getWrapper(<charset>)->wordWrap'), 
                E_USER_DEPRECATED);
        
        try {
            return StringUtils::getWrapper($charset)->wordWrap($string, $width, 
                    $break, $cut);
            ...
            $padType = STR_PAD_RIGHT, $charset = 'utf-8')
    {
        ...
        return StringUtils::getWrapper($charset)->strPad($input, $padLength, 
                $padString, $padType);
    }
}
