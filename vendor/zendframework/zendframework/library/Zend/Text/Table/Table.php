...
class Table
{
     ...
     * @throws Exception\UnexpectedValueException When no columns widths were
     *         set
     */
    ...
    public function appendRow ($row)
    {
        if (! is_array($row) && ! ($row instanceof Row)) {
            throw new Exception\InvalidArgumentException(
                    '$row must be an array or instance of Zend\Text\Table\Row');
        }
    ...
    public function render ()
    {
        ...
        foreach ($this->rows as $rowNum => $row) {
            ...
            $renderedRow = $row->render($this->columnWidths, $this->decorator, 
                    $this->padding);
            ...
            if ($rowNum === 0 && $hasHorizontal) {
                ...
                if (! $hasHorizontal) {
                    ...
                    $drawSeparator = true;
                         ...
                         $this->autoSeparate & self::AUTO_SEPARATE_HEADER) {
                    $drawSeparator = true;
                } elseif ($rowNum === ($numRows - 1) &&
                         $this->autoSeparate & self::AUTO_SEPARATE_FOOTER) {
                    $drawSeparator = true;
                } else {
                    $drawSeparator = false;
                }
