...
class Client implements Stdlib\DispatchableInterface
{
    ...
    public function setUri ($uri)
    {
        if (! empty($uri)) {
            ...
            $lastHost = $this->getRequest()
                ->getUri()
                ->getHost();
            $this->getRequest()->setUri($uri);
            
            // if host changed, the HTTP authentication should be cleared for
            // security
            // reasons, see #4215 for a discussion - currently authentication is
            // also
            // cleared for peer subdomains due to technical limits
            $nextHost = $this->getRequest()
                ->getUri()
                ->getHost();
            if (! preg_match('/' . preg_quote($lastHost, '/') . '$/i', 
                    $nextHost)) {
                $this->clearAuth();
            }
    ...
    public function setMethod ($method)
    {
        ...
        if (($method == Request::METHOD_POST || $method == Request::METHOD_PUT ||
                 ...
                 $method == Request::METHOD_PATCH) && empty($this->encType)) {
     ...
     * @return Client
     */
    public function addCookie ($cookie, $value = null, $expire = null, $path = null, 
            $domain = null, $secure = false, $httponly = true, $maxAge = null, $version = null)
    {
            ...
            foreach ($cookie as $setCookie) {
                if ($setCookie instanceof Header\SetCookie) {
                    ...
                    throw new Exception\InvalidArgumentException(
                            'The cookie parameter is not a valid Set-Cookie type');
                }
            }
        } elseif (is_string($cookie) && $value !== null) {
            $setCookie = new Header\SetCookie($cookie, $value, $expire, $path, 
                    $domain, $secure, $httponly, $maxAge, $version);
            $this->cookies[$this->getCookieId($setCookie)] = $setCookie;
    ...
    public function setCookies ($cookies)
    {
        if (is_array($cookies)) {
            ...
            throw new Exception\InvalidArgumentException(
                    'Invalid cookies passed as parameter, it must be an array');
        }
        return $this;
    }
    ...
    protected function openTempStream ()
    {
        ...
        if (false === $fp) {
            ...
            throw new Exception\RuntimeException(
                    "Could not open temp file {$this->streamName}", 0, $error);
        }
     ...
     * @return string|bool
     */
    protected function calcAuthDigest ($user, $password, 
            $type = self::AUTH_BASIC, $digest = array(), $entityBody = null)
    {
        if (! defined('self::AUTH_' . strtoupper($type))) {
            throw new Exception\InvalidArgumentException(
                    "Invalid or not supported authentication type: '$type'");
        ...
        switch (strtolower($type)) {
            ...
            case self::AUTH_DIGEST:
                ...
                if (empty($digest['qop']) || strtolower($digest['qop']) == 'auth') {
                    $ha2 = md5(
                            $this->getMethod() . ':' . $this->getUri()->getPath());
     ...
     */
    public function dispatch (Stdlib\RequestInterface $request, 
    ...
    public function send (Request $request = null)
    {
        ...
        do {
            ...
            // cookies
            $cookie = $this->prepareCookies($uri->getHost(), $uri->getPath(), 
                    $secure);
            ...
            if (is_resource($body) &&
                     ! ($this->adapter instanceof Client\Adapter\StreamInterface)) {
            ...
            $response = $this->doRequest($uri, $method, $secure, $headers, 
                    $body);
            ...
            if ($response->isRedirect() &&
                ...
                if ($response->getStatusCode() == 303 || ((! $this->config['strictredirects']) && ($response->getStatusCode() ==
                         302 || $response->getStatusCode() == 301))) {
    ...
    public function setFileUpload ($filename, $formname, $data = null, $ctype = null)
    {
        if ($data === null) {
            ...
            if ($data === false) {
                throw new Exception\RuntimeException(
                        "Unable to read file '{$filename}' for upload", 0, 
                        $error);
            }
            if (! $ctype) {
        ...
        if (! empty($this->cookies)) {
            foreach ($this->cookies as $id => $cookie) {
                ...
                if ($cookie->isValidForRequest($domain, $path, $secure)) {
                    // OAM hack some domains try to set the cookie multiple
                    // times
    ...
    protected function prepareHeaders ($body, $uri)
    {
        ...
        if (! empty($this->auth)) {
            switch ($this->auth['type']) {
                case self::AUTH_BASIC:
                    $auth = $this->calcAuthDigest($this->auth['user'], 
                ...
                case self::AUTH_DIGEST:
                    throw new Exception\RuntimeException(
                            "The digest authentication is not implemented yet");
            }
        }
    ...
    protected function prepareBody ()
    {
        ...
        if (count(
            ...
            if (stripos($this->getEncType(), self::ENC_FORMDATA) === 0) {
                ...
                $params = self::flattenParametersArray(
                        $this->getRequest()
                            ->getPost()
                            ->toArray());
                ...
                foreach ($this->getRequest()
                    ...
                    $body .= $this->encodeFormData($boundary, $file['formname'], 
                            $file['data'], $file['filename'], $fhead);
                }
                $body .= "--{$boundary}--\r\n";
     ...
     * @return string
     */
    public function encodeFormData ($boundary, $name, $value, $filename = null, 
            $headers = array())
    {
        ...
        foreach ($parray as $name => $value) {
            ...
            if (is_array($value)) {
                $parameters = array_merge($parameters, 
                        $this->flattenParametersArray($value, $key));
            } else {
                $parameters[] = array(
                        $key,
                        $value
     ...
     * @throws Exception\RuntimeException
     */
    protected function doRequest (Http $uri, $method, $secure = false, 
            $headers = array(), $body = '')
    {
        ...
        // HTTP connection
        $this->lastRawRequest = $this->adapter->write($method, $uri, 
