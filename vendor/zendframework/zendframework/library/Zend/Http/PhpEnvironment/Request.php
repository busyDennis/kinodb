...
class Request extends HttpRequest
{
    ...
    public function setServer (ParametersInterface $server)
    {
        ...
        if (function_exists('apache_request_headers')) {
            ...
            if (! isset($this->serverParams['HTTP_AUTHORIZATION'])) {
                if (isset($apacheRequestHeaders['Authorization'])) {
                    $this->serverParams->set('HTTP_AUTHORIZATION', 
        ...
        // set HTTP version
        if (isset($this->serverParams['SERVER_PROTOCOL']) && strpos(
                $this->serverParams['SERVER_PROTOCOL'], self::VERSION_10) !==
                 false) {
        ...
        if ($this->getHeaders()->get('host')) {
            ...
            // set up a validator that check if the hostname is legal (not
            // spoofed)
        ...
        if (! $host && isset($this->serverParams['SERVER_NAME'])) {
            ...
            if (isset($this->serverParams['SERVER_ADDR']) &&
     ...
     *            Parameter name to retrieve, or null to get the whole
     *            container.
     ...
     *            container.
     * @param mixed|null $default
     *            Default value to use when the parameter is missing. * @return
        ...
        foreach ($_FILES as $fileName => $fileParams) {
            ...
            foreach ($fileParams as $param => $data) {
                if (! is_array($data)) {
                    ...
                    foreach ($data as $i => $v) {
                        $this->mapPhpFileParam($files[$fileName], $param, $i, 
                                $v);
                    }
                }
            }
