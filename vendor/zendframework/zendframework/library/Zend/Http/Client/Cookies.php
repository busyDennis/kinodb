 ...
 * be used along with Zend\Http\Client in order to manage cookies across HTTP
 * requests and
 * responses.
 *
 * The class contains an array of Zend\Http\Header\Cookie objects. Cookies can
 * be added
 * automatically from a request or manually. Then, the Cookies class can find
 * and return the
 * cookies needed for a specific HTTP request.
 *
 * A special parameter can be passed to all methods of this class that return
 * cookies: Cookies
 * can be returned either in their native form (as Zend\Http\Header\Cookie
...
class Cookies
{
     ...
     *
     * @param int $retAs
     *            Whether to return cookies as objects of
    ...
    public function getCookie ($uri, $cookieName, $retAs = self::COOKIE_OBJECT)
    {
        ...
        if (isset($this->cookies[$uri->getHost()][$path][$cookieName])) {
            ...
            switch ($retAs) {
                ...
                default:
                    throw new Exception\InvalidArgumentException(
                            "Invalid value passed for \$retAs: {$retAs}");
                    break;
            }
        }
        ...
        if (is_array($ptr)) {
            ...
            foreach ($ptr as $item) {
                if ($retAs == self::COOKIE_STRING_CONCAT) {
                    ...
                    $ret = array_merge($ret, 
                            $this->_flattenCookiesArray($item, $retAs));
                }
            }
            return $ret;
     ...
     *         </p>
     *         <p>
     *         The return value will be casted to boolean if non-boolean was
     *         returned.
     */
