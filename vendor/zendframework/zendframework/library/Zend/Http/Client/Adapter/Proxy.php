...
class Proxy extends Socket
{
    ...
    public function connect ($host, $port = 80, $secure = false)
    {
        ...
        parent::connect($this->config['proxy_host'], 
    ...
    public function write ($method, $uri, $httpVer = '1.1', $headers = array(), $body = '')
    {
        ...
        if ($this->connectedTo[0] != "tcp://$host" ||
        ...
        if ($uri->getScheme() == 'https' && (! $this->negotiated)) {
            $this->connectHandshake($uri->getHost(), $uri->getPort(), $httpVer, 
                    $headers);
        ...
        if (! $test) {
            throw new AdapterException\RuntimeException(
                    "Error writing request to proxy server", 0, $error);
        }
     ...
     * @throws AdapterException\RuntimeException
     */
    protected function connectHandshake ($host, $port = 443, $httpVer = '1.1', 
            array &$headers = array())
    {
