...
class Socket implements HttpAdapter, StreamInterface
{
    ...
    public function setOptions ($options = array())
    {
        ...
        if (! is_array($options)) {
            throw new AdapterException\InvalidArgumentException(
                    'Array or Zend\Config object expected, got ' .
                             gettype($options));
        }
    ...
    public function connect ($host, $port = 80, $secure = false)
    {
        ...
        $connectedHost = (strpos($this->connectedTo[0], '://')) ? substr(
                $this->connectedTo[0], 
                (strpos($this->connectedTo[0], '://') + 3), 
                strlen($this->connectedTo[0])) : $this->connectedTo[0];
        ...
        if (! is_resource($this->socket) || ! $this->config['keepalive']) {
            ...
            if ($secure || $this->config['sslusecontext']) {
                if ($this->config['sslverifypeer'] !== null) {
                    if (! stream_context_set_option($context, 'ssl', 
                        ...
                        throw new AdapterException\RuntimeException(
                                'Unable to set sslverifypeer option');
                    }
                }
                ...
                if ($this->config['sslcapath']) {
                    if (! stream_context_set_option($context, 'ssl', 'capath', 
                        ...
                        throw new AdapterException\RuntimeException(
                                'Unable to set sslcapath option');
                    }
                }
                ...
                if ($this->config['sslallowselfsigned'] !== null) {
                    if (! stream_context_set_option($context, 'ssl', 
                        ...
                        throw new AdapterException\RuntimeException(
                                'Unable to set sslallowselfsigned option');
                    }
                }
            ...
            if (! $this->socket) {
                $this->close();
                throw new AdapterException\RuntimeException(
                        sprintf('Unable to connect to %s:%d%s', $host, $port, 
                                ($error ? ' . Error #' . $error->getCode() . ': ' .
            ...
            if ($secure || $this->config['sslusecontext']) {
                if ($this->config['ssltransport'] &&
                         isset(
                                static::$sslCryptoTypes[$this->config['ssltransport']])) {
                ...
                ErrorHandler::start();
                $test = stream_socket_enable_crypto($this->socket, true, 
                        $sslCryptoMethod);
                $error = ErrorHandler::stop();
                if (! $test || $error) {
                    ...
                    if ((! $errorString) && $this->config['sslverifypeer']) {
                        // There's good chance our error is due to sslcapath not
                        // being properly set
                        if (! ($this->config['sslcapath'] &&
                                 is_dir($this->config['sslcapath']))) {
    ...
    public function write ($method, $uri, $httpVer = '1.1', $headers = array(), $body = '')
    {
        ...
        $host = (strtolower($uri->getScheme()) == 'https' ? $this->config['ssltransport'] : 'tcp') .
                 '://' . $host;
        if ($this->connectedTo[0] != $host ||
                 $this->connectedTo[1] != $uri->getPort()) {
            throw new AdapterException\RuntimeException(
        ...
        if (false === $test) {
            throw new AdapterException\RuntimeException(
                    'Error writing request to server', 0, $error);
        }
    ...
    public function read ()
    {
        ...
        if ($transferEncoding !== false) {
            ...
            if (strtolower($transferEncoding->getFieldValue()) == 'chunked') {
                ...
                do {
                    ...
                    do {
                        ...
                        if ($this->outStream) {
                            if (stream_copy_to_stream($this->socket, 
                                    $this->outStream, $readTo - $currentPos) == 0) {
                                $this->_checkSocketReadTimeout();
                                break;
                            }
                        } else {
                ...
                $this->close();
                throw new AdapterException\RuntimeException(
                        'Cannot handle "' . $transferEncoding->getFieldValue() .
                                 '" transfer encoding');
            }
            
            // We automatically decode chunked-messages when writing to a stream
            // this means we have to disallow the Zend\Http\Response to do it
            // again
            if ($this->outStream) {
                $response = str_ireplace("Transfer-Encoding: chunked\r\n", '', 
                        $response);
            }
            // Else, if we got the content-length header, read this number of
            // bytes
            ...
            for ($readTo = $currentPos + $contentLength; $readTo > $currentPos; $currentPos = ftell(
                    $this->socket)) {
                
                if ($this->outStream) {
                    if (stream_copy_to_stream($this->socket, $this->outStream, 
                            $readTo - $currentPos) == 0) {
                        $this->_checkSocketReadTimeout();
                        break;
                    }
