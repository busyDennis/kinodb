...
class Curl implements HttpAdapter, StreamInterface
{
        ...
        foreach ($options as $k => $v) {
            ...
            switch ($option) {
                ...
                case 'proxyport':
                    ...
                    break;
                default:
                    if (is_array($v) && isset($this->config[$option]) &&
                             is_array($this->config[$option])) {
    ...
    public function connect ($host, $port = 80, $secure = false)
    {
        ...
        if ($this->curl && is_array($this->connectedTo) && ($this->connectedTo[0] !=
                 $host || $this->connectedTo[1] != $port)) {
            $this->close();
        }
        ...
        if (isset($this->config['maxredirects'])) {
            // Set Max redirects
            curl_setopt($this->curl, CURLOPT_MAXREDIRS, 
        ...
        if ($secure !== false) {
            ...
            if (isset($this->config['sslcert'])) {
                curl_setopt($this->curl, CURLOPT_SSLCERT, 
                        $this->config['sslcert']);
            }
            ...
            $body = '')
    {
        ...
        if ($this->connectedTo[0] != $uri->getHost() ||
        ...
        switch ($method) {
            ...
            case 'PUT':
                ...
                if (isset($this->config['curloptions'][CURLOPT_INFILE])) {
                    ...
                    if (! isset(
                            $this->config['curloptions'][CURLOPT_INFILESIZE])) {
                        throw new AdapterException\RuntimeException(
        ...
        if (array_key_exists('Authorization', $headers) &&
                 'Basic' == substr($headers['Authorization'], 0, 5)) {
            curl_setopt($this->curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
            curl_setopt($this->curl, CURLOPT_USERPWD, 
                    base64_decode(substr($headers['Authorization'], 6)));
        ...
        if ($method == 'POST') {
            ...
            curl_setopt($this->curl, CURLOPT_INFILE, 
                    $this->config['curloptions'][CURLOPT_INFILE]);
        ...
        if (empty($this->response)) {
            throw new AdapterException\RuntimeException(
                    "Error in cURL request: " . curl_error($this->curl));
        }
        ...
        if (stripos($this->response, 
                "HTTP/1.0 200 Connection established\r\n\r\n") !== false) {
            $this->response = str_ireplace(
                    "HTTP/1.0 200 Connection established\r\n\r\n", '', 
                    $this->response);
        }
