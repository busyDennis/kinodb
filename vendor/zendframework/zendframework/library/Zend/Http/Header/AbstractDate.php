...
abstract class AbstractDate implements HeaderInterface
{
    ...
    public static function fromString ($headerLine)
    {
        ...
        if (strtolower($name) !== strtolower($dateHeader->getFieldName())) {
            throw new Exception\InvalidArgumentException(
                    'Invalid header line for "' . $dateHeader->getFieldName() .
                             '" header string');
        }
    ...
    public function setDate ($date)
    {
        if (is_string($date)) {
            try {
                ...
                throw new Exception\InvalidArgumentException(
                        sprintf('Invalid date passed as string (%s)', 
                                (string) $date), $e->getCode(), $e);
            }
        } elseif (! ($date instanceof DateTime)) {
            throw new Exception\InvalidArgumentException(
                    'Date must be an instance of \DateTime or a string');
        }
     ...
     * Returns < 0 if date in header is less than $date; > 0 if it's greater,
    ...
    public function compareTo ($date)
    {
        ...
        return ($thisTimestamp === $dateTimestamp) ? 0 : (($thisTimestamp >
                 $dateTimestamp) ? 1 : - 1);
    }
