...
class HydratorListener extends AbstractListenerAggregate
{
    ...
    {
        $this->listeners[] = $events->attach(HydrateEvent::EVENT_HYDRATE, 
                array(
                        $this,
                        'onHydrate'
                ), $priority);
        $this->listeners[] = $events->attach(ExtractEvent::EVENT_EXTRACT, 
                array(
                        $this,
                        'onExtract'
                ), $priority);
    ...
    {
        $object = $this->hydrator->hydrate($event->getHydrationData(), 
                $event->getHydratedObject());
