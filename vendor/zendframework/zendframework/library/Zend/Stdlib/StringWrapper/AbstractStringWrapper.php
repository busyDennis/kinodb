...
abstract class AbstractStringWrapper implements StringWrapperInterface
{
    ...
    public static function isSupported ($encoding, $convertEncoding = null)
    {
        ...
        if ($convertEncoding !== null &&
                 ! in_array(strtoupper($convertEncoding), $supportedEncodings)) {
            return false;
        }
    ...
    public function setEncoding ($encoding, $convertEncoding = null)
    {
        ...
        if ($convertEncoding !== null) {
            ...
            if (! in_array($convertEncodingUpper, $supportedEncodings)) {
                throw new Exception\InvalidArgumentException(
                        'Wrapper doesn\'t support character encoding "' .
                                 $convertEncoding . '"');
            }
    ...
    public function wordWrap ($string, $width = 75, $break = "\n", $cut = false)
    {
        ...
        for ($current = 0; $current < $stringWidth; $current ++) {
            ...
            if ($possibleBreak === $break) {
                $result .= $this->substr($string, $lastStart, 
            ...
            if ($current - $lastStart >= $width && $cut &&
                     $lastStart >= $lastSpace) {
                $result .= $this->substr($string, $lastStart, 
                        $current - $lastStart) . $break;
     ...
     * @return string
     */
    public function strPad ($input, $padLength, $padString = ' ', 
            $padType = STR_PAD_RIGHT)
    {
        ...
        if ($padType === STR_PAD_BOTH) {
            ...
            $lastStringLeft = $this->substr($padString, 0, 
                    $lastStringLeftLength);
            $lastStringRight = $this->substr($padString, 0, 
                    $lastStringRightLength);
            
            return str_repeat($padString, $repeatCountLeft) . $lastStringLeft .
                     $input . str_repeat($padString, $repeatCountRight) .
                     $lastStringRight;
        }
        
        $lastString = $this->substr($padString, 0, 
                $lengthOfPadding % $padStringLength);
