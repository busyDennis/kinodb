...
class Rss extends AbstractEntry implements EntryInterface
{
    ...
    public function getDateModified ()
    {
        ...
        if ($this->getType() !== Reader\Reader::TYPE_RSS_10 &&
            ...
            $dateModified = $this->xpath->evaluate(
                    'string(' . $this->xpathQueryRss . '/pubDate)');
            if ($dateModified) {
                ...
                if ($dateModifiedParsed) {
                    ...
                    foreach ($dateStandards as $standard) {
                        try {
                            $date = date_create_from_format($standard, 
                                    $dateModified);
                            break;
                        } catch (\Exception $e) {
                            if ($standard == null) {
    ...
    public function getDescription ()
    {
        ...
        if ($this->getType() !== Reader\Reader::TYPE_RSS_10 &&
            ...
            $description = $this->xpath->evaluate(
                    'string(' . $this->xpathQueryRss . '/description)');
        } else {
            $description = $this->xpath->evaluate(
                    'string(' . $this->xpathQueryRdf . '/rss:description)');
        }
    ...
    public function getId ()
    {
        ...
        if ($this->getType() !== Reader\Reader::TYPE_RSS_10 &&
            ...
            $id = $this->xpath->evaluate(
                    'string(' . $this->xpathQueryRss . '/guid)');
        }
    ...
    public function getTitle ()
    {
        ...
        if ($this->getType() !== Reader\Reader::TYPE_RSS_10 &&
            ...
            $title = $this->xpath->evaluate(
                    'string(' . $this->xpathQueryRss . '/title)');
        } else {
            $title = $this->xpath->evaluate(
                    'string(' . $this->xpathQueryRdf . '/rss:title)');
        }
    ...
    public function getCommentLink ()
    {
        ...
        if ($this->getType() !== Reader\Reader::TYPE_RSS_10 &&
                 $this->getType() !== Reader\Reader::TYPE_RSS_090) {
            $commentlink = $this->xpath->evaluate(
                    'string(' . $this->xpathQueryRss . '/comments)');
        }
