...
class Reader
{
        ...
        if (static::$httpConditionalGet && $cache) {
            ...
            if ($response->getStatusCode() == 304) {
                ...
                if ($response->getHeaders()->get('ETag')) {
                    $cache->setItem($cacheId . '_etag', 
                            $response->getHeaders()
                                ->get('ETag')
                                ->getFieldValue());
                }
                if ($response->getHeaders()->get('Last-Modified')) {
                    $cache->setItem($cacheId . '_lastmodified', 
                            $response->getHeaders()
                                ->get('Last-Modified')
                                ->getFieldValue());
                }
        ...
        if (! $response instanceof Http\ResponseInterface) {
            throw new Exception\RuntimeException(
                    sprintf(
                            ...
                            __NAMESPACE__, 
                            (is_object($response) ? get_class($response) : gettype(
                                    $response))));
        }
    ...
    public static function importString ($string)
    {
        ...
        if (substr($type, 0, 3) == 'rss') {
            ...
            $reader = new Entry\Atom($dom->documentElement, 0, 
                    self::TYPE_ATOM_10);
    ...
    public static function importFile ($filename)
    {
        ...
        if ($feed === false) {
            throw new Exception\RuntimeException(
                    "File '{$filename}' could not be loaded", 0, $err);
        }
