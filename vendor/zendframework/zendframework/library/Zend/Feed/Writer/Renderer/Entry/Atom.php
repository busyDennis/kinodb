...
class Atom extends Renderer\AbstractRenderer implements 
        Renderer\RendererInterface
{
    ...
    public function render ()
    {
        ...
        $entry = $this->dom->createElementNS(Writer\Writer::NAMESPACE_ATOM_10, 
                'entry');
...
if (! Uri::factory($this->getDataContainer()->getId())->isValid() && ! preg_match(
        "#^urn:[a-zA-Z0-9][a-zA-Z0-9\-]{1,31}:([a-zA-Z0-9\(\)\+\,\.\:\=\@\;\$\_\!\*\-]|%[0-9a-fA-F]{2})*#", 
        $this->getDataContainer()->getId()) && ! $this->_validateTagUri(
        $this->getDataContainer()
            ->getId())) {
