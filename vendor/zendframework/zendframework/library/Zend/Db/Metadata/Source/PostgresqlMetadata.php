...
class PostgresqlMetadata extends AbstractSource
{
    ...
    protected function loadSchemaData ()
    {
        ...
        $sql = 'SELECT ' . $p->quoteIdentifier('schema_name') . ' FROM ' . $p->quoteIdentifierChain(
                array(
                        'information_schema',
                        'schemata'
                )) . ' WHERE ' . $p->quoteIdentifier('schema_name') .
                 ' != \'information_schema\'' . ' AND ' .
                 $p->quoteIdentifier('schema_name') . " NOT LIKE 'pg_%'";
    ...
    protected function loadTableNameData ($schema)
    {
        ...
        ' LEFT JOIN ' . $p->quoteIdentifierChain(
                array(
                        'information_schema',
                        'views'
                )) . ' v' . ' ON ' . $p->quoteIdentifierChain(
                array(
                        't',
                        'table_schema'
    ...
    protected function loadConstraintData ($table, $schema)
    {
        ...
        ' INNER JOIN ' . $p->quoteIdentifierChain(
                array(
                        'information_schema',
                        'table_constraints'
                )) . ' tc' . ' ON ' . $p->quoteIdentifierChain(
                array(
                        't',
                        'table_schema'
                )) . '  = ' . $p->quoteIdentifierChain(
        ...
        ' LEFT JOIN ' . $p->quoteIdentifierChain(
                array(
                        'information_schema',
                        'key_column_usage'
                )) . ' kcu2' . ' ON ' . $p->quoteIdentifierChain(
                array(
                        'rc',
        ...
        ' WHERE ' .
                 $p->quoteIdentifierChain(
                        array(
                                't',
                                'table_name'
                        )) . ' = ' . $p->quoteTrustedValue($table) . ' AND ' . $p->quoteIdentifierChain(
                        array(
                                't',
                                'table_type'
                        )) . ' IN (\'BASE TABLE\', \'VIEW\')';
        ...
        $sql .= ' ORDER BY CASE ' . $p->quoteIdentifierChain(
                array(
                        'tc',
                        'constraint_type'
                )) . " WHEN 'PRIMARY KEY' THEN 1" . " WHEN 'UNIQUE' THEN 2" .
                 " WHEN 'FOREIGN KEY' THEN 3" . " WHEN 'CHECK' THEN 4" .
                 " ELSE 5 END" . ', ' . $p->quoteIdentifierChain(
                        array(
                                'tc',
                                'constraint_name'
                        )) . ', ' . $p->quoteIdentifierChain(
