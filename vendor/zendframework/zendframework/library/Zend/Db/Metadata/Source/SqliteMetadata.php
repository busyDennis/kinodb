...
class SqliteMetadata extends AbstractSource
{
    ...
    protected function loadTableNameData ($schema)
    {
        ...
        $sql = 'SELECT "name", "type", "sql" FROM ' . $p->quoteIdentifierChain(
                array(
                        $schema,
                        'sqlite_master'
                )) .
    ...
    protected function loadColumnData ($table, $schema)
    {
        ...
        foreach ($results as $row) {
            $columns[$row['name']] = array(
                    // cid appears to be zero-based, ordinal position needs to
                    // be one-based
        ...
        foreach ($foreignKeys as $fk) {
            if ($id !== $fk['id']) {
                ...
                $constraints[$name] = array(
                        ...
                        // TODO: Verify match, on_update, and on_delete values
    ...
    protected function loadTriggerData ($schema)
    {
        ...
        $sql = 'SELECT "name", "tbl_name", "sql" FROM ' . $p->quoteIdentifierChain(
                array(
                        $schema,
                        'sqlite_master'
                )) . ' WHERE "type" = \'trigger\'';
