...
abstract class AbstractSource implements MetadataInterface
{
    ...
    public function getTable ($tableName, $schema = null)
    {
        ...
        switch ($data['table_type']) {
            ...
            default:
                throw new \Exception(
                        'Table "' . $tableName . '" is of an unsupported type "' .
                                 $data['table_type'] . '"');
        }
    ...
    public function getView ($viewName, $schema = null)
    {
        ...
        if (isset($tableNames[$viewName]) &&
                 'VIEW' == $tableNames[$viewName]['table_type']) {
    ...
    public function getConstraints ($table, $schema = null)
    {
        ...
        foreach (array_keys($this->data['constraints'][$schema][$table]) as $constraintName) {
            $constraints[] = $this->getConstraint($constraintName, $table, 
                    $schema);
        }
    ...
    public function getConstraint ($constraintName, $table, $schema = null)
    {
        ...
        if (! isset(
                $this->data['constraints'][$schema][$table][$constraintName])) {
            throw new \Exception(
                    'Cannot find a constraint by that name in this table');
        }
        
        $info = $this->data['constraints'][$schema][$table][$constraintName];
        $constraint = new Object\ConstraintObject($constraintName, $table, 
                $schema);
    ...
    public function getConstraintKeys ($constraint, $table, $schema = null)
    {
        ...
        foreach ($this->data['constraint_keys'][$schema] as $constraintKeyInfo) {
            if ($constraintKeyInfo['table_name'] == $table &&
                     $constraintKeyInfo['constraint_name'] === $constraint) {
                $keys[] = $key = new Object\ConstraintKeyObject(
                        $constraintKeyInfo['column_name']);
                ...
                if (isset($references[$constraint])) {
                    ...
                    $key->setForeignKeyUpdateRule(
                            $references[$constraint]['update_rule']);
                    $key->setForeignKeyDeleteRule(
                            $references[$constraint]['delete_rule']);
                    // $key->setReferencedTableSchema($references[$constraint]['referenced_table_schema']);
                    $key->setReferencedTableName(
                            $references[$constraint]['referenced_table_name']);
                    $key->setReferencedColumnName(
    ...
    public function getTrigger ($triggerName, $schema = null)
    {
        ...
        $trigger->setActionReferenceOldTable(
                $info['action_reference_old_table']);
        $trigger->setActionReferenceNewTable(
                $info['action_reference_new_table']);
