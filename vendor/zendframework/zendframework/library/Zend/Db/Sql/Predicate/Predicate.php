...
class Predicate extends PredicateSet
{
    ...
    public function nest ()
    {
        ...
        $this->addPredicate($predicateSet, 
     ...
     * @return Predicate
     */
    public function equalTo ($left, $right, $leftType = self::TYPE_IDENTIFIER, 
     ...
     * @return Predicate
     */
    public function notEqualTo ($left, $right, $leftType = self::TYPE_IDENTIFIER, 
            $rightType = self::TYPE_VALUE)
    {
        $this->addPredicate(
     ...
     * @return Predicate
     */
    public function lessThan ($left, $right, $leftType = self::TYPE_IDENTIFIER, 
            $rightType = self::TYPE_VALUE)
    {
        $this->addPredicate(
    ...
    {
        $this->addPredicate(
                new Operator($left, Operator::OPERATOR_LESS_THAN_OR_EQUAL_TO, 
    ...
    {
        $this->addPredicate(
                new Operator($left, Operator::OPERATOR_GREATER_THAN_OR_EQUAL_TO, 
                        $right, $leftType, $rightType), 
                ($this->nextPredicateCombineOperator) ?  : $this->defaultCombination);
        $this->nextPredicateCombineOperator = null;
    ...
    {
        // process deprecated parameters from previous literal($literal,
        // $parameters = null) signature
        if (func_num_args() >= 2) {
    ...
    {
        $this->addPredicate(new IsNull($identifier), 
                ($this->nextPredicateCombineOperator) ?  : $this->defaultCombination);
        $this->nextPredicateCombineOperator = null;
