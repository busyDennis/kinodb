...
class SelectDecorator extends Select implements PlatformDecoratorInterface
{
     ...
     * @return null
     */
    protected function processLimitOffset (PlatformInterface $platform, 
            DriverInterface $driver = null, 
            ParameterContainer $parameterContainer = null, &$sqls, &$parameters)
    {
        ...
        foreach ($selectParameters[0] as $i => $columnParameters) {
            if ($columnParameters[0] == self::SQL_STAR || (isset(
                    $columnParameters[1]) &&
                     $columnParameters[1] == self::SQL_STAR) ||
        ...
        // first, produce column list without compound names (using the AS
        // portion only)
        array_unshift($sqls, 
                $this->createSqlFromSpecificationAndParameters(
                        array(
                                'SELECT %1$s FROM (SELECT b.%1$s, rownum b_rownum FROM (' => current(
        ...
        if ($parameterContainer) {
            if ($this->limit === null) {
                ...
                // create bottom part of query, with offset and limit using
                // row_number
                array_push($sqls, 
                        ') b WHERE rownum <= (:offset+:limit)) WHERE b_rownum >= (:offset + 1)');
                $parameterContainer->offsetSet('offset', $this->offset, 
                        $parameterContainer::TYPE_INTEGER);
            ...
            if ($this->limit === null) {
                ...
                array_push($sqls, 
                        ') b WHERE rownum <= (' . (int) $this->offset . '+' .
                                 (int) $this->limit . ')) WHERE b_rownum >= (' .
                                 (int) $this->offset . ' + 1)');
            }
        }
            ...
            ParameterContainer $parameterContainer = null)
    {
        ...
        foreach ($this->joins as $j => $join) {
            ...
            // table name
            $joinSpecArgArray[$j][] = (is_array($join['name'])) ? $platform->quoteIdentifier(
                    current($join['name'])) . ' ' .
                     $platform->quoteIdentifier(key($join['name'])) : $platform->quoteIdentifier(
                            $join['name']);
            // on expression
            $joinSpecArgArray[$j][] = ($join['on'] instanceof ExpressionInterface) ? $this->processExpression(
                    $join['on'], $platform, $driver, 
                    $this->processInfo['paramPrefix'] . 'join') : $platform->quoteIdentifierInFragment(
                    $join['on'], 
                    array(
                            '=',
                            'AND',
