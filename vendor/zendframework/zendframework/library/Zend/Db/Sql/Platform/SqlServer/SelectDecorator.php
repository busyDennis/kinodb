...
class SelectDecorator extends Select implements PlatformDecoratorInterface
{
            ...
            ParameterContainer $parameterContainer = null, &$sqls, &$parameters)
    {
        ...
        if ($parameterContainer) {
            ...
            // row_number
            array_push($sqls, 
                    ') AS [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__ZEND_ROW_NUMBER] BETWEEN ?+1 AND ?+?');
            ...
            array_push($sqls, 
                    ') AS [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__ZEND_ROW_NUMBER] BETWEEN ' .
                             (int) $this->offset . '+1 AND ' . (int) $this->limit .
                             '+' . (int) $this->offset);
        }
