...
class Select extends AbstractSql implements SqlInterface, PreparableSqlInterface
{
    ...
    public function join ($name, $on, $columns = self::SQL_STAR, $type = self::JOIN_INNER)
    {
        if (is_array($name) && (! is_string(key($name)) || count($name) !== 1)) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            "join() expects '%s' as an array is a single element associative array", 
                            array_shift($name)));
        }
        if (! is_array($columns)) {
            $columns = array(
            ...
            $combination = Predicate\PredicateSet::OP_AND)
    {
        if ($predicate instanceof Where) {
            ...
            if (is_string($predicate)) {
                ...
                $predicate = (strpos($predicate, Expression::PLACEHOLDER) !==
                         false) ? new Predicate\Expression($predicate) : new Predicate\Literal(
                                $predicate);
                $this->where->addPredicate($predicate, $combination);
                ...
                foreach ($predicate as $pkey => $pvalue) {
                    ...
                    if (is_string($pkey) && strpos($pkey, '?') !== false) {
                        // First, process strings that the abstraction
                        // replacement character ?
                        // as an Expression predicate
                        $predicate = new Predicate\Expression($pkey, $pvalue);
                    } elseif (is_string($pkey)) {
                        // Otherwise, if still a string, do something
                        // intelligent with the PHP type provided
                        ...
                        if ($pvalue === null) {
                            ...
                            // otherwise assume that array('foo' => 'bar') means
                            // "foo" = 'bar'
                        ...
                        // must be an array of expressions (with int-indexed
                        // array)
                        $predicate = (strpos($pvalue, Expression::PLACEHOLDER) !==
                                 false) ? new Predicate\Expression($pvalue) : new Predicate\Literal(
                                        $pvalue);
                    }
                    $this->where->addPredicate($predicate, $combination);
                }
    ...
    public function limit ($limit)
    {
        if (! is_numeric($limit)) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s expects parameter to be numeric, "%s" given', 
                            __METHOD__, 
                            (is_object($limit) ? get_class($limit) : gettype(
                                    $limit))));
        }
    ...
    public function offset ($offset)
    {
        if (! is_numeric($offset)) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s expects parameter to be numeric, "%s" given', 
                            __METHOD__, 
                            (is_object($offset) ? get_class($offset) : gettype(
                                    $offset))));
        }
    ...
    public function setSpecification ($index, $specification)
    {
        if (! method_exists($this, 'process' . $index)) {
            throw new Exception\InvalidArgumentException(
                    'Not a valid specification name.');
        }
            ...
            StatementContainerInterface $statementContainer)
    {
        ...
        foreach ($this->specifications as $name => $specification) {
            $parameters[$name] = $this->{'process' . $name}($platform, $driver, 
                    $parameterContainer, $sqls, $parameters);
    ...
    public function getSqlString (PlatformInterface $adapterPlatform = null)
    {
        ...
        foreach ($this->specifications as $name => $specification) {
            $parameters[$name] = $this->{'process' . $name}($adapterPlatform, 
                    null, null, $sqls, $parameters);
            if ($specification && is_array($parameters[$name])) {
                $sqls[$name] = $this->createSqlFromSpecificationAndParameters(
                        $specification, $parameters[$name]);
            }
            ...
            ParameterContainer $parameterContainer = null)
    {
        ...
        if ($schema) {
            $table = $platform->quoteIdentifier($schema) .
                     $platform->getIdentifierSeparator() . $table;
        }
        ...
        foreach ($this->columns as $columnIndexOrAs => $column) {
            ...
            if ($column instanceof Expression) {
                $columnParts = $this->processExpression($column, $platform, 
                        $driver, 
                        $this->processInfo['paramPrefix'] .
                                 ((is_string($columnIndexOrAs)) ? $columnIndexOrAs : 'column'));
                if ($parameterContainer) {
                    $parameterContainer->merge(
                            $columnParts->getParameterContainer());
                }
            ...
            if (is_string($columnIndexOrAs)) {
                ...
                $columnAs = (is_string($column)) ? $platform->quoteIdentifier(
        ...
        foreach ($this->joins as $join) {
            foreach ($join['columns'] as $jKey => $jColumn) {
                ...
                if ($jColumn instanceof ExpressionInterface) {
                    ...
                    if ($parameterContainer) {
                        $parameterContainer->merge(
                                $jColumnParts->getParameterContainer());
                    }
                    $jColumns[] = $jColumnParts->getSql();
                } else {
                    $name = (is_array($join['name'])) ? key($join['name']) : $name = $join['name'];
                    if ($name instanceof TableIdentifier) {
                        $name = $platform->quoteIdentifier($name->getSchema()) .
                                 $separator .
                                 $platform->quoteIdentifier($name->getTable());
                    } else {
                        $name = $platform->quoteIdentifier($name);
                    }
                    $jColumns[] = $name . $separator .
                             $platform->quoteIdentifierInFragment($jColumn);
                }
                if (is_string($jKey)) {
        ...
        if ($this->quantifier) {
            if ($this->quantifier instanceof Expression) {
                $quantifierParts = $this->processExpression($this->quantifier, 
                        $platform, $driver, 'quantifier');
                if ($parameterContainer) {
                    $parameterContainer->merge(
                            $quantifierParts->getParameterContainer());
                }
            ...
            ParameterContainer $parameterContainer = null)
    {
        ...
        foreach ($this->joins as $j => $join) {
            ...
            if ($joinName instanceof TableIdentifier) {
                ...
                $joinName = $platform->quoteIdentifier($joinName[1]) .
                         $platform->getIdentifierSeparator() .
                         $platform->quoteIdentifier($joinName[0]);
            } else {
                if ($joinName instanceof Select) {
                    $joinName = '(' . $joinName->processSubSelect($joinName, 
                            $platform, $driver, $parameterContainer) . ')';
                } else {
                    $joinName = $platform->quoteIdentifier($joinName);
                }
            }
            $joinSpecArgArray[$j][] = (isset($joinAs)) ? $joinName . ' AS ' .
                     $joinAs : $joinName;
            
            // on expression
            // note: for Expression objects, pass them to processExpression with
            // a prefix specific to each join (used for named parameters)
            $joinSpecArgArray[$j][] = ($join['on'] instanceof ExpressionInterface) ? $this->processExpression(
                    $join['on'], $platform, $driver, 
                    $this->processInfo['paramPrefix'] . 'join' . ($j + 1) .
                             'part') : $platform->quoteIdentifierInFragment(
                    $join['on'], 
                    array(
                            '=',
                            'AND',
            ...
            if ($joinSpecArgArray[$j][2] instanceof StatementContainerInterface) {
                if ($parameterContainer) {
                    $parameterContainer->merge(
                            $joinSpecArgArray[$j][2]->getParameterContainer());
                }
            ...
            ParameterContainer $parameterContainer = null)
    {
        ...
        $whereParts = $this->processExpression($this->where, $platform, $driver, 
                $this->processInfo['paramPrefix'] . 'where');
        if ($parameterContainer) {
            ...
            ParameterContainer $parameterContainer = null)
    {
        ...
        foreach ($this->group as $column) {
            ...
            if ($column instanceof Expression) {
                $columnParts = $this->processExpression($column, $platform, 
                        $driver, $this->processInfo['paramPrefix'] . 'group');
                if ($parameterContainer) {
                    $parameterContainer->merge(
                            $columnParts->getParameterContainer());
                }
            ...
            ParameterContainer $parameterContainer = null)
    {
        ...
        $whereParts = $this->processExpression($this->having, $platform, 
                $driver, $this->processInfo['paramPrefix'] . 'having');
        if ($parameterContainer) {
            $parameterContainer->merge($whereParts->getParameterContainer());
        }
        return array(
            ...
            ParameterContainer $parameterContainer = null)
    {
        ...
        foreach ($this->order as $k => $v) {
            if ($v instanceof Expression) {
                ...
                if ($parameterContainer) {
                    $parameterContainer->merge(
                            $orderParts->getParameterContainer());
                }
                $orders[] = array(
                        $orderParts->getSql()
                );
            ...
            ParameterContainer $parameterContainer = null)
    {
        ...
        if ($driver) {
            $sql = $this->processSubSelect($this->combine['select'], $platform, 
