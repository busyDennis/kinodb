...
class Adapter implements AdapterInterface, Profiler\ProfilerAwareInterface
{
    ...
    public function getDriver ()
    {
        if ($this->driver == null) {
            throw new Exception\RuntimeException(
                    'Driver has not been set or configured for this adapter.');
        }
        return $this->driver;
    }
    ...
    {
        if (is_string($parametersOrQueryMode) && in_array(
        ...
        if ($mode == self::QUERY_MODE_PREPARE) {
            ...
            if (is_array($parameters) ||
                ...
                $this->lastPreparedStatement->setParameterContainer(
                        (is_array($parameters)) ? new ParameterContainer(
    ...
    public function createStatement ($initialSql = null, $initialParameters = null)
    {
        ...
        if ($initialParameters == null || ! $initialParameters instanceof ParameterContainer &&
                 is_array($initialParameters)) {
            $initialParameters = new ParameterContainer(
                    (is_array($initialParameters) ? $initialParameters : array()));
        }
    ...
    public function __get ($name)
    {
        switch (strtolower($name)) {
            ...
            default:
                throw new Exception\InvalidArgumentException(
                        'Invalid magic property on adapter');
        }
    }
    ...
    protected function createDriver ($parameters)
    {
        ...
        switch ($driverName) {
            case 'mysqli':
                $driver = new Driver\Mysqli\Mysqli($parameters, null, null, 
                        $options);
                break;
            case 'sqlsrv':
        ...
        if (! isset($driver) || ! $driver instanceof Driver\DriverInterface) {
            throw new Exception\InvalidArgumentException(
                    'DriverInterface expected', null, null);
        }
    ...
    protected function createPlatform ($parameters)
    {
        if (isset($parameters['platform'])) {
            ...
            $platformName = $this->driver->getDatabasePlatformName(
                    Driver\DriverInterface::NAME_FORMAT_CAMELCASE);
        } else {
            throw new Exception\InvalidArgumentException(
                    'A platform could not be determined from the provided configuration');
        }
        
        // currently only supported by the IbmDb2 & Oracle concrete
        // implementations
        ...
        switch ($platformName) {
            case 'Mysql':
                ...
                $driver = ($this->driver instanceof Driver\Mysqli\Mysqli ||
                         $this->driver instanceof Driver\Pdo\Pdo) ? $this->driver : null;
                return new Platform\Mysql($driver);
            case 'SqlServer':
                // PDO is only supported driver for quoting values in this
                // platform
                return new Platform\SqlServer(
                        ($this->driver instanceof Driver\Pdo\Pdo) ? $this->driver : null);
            case 'Oracle':
                // oracle does not accept a driver as an option, no driver
                // specific quoting available
            ...
            case 'Postgresql':
                ...
                $driver = ($this->driver instanceof Driver\Pgsql\Pgsql ||
                         $this->driver instanceof Driver\Pdo\Pdo) ? $this->driver : null;
