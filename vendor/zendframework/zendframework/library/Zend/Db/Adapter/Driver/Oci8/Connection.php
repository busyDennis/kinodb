...
class Connection implements ConnectionInterface, Profiler\ProfilerAwareInterface
{
    ...
    public function __construct ($connectionInfo = null)
    {
        if (is_array($connectionInfo)) {
            ...
            throw new Exception\InvalidArgumentException(
                    '$connection must be an array of parameters, a oci8 resource or null');
        }
    }
    ...
    public function setResource ($resource)
    {
        if (! is_resource($resource) ||
            ...
            throw new Exception\InvalidArgumentException(
                    'A resource of type "oci8 connection" was expected');
        }
    ...
    public function connect ()
    {
        ...
        $username = $findParameterValue(
                array(
                        'username'
                ));
        $password = $findParameterValue(
                array(
                        'password'
                ));
        ...
        $characterSet = $findParameterValue(
                array(
                        'character_set',
                        'charset',
                        'encoding'
        ...
        $isUnique = $findParameterValue(
                array(
                        'unique'
                ));
        $isPersistent = $findParameterValue(
                array(
                        'persistent'
                ));
        
        if ($isUnique == true) {
            $this->resource = oci_new_connect($username, $password, 
                    $connectionString, $characterSet);
        } elseif ($isPersistent == true) {
            $this->resource = oci_pconnect($username, $password, 
                    $connectionString, $characterSet);
        } else {
            $this->resource = oci_connect($username, $password, 
                    $connectionString, $characterSet);
        }
        
        if (! $this->resource) {
            $e = oci_error();
            throw new Exception\RuntimeException('Connection error', null, 
    ...
    public function beginTransaction ()
    {
        ...
        // is executed with oci_execute() using the OCI_NO_AUTO_COMMIT flag.
    ...
    public function commit ()
    {
        ...
        if ($this->inTransaction) {
            ...
            if ($valid === false) {
                ...
                throw new Exception\InvalidQueryException($e['message'], 
                        $e['code']);
            }
        }
    }
    ...
    public function rollback ()
    {
        ...
        if (! $this->inTransaction) {
            throw new Exception\RuntimeException(
                    'Must call commit() before you can rollback.');
        }
