...
class Connection implements ConnectionInterface, Profiler\ProfilerAwareInterface
{
    ...
    public function __construct ($connectionInfo)
    {
        if (is_array($connectionInfo)) {
            ...
            throw new Exception\InvalidArgumentException(
                    '$connection must be an array of parameters or a resource');
        }
    }
    ...
    public function setResource ($resource)
    {
        if (get_resource_type($resource) !== 'SQL Server Connection') {
            throw new Exception\InvalidArgumentException(
                    'Resource provided was not of type SQL Server Connection');
        }
    ...
    public function connect ()
    {
        ...
        if (! $this->resource) {
            throw new Exception\RuntimeException('Connect Error', null, 
    ...
    public function execute ($sql)
    {
        ...
        if (! $this->driver instanceof Sqlsrv) {
            throw new Exception\RuntimeException(
                    'Connection is missing an instance of Sqlsrv');
        }
        ...
        // if the returnValue is something other than a Sqlsrv_result, bypass
        // wrapping it
        if ($returnValue === false) {
            ...
            if ($errors[0]['SQLSTATE'] != '01000') {
                throw new Exception\RuntimeException(
                        ...
                        null, new ErrorException($errors));
            }
        }
