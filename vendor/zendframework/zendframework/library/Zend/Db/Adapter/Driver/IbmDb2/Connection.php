...
class Connection implements ConnectionInterface, Profiler\ProfilerAwareInterface
{
    ...
    public function setResource ($resource)
    {
        if (! is_resource($resource) ||
                 get_resource_type($resource) !== 'DB2 Connection') {
            throw new Exception\InvalidArgumentException(
                    'The resource provided must be of type "DB2 Connection"');
        }
    ...
    public function connect ()
    {
        ...
        $connection = array();
        $connection['database'] = $findParameterValue(
                array(
                        'database',
                        'db'
                ));
        $connection['username'] = $findParameterValue(
                array(
                        'username',
                        'uid',
                        'UID'
        ...
        $this->resource = db2_connect($connection['database'], 
