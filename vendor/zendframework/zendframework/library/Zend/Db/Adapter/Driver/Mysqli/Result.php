...
class Result implements Iterator, ResultInterface
{
    ...
    public function initialize ($resource, $generatedValue, $isBuffered = null)
    {
        ...
        if ($isBuffered !== null) {
            ...
            if ($resource instanceof \mysqli ||
                     $resource instanceof \mysqli_result || $resource instanceof \mysqli_stmt &&
                     $resource->num_rows != 0) {
        ...
        if ($this->resource instanceof \mysqli_stmt && $this->isBuffered !== true) {
            if ($this->position > 0) {
                throw new Exception\RuntimeException(
                        'Cannot buffer a result set that has started iteration.');
            }
    ...
    public function getAffectedRows ()
    {
        if ($this->resource instanceof \mysqli ||
                 $this->resource instanceof \mysqli_stmt) {
    ...
    {
        $data = null;
        // build the default reference based bind structure, if it does not
        // already exist
        if ($this->statementBindValues['keys'] === null) {
            ...
            $this->statementBindValues['values'] = array_fill(0, 
                    count($this->statementBindValues['keys']), null);
            $refs = array();
        ...
        // dereference
        for ($i = 0, $count = count($this->statementBindValues['keys']); $i <
                 $count; $i ++) {
    ...
    public function rewind ()
    {
        if ($this->position !== 0) {
            if ($this->isBuffered === false) {
                throw new Exception\RuntimeException(
                        'Unbuffered results cannot be rewound for multiple iterations');
            }
        }
        $this->resource->data_seek(0); // works for both mysqli_result &
                                       // mysqli_stmt
        $this->currentComplete = false;
        $this->position = 0;
    }
