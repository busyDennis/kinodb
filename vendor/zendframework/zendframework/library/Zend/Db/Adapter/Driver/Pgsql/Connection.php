...
class Connection implements ConnectionInterface, Profiler\ProfilerAwareInterface
{
    ...
    public function connect ()
    {
        ...
        $connection = array();
        $connection['host'] = $findParameterValue(
                array(
                        'hostname',
                        'host'
                ));
        $connection['user'] = $findParameterValue(
                array(
                        'username',
                        'user'
                ));
        $connection['password'] = $findParameterValue(
                array(
                        'password',
                        'passwd',
                        'pw'
                ));
        $connection['dbname'] = $findParameterValue(
                array(
                        'database',
                        'dbname',
                        'db',
        ...
        $connection = http_build_query($connection, null, ' '); // @link
        ...
        set_error_handler(
                function  ($number, $string)
                {
                    throw new Exception\RuntimeException(
                            __METHOD__ . ': Unable to connect to database', null, 
        ...
        if ($this->resource === false) {
            throw new Exception\RuntimeException(
                    sprintf('%s: Unable to connect to database', __METHOD__));
        }
    ...
    public function execute ($sql)
    {
        ...
        // if the returnValue is something other than a pg result resource,
        // bypass wrapping it
        ...
        $resultPrototype = $this->driver->createResult(
                ($resultResource === true) ? $this->resource : $resultResource);
    ...
    public function getLastGeneratedValue ($name = null)
    {
        ...
        $result = pg_query($this->resource, 
                'SELECT CURRVAL(\'' . str_replace('\'', '\\\'', $name) .
                         '\') as "currval"');
