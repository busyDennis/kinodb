...
class IbmDb2 implements PlatformInterface
{
    ...
    {
        if (isset($options['quote_identifiers']) && ($options['quote_identifiers'] ==
                 false || $options['quote_identifiers'] === 'false')) {
    ...
    {
        if ($this->quoteIdentifiers === false) {
            return (is_array($identifierChain)) ? implode(
                    $this->identifierSeparator, $identifierChain) : $identifierChain;
        }
        $identifierChain = str_replace('"', '\\"', $identifierChain);
        if (is_array($identifierChain)) {
            $identifierChain = implode('"' . $this->identifierSeparator . '"', 
                    $identifierChain);
        }
