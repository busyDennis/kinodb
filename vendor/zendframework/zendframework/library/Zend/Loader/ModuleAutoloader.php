...
class ModuleAutoloader implements SplAutoloader
{
    ...
    public function autoload ($class)
    {
        ...
        if (isset($this->explicitPaths[$moduleName])) {
            $classLoaded = $this->loadModuleFromDir(
                    $this->explicitPaths[$moduleName], $class);
            if ($classLoaded) {
                return $classLoaded;
            }
            
            $classLoaded = $this->loadModuleFromPhar(
                    $this->explicitPaths[$moduleName], $class);
            if ($classLoaded) {
                return $classLoaded;
            }
        ...
        if (count($this->namespacedPaths) >= 1) {
            foreach ($this->namespacedPaths as $namespace => $path) {
                ...
                $moduleNameBuffer = str_replace($namespace . "\\", "", 
                        $moduleName);
                $path .= DIRECTORY_SEPARATOR . $moduleNameBuffer .
                         DIRECTORY_SEPARATOR;
        ...
        if ($this->pharExtensions) {
            $pharSuffixPattern = '(' .
                     implode('|', 
                            array_map('preg_quote', $this->pharExtensions)) . ')';
        }
        ...
        foreach ($this->paths as $path) {
            ...
            if ($pharSuffixPattern) {
                foreach (new GlobIterator($path . '.*') as $entry) {
                    ...
                    if (! preg_match('#.+\.' . $pharSuffixPattern . '$#', 
                            $entry->getPathname())) {
                        continue;
                    }
                    
                    $classLoaded = $this->loadModuleFromPhar(
                            $entry->getPathname(), $class);
                    if ($classLoaded) {
                        return $classLoaded;
                    }
                }
    ...
    protected function loadModuleFromPhar ($pharPath, $class)
    {
        ...
        // Phase 1: Not executable phar, no stub, or stub did not provide Module
        // class; try Module.php directly
        ...
        $moduleClassFile = 'phar://' . $fileRealPath . '/' . $pharBaseName .
                 '/Module.php';
        $moduleFile = new SplFileInfo($moduleClassFile);
        if ($moduleFile->isReadable() && $moduleFile->isFile()) {
            require_once $moduleClassFile;
    ...
    public function registerPath ($path, $moduleName = false)
    {
        ...
        if ($moduleName) {
            if (in_array(substr($moduleName, - 2), 
                ...
                $this->namespacedPaths[substr($moduleName, 0, - 2)] = static::normalizePath(
                        $path);
            } else {
