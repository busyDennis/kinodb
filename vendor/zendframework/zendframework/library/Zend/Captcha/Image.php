...
class Image extends AbstractWord
{
     ...
     * @throws Exception\ImageNotLoadableException if start image cannot be
     *         loaded
     */
        ...
        if (empty($this->startImage)) {
            ...
            if (! $img || $error) {
                throw new Exception\ImageNotLoadableException(
                        "Can not load start image '{$this->startImage}'", 0, 
                        $error);
            }
            $w = imagesx($img);
            $h = imagesy($img);
        ...
        for ($x = 0; $x < $w; $x ++) {
            for ($y = 0; $y < $h; $y ++) {
                ...
                if ($sx < 0 || $sy < 0 || $sx >= $w - 1 || $sy >= $h - 1) {
                    ...
                    $colorXY = (imagecolorat($img, $sx + 1, $sy + 1) >> 16) &
                             0xFF;
                }
                ...
                if ($color == 255 && $colorX == 255 && $colorY == 255 &&
                    ...
                    // ignore background
                    continue;
                } elseif ($color == 0 && $colorX == 0 && $colorY == 0 &&
                         $colorXY == 0) {
