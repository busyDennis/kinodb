...
class Wddx extends AbstractAdapter
{
    ...
    public function serialize ($value)
    {
        ...
        if ($wddx === false) {
            throw new Exception\RuntimeException('Serialization failed', 0, 
                    $error);
        }
    ...
    public function unserialize ($wddx)
    {
        ...
        if ($ret === null && class_exists('SimpleXMLElement', false)) {
            ...
            try {
                $oldLibxmlDisableEntityLoader = libxml_disable_entity_loader(
                        true);
                $dom = new \DOMDocument();
                $dom->loadXML($wddx);
                ...
                throw new Exception\RuntimeException(
                        'Unserialization failed: Invalid wddx packet');
            } catch (\Exception $e) {
                throw new Exception\RuntimeException(
