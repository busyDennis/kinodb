...
class PythonPickle extends AbstractAdapter
{
     ...
     *       The listing here is in kind-of alphabetical order of 1-character
     *       pickle code.
    ...
    const OP_POP_MARK = '1';
    // discard stack top through topmost
    // markobject
    const OP_DUP = '2';
    // duplicate top stack item
    const OP_FLOAT = 'F';
    // push float object; decimal string
    // argument
    const OP_INT = 'I';
    // push integer or bool; decimal string
    // argument
    const OP_BININT = 'J';
    ...
    // push None
    const OP_PERSID = 'P';
    // push persistent object; id is taken from
    // string arg
    ...
    const OP_STRING = 'S';
    // push string; NL-terminated string
    // argument
    const OP_BINSTRING = 'T';
    // push string; counted binary string
    // argument
    ...
    const OP_GLOBAL = 'c';
    // push self.find_class(modname, name); 2
    // string args
    const OP_DICT = 'd';
    ...
    // push empty dict
    const OP_APPENDS = 'e';
    // extend list on stack by topmost stack
    // slice
    const OP_GET = 'g';
    // push item from memo on stack; index is
    // string arg
    const OP_BINGET = 'h';
    // " " " " " " ; " " 1-byte arg
    const OP_INST = 'i';
    ...
    // build & push class instance
    const OP_PUT = 'p';
    // store stack top in memo; index is string
    // arg
    const OP_BINPUT = 'q';
    // " " " " " ; " " 1-byte arg
    ...
    // push empty tuple
    const OP_SETITEMS = 'u';
    // modify dict by adding topmost key+value
    // pairs
    const OP_BINFLOAT = 'G';
    ...
    const OP_NEWOBJ = "\x81";
    // build object by applying cls.__new__
    // to argtuple
    const OP_EXT1 = "\x82";
    // push object from extension registry;
    // 1-byte index
    const OP_EXT2 = "\x83";
    ...
    const OP_TUPLE2 = "\x86";
    // build 2-tuple from two topmost stack
    // items
    const OP_TUPLE3 = "\x87";
    // build 3-tuple from three topmost stack
    // items
    ...
    const OP_BINBYTES = 'B';
    // push bytes; counted binary string
    // argument
    ...
    protected function write ($value)
    {
        if ($value === null) {
            ...
            throw new Exception\RuntimeException(
                    sprintf('PHP-Type "%s" can not be serialized by %s', 
                            gettype($value), get_class($this)));
        }
    }
    ...
    protected function hex2Utf8 ($hex)
    {
        ...
        if ($uniCode < 0x80) { // 1Byte
            ...
            throw new Exception\RuntimeException(
                    sprintf('Unsupported unicode character found "%s"', 
                            dechex($uniCode)));
        }
     ...
     * Load a proto value
     *
     * @throws Exception\RuntimeException if Pickle version does not support
     *         this feature
     */
