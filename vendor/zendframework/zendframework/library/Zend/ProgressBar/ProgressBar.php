...
class ProgressBar
{
     ...
     */
    public function __construct (Adapter\AbstractAdapter $adapter, $min = 0, 
            $max = 100, $persistenceNamespace = null)
    {
        ...
        if ($persistenceNamespace !== null) {
            $this->persistenceNamespace = new Session\Container(
    ...
    public function update ($value = null, $text = null)
    {
        ...
        if ($this->min === $this->max) {
            ...
            $percent = (float) ($this->current - $this->min) /
                     ($this->max - $this->min);
        }
        ...
        // Poll the adapter
        $this->adapter->notify($this->current, $this->max, $percent, $timeTaken, 
                $timeRemaining, $this->statusText);
    }
    ...
    {
        $this->update(max($this->min, min($this->max, $this->current + $diff)), 
                $text);
    }
