...
class Console extends AbstractAdapter
{
    ...
    public function setOutputStream ($resource)
    {
        ...
        if ($stream === false) {
            throw new Exception\RuntimeException('Unable to open stream', 0, 
                    $error);
        }
    ...
    public function setWidth ($width = null)
    {
        if ($width === null || ! is_int($width)) {
            if (substr(PHP_OS, 0, 3) === 'WIN') {
                ...
                } elseif (preg_match('#columns = (\d+);#', shell_exec('stty'), 
                        $match) === 1) {
                    $this->width = (int) $match[1];
                }
                ErrorHandler::stop();
        ...
        foreach ($this->elements as $element) {
            switch ($element) {
                ...
                case self::ELEMENT_TEXT:
                    $renderedElements[] = StringUtils::getWrapper(
                            $this->charset)->strPad(
                            substr($text, 0, $this->textWidth), $this->textWidth, 
                            ' ', STR_PAD_RIGHT);
                    break;
            }
        }
