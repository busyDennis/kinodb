...
final class Version
{
     ...
     *
     * If $service is set to VERSION_SERVICE_ZEND this will fall back to calling
     * the
     ...
     * @link
     *       https://api.github.com/repos/zendframework/zf2/git/refs/tags/release-
        ...
        if (null === static::$latestVersion) {
            ...
            if ($service == self::VERSION_SERVICE_GITHUB) {
                ...
                $tags = array_map(
                        function  ($tag)
                        {
                            return substr($tag['ref'], 18); // Reliable because
                                                                // we're filtering
                                                                // on
                                                                // 'refs/tags/release-'
                        }, $apiResponse);
                ...
                $handle = fopen('http://framework.zend.com/api/zf-version?v=2', 
                        'r');
                if (false !== $handle) {
