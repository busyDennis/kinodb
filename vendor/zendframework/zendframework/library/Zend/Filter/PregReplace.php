...
class PregReplace extends AbstractFilter
{
    ...
    public function __construct ($options = null)
    {
        ...
        if (! is_array($options) || (! isset($options['pattern']) && ! isset(
                $options['replacement']))) {
            $args = func_get_args();
            if (isset($args[0])) {
        ...
        if (! is_array($pattern) && ! is_string($pattern)) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, 
                            (is_object($pattern) ? get_class($pattern) : gettype(
                                    $pattern))));
        }
        ...
        if (! is_array($replacement) && ! is_string($replacement)) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, 
                            (is_object($replacement) ? get_class($replacement) : gettype(
                                    $replacement))));
        }
    ...
    public function filter ($value)
    {
        if ($this->options['pattern'] === null) {
            throw new Exception\RuntimeException(
                    sprintf('Filter %s does not have a valid pattern set', 
                            get_class($this)));
        }
        
        return preg_replace($this->options['pattern'], 
                $this->options['replacement'], $value);
    }
