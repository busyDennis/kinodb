...
class RenameUpload extends AbstractFilter
{
    ...
    protected function moveUploadedFile ($sourceFile, $targetFile)
    {
        ...
        if (! $result || null !== $warningException) {
            throw new Exception\RuntimeException(
                    sprintf(
                            "File '%s' could not be renamed. An error occurred while processing the file.", 
                            $sourceFile), 0, $warningException);
        }
    ...
    protected function getFinalTarget ($uploadData)
    {
        ...
        if ($this->getUseUploadName()) {
            ...
            if ($this->getUseUploadExtension() && ! $this->getRandomize()) {
                ...
                if (isset($sourceinfo['extension'])) {
                    $targetFile = $targetInfo['filename'] . '.' .
        ...
        if ($this->getRandomize()) {
            $targetFile = $this->applyRandomToFilename($uploadData['name'], 
                    $targetFile);
        }
    ...
    protected function applyRandomToFilename ($source, $filename)
    {
        ...
        $extension = '';
        if ($this->getUseUploadExtension() === true &&
                 isset($sourceinfo['extension'])) {
