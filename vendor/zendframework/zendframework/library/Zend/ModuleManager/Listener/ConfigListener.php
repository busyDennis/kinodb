 ...
 * Config listener
 */
class ConfigListener extends AbstractListener implements ConfigMergerInterface, 
        ListenerAggregateInterface
{
    ...
    public function __construct (ListenerOptions $options = null)
    {
        ...
        if ($this->hasCachedConfig()) {
            ...
            $this->addConfigGlobPaths(
                    $this->getOptions()
                        ->getConfigGlobPaths());
            $this->addConfigStaticPaths(
                    $this->getOptions()
    ...
    public function attach (EventManagerInterface $events)
    {
        ...
        if ($this->skipConfig) {
            // We already have the config from cache, no need to collect or
            // merge.
            return $this;
        }
        
        $this->callbacks[] = $events->attach(ModuleEvent::EVENT_LOAD_MODULE, 
                array(
                        $this,
                        'onLoadModule'
                ));
    ...
    public function onLoadModule (ModuleEvent $e)
    {
        ...
        if (! $module instanceof ConfigProviderInterface && ! is_callable(
                array(
                        $module,
                        'getConfig'
                ))) {
    ...
    /**
     * Add an array of static paths of config files to merge after loading
     * modules
     *
    ...
    protected function addConfigByPath ($path, $type)
    {
        switch ($type) {
            ...
            case self::GLOB_PATH:
                // We want to keep track of where each value came from so we
                // don't
                // use ConfigFactory::fromFiles() since it does merging
                // internally.
