...
class ModuleManager implements ModuleManagerInterface
{
            ...
            if (is_object($module)) {
                if (! is_string($moduleName)) {
                    throw new Exception\RuntimeException(
                            sprintf('Module (%s) must have a key identifier.', 
                                    get_class($module)));
                }
                $module = array(
                        $moduleName => $module
    ...
    public function loadModules ()
    {
         ...
         */
        $this->getEventManager()->trigger(ModuleEvent::EVENT_LOAD_MODULES_POST, 
                $this, $this->getEvent());
    ...
    public function loadModule ($module)
    {
        ...
        $this->getEventManager()->trigger(ModuleEvent::EVENT_LOAD_MODULE, $this, 
                $event);
    ...
    protected function loadModuleByName ($event)
    {
        $result = $this->getEventManager()->trigger(
                ModuleEvent::EVENT_LOAD_MODULE_RESOLVE, $this, $event, 
                function  ($r)
                {
                    return (is_object($r));
                });
        ...
        if (! is_object($module)) {
            throw new Exception\RuntimeException(
                    sprintf('Module (%s) could not be initialized.', 
                            $event->getModuleName()));
        }
