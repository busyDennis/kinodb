...
class Escaper
{
    ...
    /**
     * Constructor: Single parameter allows setting of global encoding for use
     * by
     * the current object.
        ...
        if ($encoding !== null) {
            ...
            if ($encoding === '') {
                throw new Exception\InvalidArgumentException(
                        get_class($this) .
                                 ' constructor parameter does not allow a blank value');
            }
            
            $encoding = strtolower($encoding);
            if (! in_array($encoding, $this->supportedEncodings)) {
                throw new Exception\InvalidArgumentException(
                        'Value of \'' . $encoding . '\' passed to ' .
                                 get_class($this) .
    ...
    {
        $result = htmlspecialchars($string, $this->htmlSpecialCharsFlags, 
                $this->encoding);
        return $result;
    }
    ...
    public function escapeJs ($string)
    {
        ...
        $result = preg_replace_callback('/[^a-z0-9,\._]/iSu', $this->jsMatcher, 
                $string);
        return $this->fromUtf8($result);
    }
    ...
    public function escapeCss ($string)
    {
        ...
        $result = preg_replace_callback('/[^a-z0-9]/iSu', $this->cssMatcher, 
                $string);
        return $this->fromUtf8($result);
    }
    ...
    protected function toUtf8 ($string)
    {
        if ($this->getEncoding() === 'utf-8') {
            ...
            $result = $this->convertEncoding($string, 'UTF-8', 
                    $this->getEncoding());
        }
    ...
    /**
     * Encoding conversion helper which wraps iconv and mbstring where they
     * exist or throws
