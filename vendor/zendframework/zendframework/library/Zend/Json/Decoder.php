...
class Decoder
{
     ...
     *            {@link Zend\Json\Json::TYPE_ARRAY} and {@link
     *            Zend\Json\Json::TYPE_OBJECT} for
     *            valid values
        ...
        switch ($decodeType) {
            ...
            default:
                throw new InvalidArgumentException(
                        "Unknown decode type '{$decodeType}', please use one of the constants Json::TYPE_*");
        }
     ...
     * return a stdClass object instead, pass {@link
     * Zend\Json\Json::TYPE_OBJECT} to
        ...
        while ($tok && $tok != self::RBRACE) {
            if ($tok != self::DATUM || ! is_string($this->tokenValue)) {
                throw new RuntimeException(
                        'Missing key in object encoding: ' . $this->source);
            }
            ...
            if ($tok != self::COLON) {
                throw new RuntimeException(
                        'Missing ":" in object encoding: ' . $this->source);
            }
    ...
    protected function _decodeArray ()
    {
        ...
        while ($tok && $tok != self::RBRACKET) {
            ...
            if ($tok != self::COMMA) {
                throw new RuntimeException(
                        'Missing "," in array encoding: ' . $this->source);
            }
    ...
    protected function _getNextToken ()
    {
        ...
        switch ($str{$i}) {
            ...
            case '"':
                ...
                // $this->tokenValue = substr($str, $start + 1, $i - $start -
                // 1);
