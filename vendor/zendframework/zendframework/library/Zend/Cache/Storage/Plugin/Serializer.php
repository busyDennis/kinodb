...
class Serializer extends AbstractPlugin
{
    ...
    {
        // The higher the priority the sooner the plugin will be called on pre
        // events
        ...
        // read
        $this->listeners[] = $events->attach('getItem.post', 
                array(
                        $this,
                        'onReadItemPost'
                ), $postPriority);
        $this->listeners[] = $events->attach('getItems.post', 
                array(
                        $this,
                        'onReadItemsPost'
                ), $postPriority);
        ...
        // write
        $this->listeners[] = $events->attach('setItem.pre', 
                array(
                        $this,
                        'onWriteItemPre'
                ), $prePriority);
        $this->listeners[] = $events->attach('setItems.pre', 
                array(
                        $this,
                        'onWriteItemsPre'
                ), $prePriority);
        
        $this->listeners[] = $events->attach('addItem.pre', 
                array(
                        $this,
                        'onWriteItemPre'
                ), $prePriority);
        $this->listeners[] = $events->attach('addItems.pre', 
                array(
                        $this,
                        'onWriteItemsPre'
                ), $prePriority);
        
        $this->listeners[] = $events->attach('replaceItem.pre', 
                array(
                        $this,
                        'onWriteItemPre'
                ), $prePriority);
        $this->listeners[] = $events->attach('replaceItems.pre', 
                array(
                        $this,
                        'onWriteItemsPre'
                ), $prePriority);
        
        $this->listeners[] = $events->attach('checkAndSetItem.pre', 
                array(
                        $this,
                        'onWriteItemPre'
                ), $prePriority);
        ...
        $this->listeners[] = $events->attach('incrementItem.pre', 
                array(
                        $this,
                        'onIncrementItemPre'
                ), $prePriority);
        $this->listeners[] = $events->attach('incrementItems.pre', 
                array(
                        $this,
                        'onIncrementItemsPre'
                ), $prePriority);
        
        $this->listeners[] = $events->attach('decrementItem.pre', 
                array(
                        $this,
                        'onDecrementItemPre'
                ), $prePriority);
        $this->listeners[] = $events->attach('decrementItems.pre', 
                array(
                        $this,
                        'onDecrementItemsPre'
                ), $prePriority);
        ...
        $this->listeners[] = $events->attach('getCapabilities.post', 
                array(
                        $this,
    ...
    public function onIncrementItemPre (Event $event)
    {
        ...
        if ($success) {
            $storage->checkAndSetItem($casToken, $params['key'], 
    ...
    public function onIncrementItemsPre (Event $event)
    {
        $storage = $event->getTarget();
        $params = $event->getParams();
        $keyValuePairs = $storage->getItems(
                array_keys($params['keyValuePairs']));
    ...
    public function onGetCapabilitiesPost (PostEvent $event)
    {
        ...
        if (! isset($this->capabilities[$index])) {
            $this->capabilities[$index] = new Capabilities(
                    $baseCapabilities->getAdapter(), new stdClass(),  // marker
                    array(
