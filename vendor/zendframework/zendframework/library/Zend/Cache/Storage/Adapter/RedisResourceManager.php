...
class RedisResourceManager
{
    ...
    protected function connect (array & $resource)
    {
        ...
        if ($resource['persistent_id'] !== '') {
            ...
            $success = $redis->pconnect($server['host'], $server['port'], 
                    $server['timeout'], $server['persistend_id']);
        } elseif ($server['port']) {
            $success = $redis->connect($server['host'], $server['port'], 
                    $server['timeout']);
        ...
        if (! $success) {
            throw new Exception\RuntimeException(
                    'Could not estabilish connection with Redis instance');
        }
    ...
    protected function normalizeLibOptionKey (& $key)
    {
        ...
        if (is_string($key)) {
            ...
            if (! defined($const)) {
                throw new Exception\InvalidArgumentException(
                        "Unknown redis option '{$key}' ({$const})");
            }
            $key = constant($const);
        } else {
            $key = (int) $key;
    ...
    protected function normalizeServer (& $server)
    {
        ...
        if (is_array($server)) {
            ...
            // array('host' => <host>[, 'port' => <port>, ['timeout' =>
            // <timeout>]])
