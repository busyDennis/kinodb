...
class Dba extends AbstractAdapter implements AvailableSpaceCapableInterface, 
        ClearByNamespaceInterface, ClearByPrefixInterface, FlushableInterface, 
        IterableInterface, OptimizableInterface, TotalSpaceCapableInterface
{
        ...
        if ($this->totalSpace === null) {
            ...
            if ($total === false) {
                throw new Exception\RuntimeException(
                        "Can't detect total space of '{$pathname}'", 0, $error);
            }
    ...
    public function getAvailableSpace ()
    {
        ...
        if ($avail === false) {
            throw new Exception\RuntimeException(
                    "Can't detect free space of '{$pathname}'", 0, $error);
        }
    ...
    public function flush ()
    {
        ...
        if (file_exists($pathname)) {
            ...
            if (! $result) {
                throw new Exception\RuntimeException(
                        "unlink('{$pathname}') failed", 0, $error);
            }
        }
    ...
    public function clearByPrefix ($prefix)
    {
        ...
        $prefix = ($namespace === '') ? '' : $namespace .
                 $options->getNamespaceSeparator() . $prefix;
        $prefixL = strlen($prefix);
        $result = true;
    ...
    protected function internalSetItem (& $normalizedKey, & $value)
    {
        ...
        if (! dba_replace($internalKey, $value, $this->handle)) {
            throw new Exception\RuntimeException(
                    "dba_replace('{$internalKey}', ...) failed");
        }
