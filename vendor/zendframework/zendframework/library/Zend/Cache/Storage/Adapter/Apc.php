...
class Apc extends AbstractAdapter implements AvailableSpaceCapableInterface, 
        ClearByNamespaceInterface, ClearByPrefixInterface, FlushableInterface, 
        IterableInterface, TotalSpaceCapableInterface
{
    ...
    public function clearByNamespace ($namespace)
    {
        ...
        $pattern = '/^' . preg_quote($prefix, '/') . '/';
        return apc_delete(
                new BaseApcIterator('user', $pattern, 0, 1, APC_LIST_ACTIVE));
    }
    ...
    protected function internalGetMetadata (& $normalizedKey)
    {
        ...
        if (! apc_exists($internalKey)) {
            ...
            $it = new BaseApcIterator('user', $regexp, $format, 100, 
                    APC_LIST_ACTIVE);
            $metadata = $it->current();
        }
    ...
    protected function internalGetMetadatas (array & $normalizedKeys)
    {
        ...
        $format = APC_ITER_ALL ^ APC_ITER_VALUE ^ APC_ITER_TYPE ^
                 APC_ITER_REFCOUNT;
        $it = new BaseApcIterator('user', $pattern, $format, 100, 
                APC_LIST_ACTIVE);
        $result = array();
        $prefixL = strlen($prefix);
    ...
    protected function internalSetItems (array & $normalizedKeyValuePairs)
    {
        ...
        if ($namespace === '') {
            return array_keys(
                    apc_store($normalizedKeyValuePairs, null, 
                            $options->getTtl()));
        }
        ...
        $failedKeys = apc_store($internalKeyValuePairs, null, 
                $options->getTtl());
    ...
    protected function internalAddItems (array & $normalizedKeyValuePairs)
    {
        ...
        if ($namespace === '') {
            return array_keys(
                    apc_add($normalizedKeyValuePairs, null, $options->getTtl()));
        }
    ...
    protected function normalizeMetadata (array & $metadata)
    {
        ...
        unset($metadata['key'], $metadata['creation_time'], 
                $metadata['access_time'], $metadata['deletion_time'], 
                $metadata['mem_size'], $metadata['num_hits']);
    }
}
