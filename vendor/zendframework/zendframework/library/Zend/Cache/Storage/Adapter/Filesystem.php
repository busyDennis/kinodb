...
class Filesystem extends AbstractAdapter implements 
        AvailableSpaceCapableInterface, ClearByNamespaceInterface, 
        ClearByPrefixInterface, ClearExpiredInterface, FlushableInterface, 
        IterableInterface, OptimizableInterface, TaggableInterface, 
        TotalSpaceCapableInterface
{
    ...
    public function flush ()
    {
        ...
        if ($error) {
            throw new Exception\RuntimeException(
                    "Flushing directory '{$dir}' failed", 0, $error);
        }
    ...
    public function clearExpired ()
    {
        ...
        $path = $options->getCacheDir() .
                 str_repeat(DIRECTORY_SEPARATOR . $prefix . '*', 
                        $options->getDirLevel()) . DIRECTORY_SEPARATOR . $prefix .
                 '*.dat';
        ...
        $error = ErrorHandler::stop();
        if ($error) {
            throw new Exception\RuntimeException("Failed to clear expired items", 
                    0, $error);
        }
    ...
    public function clearByNamespace ($namespace)
    {
        ...
        if ($error) {
            throw new Exception\RuntimeException(
                    "Failed to remove files of '{$path}'", 0, $error);
        }
    ...
    public function clearByPrefix ($prefix)
    {
        ...
        $flags = GlobIterator::SKIP_DOTS | GlobIterator::CURRENT_AS_PATHNAME;
        $path = $options->getCacheDir() .
                 str_repeat(DIRECTORY_SEPARATOR . $nsPrefix . '*', 
                        $options->getDirLevel()) . DIRECTORY_SEPARATOR . $nsPrefix .
                 $prefix . '*';
        $glob = new GlobIterator($path, $flags);
    ...
    public function getTotalSpace ()
    {
        if ($this->totalSpace === null) {
            ...
            if ($total === false) {
                throw new Exception\RuntimeException(
                        "Can't detect total space of '{$path}'", 0, $error);
            }
            ...
            $callback = function  ($event) use( & $events, & $handle, 
            & $totalSpace)
            {
    ...
    public function getAvailableSpace ()
    {
        ...
        if ($avail === false) {
            throw new Exception\RuntimeException(
                    "Can't detect free space of '{$path}'", 0, $error);
        }
    ...
    protected function internalGetItems (array & $normalizedKeys)
    {
        ...
        while ($keys) {
            ...
            foreach ($keys as $i => $key) {
                ...
                $filespec = $this->getFileSpec($key);
                $data = $this->getFileContent($filespec . '.dat', $nonBlocking, 
                        $wouldblock);
     ...
     * @see setItem()
     */
    protected function internalCheckAndSetItem (& $token, & $normalizedKey, 
            & $value)
    {
        if (! $this->internalHasItem($normalizedKey)) {
            return false;
    ...
    protected function internalGetCapabilities ()
    {
        if ($this->capabilities === null) {
            ...
            $capabilities = new Capabilities($this, $marker, 
                    array(
                            ...
                            'ttlPrecision' => 1,
                            'expiredRead' => true,
                            'maxKeyLength' => 251, // 255 - strlen(.dat |
                                                   // .tag)
            ...
            $this->getEventManager()->attach('option', 
                    function  ($event) use( $capabilities, $marker)
                    {
                        $params = $event->getParams();
                        
                        if (isset($params['namespace_separator'])) {
                            $capabilities->setNamespaceSeparator($marker, 
                                    $params['namespace_separator']);
                        }
                        
                        if (isset($params['no_atime']) ||
                            ...
                            if (isset($params['no_atime']) &&
                                     ! $params['no_atime']) {
                                $metadata[] = 'atime';
                            } elseif (isset($params['no_atime']) && ($index = array_search(
                                    'atime', $metadata)) !== false) {
                                unset($metadata[$index]);
                            }
                            
                            if (isset($params['no_ctime']) &&
                                     ! $params['no_ctime']) {
                                $metadata[] = 'ctime';
                            } elseif (isset($params['no_ctime']) && ($index = array_search(
                                    'ctime', $metadata)) !== false) {
                                unset($metadata[$index]);
                            }
                            
                            $capabilities->setSupportedMetadata($marker, 
                                    $metadata);
                        }
                    });
    ...
    {
        $glob = glob($dir . DIRECTORY_SEPARATOR . $prefix . '*', 
        ...
        foreach ($glob as $subdir) {
            // skip removing current directory if removing of sub-directory
            // failed
     ...
     * @throws Exception\RuntimeException
     */
    protected function readInfoFile ($file, $nonBlocking = false, 
            & $wouldblock = null)
    {
        if (! file_exists($file)) {
            return false;
     ...
     * @throws Exception\RuntimeException
     */
    protected function getFileContent ($file, $nonBlocking = false, 
            & $wouldblock = null)
    {
        ...
        if ($locking) {
            ...
            if ($res === false) {
                $err = ErrorHandler::stop();
                throw new Exception\RuntimeException(
                        "Error getting file contents for file '{$file}'", 0, 
                        $err);
            }
        }
    ...
    protected function prepareDirectoryStructure ($file)
    {
        ...
        if ($perm === false || $level == 1) {
            ...
            // build-in mkdir function sets permission together with current
            // umask
     ...
     * @throws Exception\RuntimeException
     */
    protected function putFileContent ($file, $data, $nonBlocking = false, 
            & $wouldblock = null)
    {
        $options = $this->getOptions();
        ...
        // if locking and non blocking is enabled -> file_put_contents can't
        // used
        if ($locking && $nonBlocking) {
            ...
            if (! ftruncate($fp, strlen($data))) {
                ...
                $err = ErrorHandler::stop();
                throw new Exception\RuntimeException(
                        "Error truncating file '{$file}'", 0, $err);
            }
