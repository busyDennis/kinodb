...
class MemcachedResourceManager
{
    ...
    public function getResource ($id)
    {
        ...
        // merge and add servers (with persistence id servers could be added
        // already)
        $servers = array_udiff($resource['servers'], $memc->getServerList(), 
                array(
                        $this,
                        'compareServers'
                ));
    ...
    protected function normalizeLibOptionKey (& $key)
    {
        // convert option name into it's constant value
        if (is_string($key)) {
            $const = 'Memcached::OPT_' . str_replace(
                    array(
                            ' ',
                            '-'
                    ), '_', strtoupper($key));
            if (! defined($const)) {
                throw new Exception\InvalidArgumentException(
                        "Unknown libmemcached option '{$key}' ({$const})");
            }
            $key = constant($const);
        } else {
            $key = (int) $key;
    ...
    public function setServers ($id, $servers)
    {
        ...
        if ($resource instanceof MemcachedResource) {
            ...
            $servers = array_udiff($servers, $resource->getServerList(), 
                    array(
                            $this,
                            'compareServers'
                    ));
    ...
    public function addServers ($id, $servers)
    {
        ...
        if ($resource instanceof MemcachedResource) {
            ...
            // don't add servers twice
            $resource['servers'] = array_merge($resource['servers'], 
                    array_udiff($servers, $resource['servers'], 
                            array(
                                    $this,
                                    'compareServers'
                            )));
    ...
    protected function normalizeServer (& $server)
    {
        ...
        if (is_array($server)) {
            ...
            // array('host' => <host>[, 'port' => <port>[, 'weight' =>
            // <weight>]])
