        ...
        TotalSpaceCapableInterface
{
    ...
    public function __construct ($options = null)
    {
        ...
        $this->getEventManager()->attach('option', 
                function  ($event) use( & $initialized)
                {
                    $initialized = false;
                });
    }
    ...
    protected function internalGetItems (array & $normalizedKeys)
    {
        ...
        return array_filter(array_combine($normalizedKeys, $results), 
                function  ($value)
                {
                    return $value !== false;
                });
     ...
     *
     * @param
     *            string &$normalizedKey Key in Redis under which value will be
     *            saved
     * @param
    ...
    protected function internalSetItem (& $normalizedKey, & $value)
    {
        ...
        try {
            if ($ttl) {
                if ($this->resourceManager->getMayorVersion($this->resourceId) <
                         2) {
                    throw new Exception\UnsupportedMethodCallException(
                            "To use ttl you need version >= 2.0.0");
                }
                $success = $redis->setex(
                        $this->namespacePrefix . $normalizedKey, $ttl, $value);
            } else {
                $success = $redis->set($this->namespacePrefix . $normalizedKey, 
                        $value);
            }
        } catch (RedisResourceException $e) {
            throw new Exception\RuntimeException($redis->getLastError(), 
     ...
     *
     * @param
     *            array &$normalizedKeyValuePairs An array of normalized
     *            key/value pairs
     *            
    ...
    protected function internalAddItem (& $normalizedKey, & $value)
    {
        $redis = $this->getRedisResource();
        try {
            return $redis->setnx($this->namespacePrefix . $normalizedKey, 
                    $value);
        } catch (RedisResourceException $e) {
            throw new Exception\RuntimeException($redis->getLastError(), 
                    $e->getCode(), $e);
    ...
    protected function internalIncrementItem (& $normalizedKey, & $value)
    {
        $redis = $this->getRedisResource();
        try {
            return $redis->incrBy($this->namespacePrefix . $normalizedKey, 
                    $value);
        } catch (RedisResourceException $e) {
            throw new Exception\RuntimeException($redis->getLastError(), 
                    $e->getCode(), $e);
    ...
    protected function internalDecrementItem (& $normalizedKey, & $value)
    {
        $redis = $this->getRedisResource();
        try {
            return $redis->decrBy($this->namespacePrefix . $normalizedKey, 
                    $value);
        } catch (RedisResourceException $e) {
            throw new Exception\RuntimeException($redis->getLastError(), 
                    $e->getCode(), $e);
    ...
    protected function internalGetCapabilities ()
    {
        if ($this->capabilities === null) {
            ...
            $minTtl = $this->resourceManager->getMayorVersion($this->resourceId) <
                     2 ? 0 : 1;
