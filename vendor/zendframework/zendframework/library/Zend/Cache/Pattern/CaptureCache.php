...
class CaptureCache extends AbstractPattern
{
    ...
    public function start ($pageId = null)
    {
        ...
        $that = $this;
        ob_start(
                function  ($content) use( $that, $pageId)
                {
    ...
    public function set ($content, $pageId = null)
    {
        ...
        $this->createDirectoryStructure(
                $publicDir . DIRECTORY_SEPARATOR . $path);
        $this->putFileContent($publicDir . DIRECTORY_SEPARATOR . $file, 
                $content);
    }
    ...
    public function get ($pageId = null)
    {
        ...
        $file = $publicDir . DIRECTORY_SEPARATOR . $this->pageId2Path($pageId) .
    ...
    public function clearByGlob ($pattern = '**')
    {
        ...
        $it = new \GlobIterator($publicDir . '/' . $pattern, 
                \GlobIterator::CURRENT_AS_SELF | \GlobIterator::SKIP_DOTS |
                         \GlobIterator::UNIX_PATHS);
    ...
    protected function putFileContent ($file, $data)
    {
        ...
        if ($rs === false) {
            $err = ErrorHandler::stop();
            throw new Exception\RuntimeException("Error writing file '{$file}'", 
                    0, $err);
        }
