...
class CallbackCache extends AbstractPattern
{
        ...
        if ($success) {
            if (! array_key_exists(0, $result)) {
                throw new Exception\RuntimeException(
                        "Invalid cached data for key '{$key}'");
            }
    ...
    protected function generateCallbackKey ($callback, array $args)
    {
        ...
        if (isset($object)) {
            ...
            try {
                ...
                ErrorHandler::stop();
                throw new Exception\RuntimeException(
                        "Can't serialize callback: see previous exception", 0, 
                        $e);
            }
            ...
            if (! $serializedObject) {
                throw new Exception\RuntimeException(
                        sprintf('Cannot serialize callback%s', 
    ...
    protected function generateArgumentsKey (array $args)
    {
        ...
        try {
            ...
            ErrorHandler::stop();
            throw new Exception\RuntimeException(
                    "Can't serialize arguments: see previous exception", 0, $e);
        }
        $error = ErrorHandler::stop();
        
        if (! $serializedArgs) {
            throw new Exception\RuntimeException(
                    sprintf('Cannot serialize arguments%s', 
                            ($error ? ': ' . $error->getMessage() : '')), 0, 
                    $error);
        }
