...
class Logger implements LoggerInterface
{
                ...
                foreach ($options['writers'] as $writer) {
                    ...
                    if (! isset($writer['name'])) {
                        throw new Exception\InvalidArgumentException(
                                'Options must contain a name for the writer');
                    }
        ...
        if (is_string($writer)) {
            ...
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __NAMESPACE__, 
                            is_object($writer) ? get_class($writer) : gettype(
                                    $writer)));
        }
     ...
     * @throws Exception\InvalidArgumentException
     */
    public function addProcessor ($processor, $priority = 1, 
            array $options = null)
    {
        if (is_string($processor)) {
            ...
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            is_object($processor) ? get_class($processor) : gettype(
                                    $processor)));
        }
     ...
     *         string
     * @throws Exception\InvalidArgumentException if extra can't be iterated
     *         over
        ...
        if (! is_int($priority) || ($priority < 0) ||
            ...
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            count($this->priorities), var_export($priority, 1)));
        }
            ...
            $continueNativeHandler = false)
    {
        ...
        $previous = set_error_handler(
                ...
                $errorPriorityMap, $continueNativeHandler)
                {
    ...
    public static function registerExceptionHandler (Logger $logger)
    {
        ...
        if ($logger === null) {
            throw new Exception\InvalidArgumentException(
                    'Invalid Logger specified');
        }
        
        $errorPriorityMap = static::$errorPriorityMap;
        
        set_exception_handler(
                function  ($exception) use( $logger, $errorPriorityMap)
                {
                    $logMessages = array();
                    ...
                    do {
                        ...
                        if ($exception instanceof ErrorException &&
                                 isset(
                                        $errorPriorityMap[$exception->getSeverity()])) {
                    ...
                    foreach (array_reverse($logMessages) as $logMessage) {
                        $logger->log($logMessage['priority'], 
                                $logMessage['message'], $logMessage['extra']);
                    }
                });
