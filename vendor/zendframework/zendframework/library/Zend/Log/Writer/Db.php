...
class Db extends AbstractWriter
{
     ...
     */
    public function __construct ($db, $tableName = null, array $columnMap = null, 
            $separator = null)
    {
        ...
        if (! $db instanceof Adapter) {
            throw new Exception\InvalidArgumentException(
                    'You must pass a valid Zend\Db\Adapter\Adapter');
        }
    ...
    protected function doWrite (array $event)
    {
        ...
        $statement = $this->db->query(
                $this->prepareInsert($this->db, $this->tableName, $dataToInsert));
    ...
    protected function prepareInsert (Adapter $db, $tableName, array $fields)
    {
        ...
        $sql = 'INSERT INTO ' . $db->platform->quoteIdentifier($tableName) . ' (' . implode(
                ",", 
                array_map(
                        array(
                                $db->platform,
