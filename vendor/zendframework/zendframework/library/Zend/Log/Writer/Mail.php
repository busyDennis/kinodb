...
class Mail extends AbstractWriter
{
        ...
        if (! $transport instanceof Transport\TransportInterface) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            (is_object($transport) ? get_class($transport) : gettype(
                                    $transport))));
        }
     ...
     * set
     * once, this method cannot be used if the Zend\Mail\Message object already
     * has a
     * subject set.
     *
    ...
    public function shutdown ()
    {
        ...
        if ($this->subjectPrependText !== null) {
            ...
            $this->mail->setSubject(
                    "{$this->subjectPrependText} ({$numEntries})");
        }
