...
class Paginator implements Countable, IteratorAggregate
{
        ...
        if (! $scrollingAdapters instanceof ScrollingStylePluginManager) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            (is_object($scrollingAdapters) ? get_class(
                                    $scrollingAdapters) : gettype(
                                    $scrollingAdapters))));
        }
            ...
            foreach ($cacheIterator as $key) {
                ...
                if ($tags && in_array($this->_getCacheInternalId(), $tags)) {
                    if (substr($key, 0, $prefixLength) == self::CACHE_TAG_PREFIX) {
                        static::$cache->removeItem(
                                $this->_getCacheId(
                                        (int) substr($key, $prefixLength)));
                    }
                }
            }
        } else {
    ...
    public function getCurrentItemCount ()
    {
        if ($this->currentItemCount === null) {
            $this->currentItemCount = $this->getItemCount(
    ...
    public function getCurrentItems ()
    {
        if ($this->currentItems === null) {
            $this->currentItems = $this->getItemsByPage(
    ...
    public function getItemCount ($items)
    {
        ...
        } elseif ($items instanceof Traversable) { // $items is something like
                                                   // LimitIterator
    ...
    protected function _loadScrollingStyle ($scrollingStyle = null)
    {
        ...
        switch (strtolower(gettype($scrollingStyle))) {
            ...
            case 'string':
                return static::getScrollingStylePluginManager()->get(
                        $scrollingStyle);
