...
abstract class AbstractControllerTestCase extends PHPUnit_Framework_TestCase
{
    ...
    public function setApplicationConfig ($applicationConfig)
    {
        ...
        if (isset(
                $applicationConfig['module_listener_options']['config_cache_enabled'])) {
    ...
    {
        if (! isset($method) && $this->getRequest() instanceof HttpRequest &&
    ...
    public function triggerApplicationEvent ($eventName)
    {
        ...
        if ($eventName != MvcEvent::EVENT_ROUTE &&
                 $eventName != MvcEvent::EVENT_DISPATCH) {
    ...
    {
        $moduleManager = $this->getApplicationServiceLocator()->get(
                'ModuleManager');
        $modulesLoaded = $moduleManager->getModules();
        $list = array_diff($modules, $modulesLoaded);
        if ($list) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Several modules are not loaded "%s"', 
                            implode(', ', $list)));
        }
    ...
    public function assertNotModulesLoaded (array $modules)
    {
        ...
        if ($list) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Several modules WAS not loaded "%s"', 
                            implode(', ', $list)));
        }
        $this->assertEquals(count($list), 0);
    }
    ...
    public function assertNotResponseStatusCode ($code)
    {
        ...
        if ($code == $match) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Failed asserting response code was NOT "%s"', 
                            $code));
        }
    ...
    public function assertApplicationException ($type, $message = null)
    {
        ...
        if (true === $this->traceError) {
            // set exception as null because we know and have assert the
            // exception
            $this->getApplication()
                ->getMvcEvent()
    ...
    {
        $routeMatch = $this->getApplication()
            ->getMvcEvent()
            ->getRouteMatch();
        $controllerIdentifier = $routeMatch->getParam('controller');
        $controllerManager = $this->getApplicationServiceLocator()->get(
                'ControllerLoader');
    ...
    /**
     * Assert that the application route match used NOT the given controller
     * class
     *
     * @param string $controller            
     */
    public function assertNotControllerClass ($controller)
    {
        ...
        if ($controller == $match) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Failed asserting controller class was NOT "%s"', 
                            $controller));
        }
    ...
    public function assertNotControllerName ($controller)
    {
        ...
        if ($controller == $match) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Failed asserting controller name was NOT "%s"', 
                            $controller));
        }
        $this->assertNotEquals($controller, $match);
    }
    ...
    public function assertNotActionName ($action)
    {
        ...
        if ($action == $match) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Failed asserting action name was NOT "%s"', 
                            $action));
        }
    ...
    public function assertNotMatchedRouteName ($route)
    {
        ...
        if ($route == $match) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Failed asserting route matched was NOT "%s"', 
                            $route));
        }
