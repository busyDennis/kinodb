...
abstract class AbstractHttpControllerTestCase extends AbstractControllerTestCase
{
    ...
    public function assertHasResponseHeader ($header)
    {
        ...
        if (false === $responseHeader) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Failed asserting response header "%s" found', 
                            $header));
        }
    ...
    public function assertResponseHeaderRegex ($header, $pattern)
    {
        ...
        $this->assertTrue(
                (bool) preg_match($pattern, $responseHeader->getFieldValue()));
    }
    ...
    public function assertNotRedirectTo ($url)
    {
        ...
        if ($url == $responseHeader->getFieldValue()) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Failed asserting response redirects to "%s"', $url));
        }
    ...
    private function queryAssertion ($path, $useXpath = false)
    {
        ...
        if (! $match > 0) {
            throw new PHPUnit_Framework_ExpectationFailedException(
                    sprintf('Failed asserting node DENOTED BY %s EXISTS', $path));
        }
    ...
    /**
     * Assert against DOM/XPath selection; should contain at least this number
     * of nodes
     *
     * @param string $path
     *            CSS selector path
     ...
     * @param bool $useXpath            
     */
    private function queryContentContainsAssertion ($path, $match, 
            $useXpath = false)
    {
     ...
     * @param bool $useXpath            
     */
    private function notQueryContentContainsAssertion ($path, $match, 
            $useXpath = false)
    {
        $result = $this->query($path, $useXpath);
        if ($result->count() == 0) {
     ...
     * @param bool $useXpath            
     */
    private function queryContentRegexAssertion ($path, $pattern, 
            $useXpath = false)
    {
        $result = $this->query($path, $useXpath);
        if ($result->count() == 0) {
        ...
        $this->assertTrue(
                (bool) preg_match($pattern, $result->current()->nodeValue));
    }
     ...
     * @param bool $useXpath            
     */
    private function notQueryContentRegexAssertion ($path, $pattern, 
            $useXpath = false)
    {
        $result = $this->query($path, $useXpath);
        if ($result->count() == 0) {
