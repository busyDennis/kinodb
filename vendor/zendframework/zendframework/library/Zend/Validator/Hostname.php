...
class Hostname extends AbstractValidator
{
    ...
    // Allows IP addresses
    const ALLOW_LOCAL = 4;
    // Allows local network names (e.g., localhost,
    // www.localdomain)
    const ALLOW_URI = 8;
    ...
    protected $validIdns = array(
            ...
            'IT' => array(
                    1 => '/^[\x{002d}0-9a-zàâäèéêëìîïòôöùûüæœçÿß-]{1,63}$/iu'
            ),
            'JP' => 'Hostname/Jp.php',
            'KR' => array(
        ...
        if ((count($domainParts) > 1) && (strlen($value) >= 4) &&
            ...
            do {
                ...
                if (preg_match('/([^.]{2,10})$/i', end($domainParts), $matches) ||
                    ...
                    if ($this->getIdnCheck() &&
                             isset($this->validIdns[strtoupper($this->tld)])) {
                        if (is_string($this->validIdns[strtoupper($this->tld)])) {
                            $regexChars += include __DIR__ . '/' .
                             $this->validIdns[strtoupper($this->tld)];
                } else {
            ...
            foreach ($domainParts as $domainPart) {
                ...
                // Check dash (-) does not start, end or appear in 3rd and 4th
                // positions
                if ((strpos($domainPart, '-') === 0) || ((strlen($domainPart) > 2) &&
                ...
                foreach ($regexChars as $regexKey => $regexChar) {
                    ...
                    if ($status > 0) {
                        $length = 63;
                        if (array_key_exists(strtoupper($this->tld), 
                                $this->idnLength) &&
                                 (array_key_exists($regexKey, 
                                        $this->idnLength[strtoupper($this->tld)]))) {
    ...
    // If the input passes as an Internet domain name, and domain names are
    // allowed, then the hostname
    // passes validation
...
if ($this->getAllow() & self::ALLOW_URI) {
    if (preg_match("/^([a-zA-Z0-9-._~!$&\'()*+,;=]|%[[:xdigit:]]{2}){1,254}$/i", 
            $value)) {
        return true;
    } else {
...
// If the input passes as a local network name, and local network names are
// allowed, then the
...
for ($indexe = ($separator) ? ($separator + 1) : 0; $indexe < $lengthe; ++ $lengthd) {
    for ($oldIndex = $index, $pos = 1, $key = 36; 1; $key += 36) {
        ...
        $digit = ($hex - 48 < 10) ? $hex - 22 : (($hex - 65 < 26) ? $hex - 65 : (($hex -
                 97 < 26) ? $hex - 97 : 36));
    ...
    $delta = intval(
            $init ? (($index - $oldIndex) / 700) : (($index - $oldIndex) / 2));
