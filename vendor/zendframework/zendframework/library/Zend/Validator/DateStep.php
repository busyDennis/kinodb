...
class DateStep extends Date
{
    ...
    protected function convertToDateTime ($param)
    {
        ...
        if (is_int($param)) {
            ...
            if (strpos($this->getFormat(), 'Y-\WW') === 0 &&
                ...
                $dateObj = DateTime::createFromFormat($this->getFormat(), 
                        $param, $this->getTimezone());
            }
        }
        if (! ($dateObj instanceof DateTime)) {
            throw new Exception\InvalidArgumentException(
                    'Invalid date param given');
        }
    ...
    if (in_array($intervalUnit, 
        ...
        if (1 == $stepValue) {
            if ('hours' === $intervalUnit && 0 == $diffParts['minutes'] &&
                     0 == $diffParts['seconds']) {
                return true;
            ...
            switch ($intervalUnit) {
                ...
                case 'seconds':
                    $seconds = ($diffParts['hours'] * 60) +
                             ($diffParts['minutes'] * 60) + $diffParts['seconds'];
