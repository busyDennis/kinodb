...
class MimeType extends AbstractValidator
{
     ...
     * - FALSE means disabling searching for mimetype, should be used for PHP
     * 5.3
        ...
        if (null === $this->options['magicFile']) {
            ...
            if (! ($safeMode == 'On' || $safeMode === 1)) {
                foreach ($this->magicFiles as $file) {
                    // suppressing errors which are thrown due to openbase_dir
                    // restrictions
                    try {
    ...
    public function setMagicFile ($file)
    {
        if ($file === false) {
            ...
            throw new Exception\InvalidArgumentException(
                    sprintf('The given magicfile ("%s") could not be read', 
                            $file));
        } else {
            ...
            if (empty($this->finfo)) {
                $this->finfo = null;
                throw new Exception\InvalidMagicMimeFileException(
                        sprintf(
     ...
     *
     * @param bool $asArray
     *            Returns the values as array, when false a concatenated string
     *            is returned
     * @return string|array
     */
