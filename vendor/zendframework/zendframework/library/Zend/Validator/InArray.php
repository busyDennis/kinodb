...
class InArray extends AbstractValidator
{
     ...
     * false/positive.
     * This is the most secure option for non-strict checks and replaces strict
     * = false
     ...
     * an option has been added to prevent this. When $strict = 0/false, the
     * most
     * secure non-strict check is implemented. if $strict = -1, the default
     * in_array non-strict
     * behaviour is used
     *
     * @var int
    ...
    public function isValid ($value)
    {
        ...
        if (self::COMPARE_NOT_STRICT_AND_PREVENT_STR_TO_INT_VULNERABILITY ==
                 $this->strict && (is_int($value) || is_float($value))) {
            $value = (string) $value;
        }
        ...
        if ($this->getRecursive()) {
            ...
            foreach ($iterator as $element) {
                if (self::COMPARE_STRICT == $this->strict) {
                    ...
                    $el = $element;
                    if (self::COMPARE_NOT_STRICT_AND_PREVENT_STR_TO_INT_VULNERABILITY ==
                             $this->strict && is_string($value) &&
                             (is_int($el) || is_float($el))) {
                        $el = (string) $el;
                    }
            ...
            /**
             * If the check is not strict, then, to prevent "asdf" being
             * converted to 0
             ...
             *
             * This occurs only if the input is a string and a haystack member
             * is an int
             */
            if (self::COMPARE_NOT_STRICT_AND_PREVENT_STR_TO_INT_VULNERABILITY ==
                     $this->strict && is_string($value)) {
            ...
            if (in_array($value, $haystack, 
                    $this->strict == self::COMPARE_STRICT ? true : false)) {
                return true;
            }
        }
