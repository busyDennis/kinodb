...
class EmailAddress extends AbstractValidator
{
     ...
     *
     * @see http://en.wikipedia.org/wiki/Reserved_IP_addresses As of RFC5753
     *      (JAN 2010), the following blocks are no longer reserved:
     *      - 128.0.0.0/16
     *      - 191.255.0.0/16
     *      - 223.255.255.0/24
     * @see http://tools.ietf.org/html/rfc5735#page-6 As of RFC6598 (APR 2012),
     *      the following blocks are now reserved:
     *      - 100.64.0.0/10
    ...
    protected function isReserved ($host)
    {
        ...
        foreach ($host as $server) {
            ...
            ! preg_match(
                    ...
                    $server) && 
            // Search for 192.0.2.0/24, 192.88.99.0/24, 198.51.100.0/24,
            // 203.0.113.0/24
            ! preg_match(
    ...
    protected function validateMXRecords ()
    {
        ...
        foreach ($this->mxRecord as $hostname => $weight) {
            ...
            if (! $res && (checkdnsrr($hostname, "A") ||
                     checkdnsrr($hostname, "AAAA") || checkdnsrr($hostname, 
                            "A6"))) {
                $validAddress = true;
                break;
            }
    ...
    public function isValid ($value)
    {
        ...
        if ($local && $length) {
            if (($this->options['useDomainCheck'] && $hostname) ||
