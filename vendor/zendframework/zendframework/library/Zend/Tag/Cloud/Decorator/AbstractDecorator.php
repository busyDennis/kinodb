...
abstract class AbstractDecorator implements Decorator
{
     ...
     *
     * If none registered, instantiates and registers one using current
     * encoding.
     *
     * @return Escaper
     */
    ...
    protected function validateElementName ($name)
    {
        if (! preg_match('/^[a-z0-9]+$/i', $name)) {
            throw new Exception\InvalidElementNameException(
                    sprintf(
                            '%s: Invalid element name "%s" provided; please provide valid HTML element names', 
                            __METHOD__, $this->getEscaper()->escapeHtml($name)));
        }
    }
    ...
    protected function wrapTag ($html)
    {
        ...
        foreach ($this->getHTMLTags() as $key => $data) {
            ...
            $html = sprintf('<%1$s%3$s>%2$s</%1$s>', $htmlTag, $html, 
                    $attributes);
        }
        return $html;
    }
