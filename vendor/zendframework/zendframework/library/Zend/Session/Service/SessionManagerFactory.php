...
class SessionManagerFactory implements FactoryInterface
{
     ...
     *
     * - enable_default_container_manager: whether to inject the created
     * instance
            ...
            if (! $storage instanceof StorageInterface) {
                throw new ServiceNotCreatedException(
                        sprintf(
                                ...
                                'Zend\Session\Storage\StorageInterface', 
                                'Zend\Session\Storage\StorageInterface', 
                                (is_object($storage) ? get_class($storage) : gettype(
                                        $storage))));
            }
        }
        
        if ($services->has('Zend\Session\SaveHandler\SaveHandlerInterface')) {
            $saveHandler = $services->get(
                    'Zend\Session\SaveHandler\SaveHandlerInterface');
            if (! $saveHandler instanceof SaveHandlerInterface) {
                throw new ServiceNotCreatedException(
                        sprintf(
                                ...
                                'Zend\Session\SaveHandler\SaveHandlerInterface', 
                                'Zend\Session\SaveHandler\SaveHandlerInterface', 
                                (is_object($saveHandler) ? get_class(
                                        $saveHandler) : gettype($saveHandler))));
            }
        }
        
        $manager = new SessionManager($config, $storage, $saveHandler);
        
        // Get session manager configuration, if any, and merge with default
        // configuration
        if ($services->has('Config')) {
            $configService = $services->get('Config');
            if (isset($configService['session_manager']) &&
                     is_array($configService['session_manager'])) {
                $managerConfig = array_merge($managerConfig, 
                        $configService['session_manager']);
            }
        }
        
        // If configuration enables the session manager as the default manager
        // for container
        // instances, do so.
