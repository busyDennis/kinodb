...
class MongoDB implements SaveHandlerInterface
{
    ...
    public function __construct ($mongo, MongoDBOptions $options)
    {
        ...
        if (null === ($database = $options->getDatabase())) {
            throw new InvalidArgumentException(
                    'The database option cannot be emtpy');
        }
        
        if (null === ($collection = $options->getCollection())) {
            throw new InvalidArgumentException(
                    'The collection option cannot be emtpy');
        }
        
        $this->mongoCollection = $mongo->selectCollection($database, 
                $collection);
    ...
    public function read ($id)
    {
        ...
        if (null !== $session) {
            if ($session[$this->options->getModifiedField()] instanceof MongoDate && $session[$this->options->getModifiedField()]->sec +
    ...
    {
        $saveOptions = array_replace($this->options->getSaveOptions(), 
                array(
                        'upsert' => true,
                        'multiple' => false
                ));
         ...
         */
        $result = $this->mongoCollection->update($criteria, $newObj, 
                $saveOptions);
