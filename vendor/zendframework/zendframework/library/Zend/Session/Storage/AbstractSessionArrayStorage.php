 ...
 */
abstract class AbstractSessionArrayStorage implements IteratorAggregate, 
        StorageInterface, StorageInitializationInterface
{
    ...
    public function setMetadata ($key, $value, $overwriteArray = false)
    {
        if ($this->isImmutable()) {
            throw new Exception\RuntimeException(
                    sprintf(
                            'Cannot set key "%s" as storage is marked isImmutable', 
                            $key));
        }
        ...
        if (isset($_SESSION['__ZF'][$key]) && is_array($value)) {
            if ($overwriteArray) {
                ...
                $_SESSION['__ZF'][$key] = array_replace_recursive(
                        $_SESSION['__ZF'][$key], $value);
            }
        } else {
