...
abstract class Factory
{
    ...
    public static function factory ($type, $options = array())
    {
        if (! is_string($type)) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, 
                            (is_object($type) ? get_class($type) : gettype(
                                    $type))));
        }
        ...
        switch (true) {
            case (in_array('Zend\Session\Storage\AbstractSessionArrayStorage', 
                    class_parents($type))):
                return static::createSessionArrayStorage($type, $options);
                break;
            case ($type === 'Zend\Session\Storage\ArrayStorage'):
            case (in_array('Zend\Session\Storage\ArrayStorage', 
                    class_parents($type))):
                return static::createArrayStorage($type, $options);
                break;
            case (in_array('Zend\Session\Storage\StorageInterface', 
            ...
            if (! is_array($options['input'])) {
                throw new Exception\InvalidArgumentException(
                        sprintf(
                                ...
                                $type, 
                                (is_object($options['input']) ? get_class(
                                        $options['input']) : gettype(
                                        $options['input']))));
            }
            $input = $options['input'];
        }
            ...
            if (! class_exists($options['iterator_class'])) {
                throw new Exception\InvalidArgumentException(
                        sprintf(
                                ...
                                $type, 
                                (is_object($options['iterator_class']) ? get_class(
                                        $options['iterator_class']) : gettype(
                                        $options['iterator_class']))));
            }
    ...
    /**
     * Create a storage object from a class extending
    ...
    protected static function createSessionArrayStorage ($type, array $options)
    {
        ...
        if (isset($options['input'])) {
            if (null !== $options['input'] && ! is_array($options['input']) &&
