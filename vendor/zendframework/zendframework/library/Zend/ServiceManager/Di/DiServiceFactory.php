...
class DiServiceFactory extends Di implements FactoryInterface
{
     ...
     */
    public function __construct (Di $di, $name, array $parameters = array(), 
            $useServiceLocator = self::USE_SL_NONE)
    {
        $this->di = $di;
        $this->name = $name;
        $this->parameters = $parameters;
        if (in_array($useServiceLocator, 
                array(
    ...
    {
        // allow this di service to get dependencies from the service locator
        // BEFORE trying di
        if ($this->useServiceLocator == self::USE_SL_BEFORE_DI &&
        ...
        try {
            ...
            // allow this di service to get dependencies from the service
            // locator AFTER trying di
            if ($this->useServiceLocator == self::USE_SL_AFTER_DI &&
                     $this->serviceLocator->has($name)) {
                return $this->serviceLocator->get($name);
            } else {
