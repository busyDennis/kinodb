...
class ServiceManager implements ServiceLocatorInterface
{
    ...
    /**
     * Set flag indicating whether to pull from peering manager before
     * attempting creation
     *
    ...
    public function setRetrieveFromPeeringManagerFirst (
            $retrieveFromPeeringManagerFirst = true)
    {
    ...
    public function setFactory ($name, $factory, $shared = null)
    {
        ...
        if (! ($factory instanceof FactoryInterface || is_string($factory) ||
                 is_callable($factory))) {
    ...
    {
        if (! $factory instanceof AbstractFactoryInterface && is_string(
                $factory)) {
            $factory = new $factory();
        }
    ...
    public function addInitializer ($initializer, $topOfStack = true)
    {
        if (! ($initializer instanceof InitializerInterface ||
            ...
            if (! ($initializer instanceof InitializerInterface ||
                     is_callable($initializer))) {
                throw new Exception\InvalidArgumentException(
                        '$initializer should be callable.');
            }
        }
            ...
            if ($this->allowOverride === false) {
                throw new Exception\InvalidServiceNameException(
                        sprintf(
                                ...
                                get_class($this) . '::' . __FUNCTION__, $name));
            }
            $this->unregisterService($cName);
        }
    ...
    public function create ($name)
    {
        if (is_array($name)) {
            ...
            // inlined code from ServiceManager::canonicalizeName for
            // performance
        ...
        if (isset($this->delegators[$cName])) {
            ...
            $creationCallback = function  () use( $serviceManager, $rName, 
            $cName)
            {
            ...
            for ($i = 0; $i < $additionalDelegators; $i += 1) {
                $creationCallback = $this->createDelegatorCallback(
                        $this->delegators[$cName][$i], $rName, $cName, 
                        $creationCallback);
            }
            ...
            return $delegatorFactory->createDelegatorWithName($this, $cName, 
                    $rName, $creationCallback);
        }
     ...
     * @return callable
     */
    private function createDelegatorCallback ($delegatorFactoryName, $rName, 
            $cName, $creationCallback)
    {
        $serviceManager = $this;
        
        return function  () use( $serviceManager, $delegatorFactoryName, $rName, 
        $cName, $creationCallback)
        {
            /* @var $delegatorFactory DelegatorFactoryInterface */
            $delegatorFactory = $serviceManager->get($delegatorFactoryName);
            
            return $delegatorFactory->createDelegatorWithName($serviceManager, 
                    $cName, $rName, $creationCallback);
        };
    }
    ...
    public function doCreate ($rName, $cName)
    {
        ...
        if ($instance === false && $this->throwExceptionInCreate) {
            throw new Exception\ServiceNotFoundException(
                    sprintf('No valid instance was found for %s%s', $cName, 
                            ($rName ? '(alias: ' . $rName . ')' : '')));
        }
    ...
    public function canCreate ($name, $checkAbstractFactories = true)
    {
        ...
        return (isset($this->invokableClasses[$cName]) ||
                 isset($this->factories[$cName]) || isset(
                        $this->aliases[$cName]) || isset(
                        $this->instances[$cName]) || ($checkAbstractFactories &&
                 $this->canCreateFromAbstractFactory($cName, $rName)));
    }
    ...
    public function canCreateFromAbstractFactory ($cName, $rName)
    {
        ...
        foreach ($this->abstractFactories as $abstractFactory) {
            ...
            if (isset($this->pendingAbstractFactoryRequests[$factoryClass]) && $this->pendingAbstractFactoryRequests[$factoryClass] ==
                     $rName) {
                return false;
            }
            ...
            if ($this->lastAbstractFactoryUsed === $objectHash &&
            ...
            if ($abstractFactory->canCreateServiceWithName($this, $cName, 
                    $rName)) {
                $this->lastAbstractFactoryUsed = $this->lastCanonicalNameUsed = null;
                return true;
            }
    ...
    public function setAlias ($alias, $nameOrAlias)
    {
        if (! is_string($alias) || ! is_string($nameOrAlias)) {
            throw new Exception\InvalidServiceNameException(
                    'Service or alias names must be strings.');
        }
        ...
        if ($alias == '' || $nameOrAlias == '') {
            throw new Exception\InvalidServiceNameException(
                    'Invalid service name alias');
        }
        
        if ($this->allowOverride === false && $this->has(
                array(
                        $cAlias,
                        $alias
                ), false)) {
            throw new Exception\InvalidServiceNameException(
                    sprintf('An alias by the name "%s" or "%s" already exists', 
                            $cAlias, $alias));
        }
     ...
     * @throws Exception\ServiceNotFoundException If resolved class does not
     *         exist
     */
    ...
    protected function createFromAbstractFactory ($canonicalName, $requestedName)
    {
        foreach ($this->abstractFactories as $index => $abstractFactory) {
            ...
            if (is_string($abstractFactory) &&
                     class_exists($abstractFactory, true)) {
            ...
            try {
                if ($abstractFactory->canCreateServiceWithName($this, 
                        $canonicalName, $requestedName)) {
                    $this->pendingAbstractFactoryRequests[get_class(
                            $abstractFactory)] = $requestedName;
                    $instance = $this->createServiceViaCallback(
                            array(
                                    $abstractFactory,
