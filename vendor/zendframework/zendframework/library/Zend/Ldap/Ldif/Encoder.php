...
class Encoder
{
    ...
    protected function _decode ($string)
    {
        ...
        foreach (explode("\n", $string) as $line) {
            ...
            if (substr($line, 0, 1) === ' ' && $last !== null && ! $inComment) {
                ...
                $inComment = true;
                continue;
            } elseif (preg_match('/^([a-z0-9;-]+)(:[:<]?\s*)([^<]*)$/i', $line, 
                    $matches)) {
                $inComment = false;
    ...
    protected function encodeAttribute ($name, $value)
    {
        ...
        foreach ($value as $v) {
            ...
            if (isset($this->options['wrap']) &&
                     strlen($attribute) > $this->options['wrap']) {
                $attribute = trim(
                        chunk_split($attribute, $this->options['wrap'], 
                                PHP_EOL . ' '));
            }
    ...
    protected function encodeAttributes (array $attributes)
    {
        ...
        if (! $this->versionWritten && array_key_exists('dn', $attributes) &&
                 isset($this->options['version']) &&
                 array_key_exists('objectclass', $attributes)) {
            $string .= sprintf('version: %d', $this->options['version']) .
             PHP_EOL;
