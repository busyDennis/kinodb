...
class Converter
{
    ...
    {
        $string = preg_replace_callback('/\\\([0-9A-Fa-f]{2})/', 
                function  ($matches)
                {
    ...
    public static function toLdap ($value, $type = self::STANDARD)
    {
        try {
            ...
            throw new Exception\ConverterException($e->getMessage(), 
                    $e->getCode(), $e);
        }
    }
     ...
     *            The date-entity
     * @param bool $asUtc
     *            Whether to return the LDAP-compatible date-string as UTC or as
     *            local value
     * @return string
