...
class BaseInputFilter implements InputFilterInterface, 
        UnknownInputsCapableInterface, InitializableInterface
{
    ...
    public function add ($input, $name = null)
    {
        if (! $input instanceof InputInterface &&
            ...
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, 'Zend\InputFilter\InputInterface', 
                            'Zend\InputFilter\InputFilterInterface', 
                            (is_object($input) ? get_class($input) : gettype(
                                    $input))));
        }
        ...
        if (isset($this->inputs[$name]) &&
            ...
            // The element already exists, so merge the config. Please note that
            // the order is important (already existing
    ...
    public function get ($name)
    {
        if (! array_key_exists($name, $this->inputs)) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s: no input found matching "%s"', __METHOD__, 
                            $name));
        }
    ...
    public function isValid ()
    {
        if (null === $this->data) {
            throw new Exception\RuntimeException(
                    sprintf('%s: no data present to validate!', __METHOD__));
        }
    ...
    protected function validateInputs (array $inputs)
    {
        ...
        foreach ($inputs as $name) {
            ...
            if (! $dataExists && $input instanceof InputInterface &&
                     ! $input->isRequired()) {
            ...
            if (! $dataExists && $input instanceof InputInterface &&
                ...
                if (! ($input instanceof EmptyContextInterface &&
    ...
    if ($dataExists && null === $this->data[$name] &&
             $input instanceof InputInterface && ! $input->isRequired()) {
        $this->validInputs[$name] = $input;
        continue;
    }
...
if ($dataExists && '' === $this->data[$name] && $input instanceof InputInterface &&
     ! $input->isRequired()) {
$this->validInputs[$name] = $input;
continue;
}
...
if ($dataExists && is_array($this->data[$name]) && ((isset(
    $this->data[$name]['error']) &&
     $this->data[$name]['error'] === UPLOAD_ERR_NO_FILE) || (count(
            $this->data[$name]) === 1 && isset($this->data[$name][0]) &&
     is_array($this->data[$name][0]) && isset($this->data[$name][0]['error']) &&
 ...
 * set.
 *
 * If the initial value is {@link VALIDATE_ALL}, the current validation group,
 * if
...
if (! array_key_exists($name, $this->inputs)) {
throw new Exception\InvalidArgumentException(
    sprintf('%s expects a valid input name; "%s" was not found in the filter', 
            __METHOD__, $name));
}
$input = $this->inputs[$name];
