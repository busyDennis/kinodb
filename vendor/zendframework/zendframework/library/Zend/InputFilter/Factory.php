...
class Factory
{
    ...
    /**
     * Clear the default validator chain (i.e., don't inject one into new
     * inputs)
     *
     * @return void
     */
    ...
    public function createInput ($inputSpecification)
    {
        if (! is_array($inputSpecification) &&
                 ! $inputSpecification instanceof Traversable) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s expects an array or Traversable; received "%s"', 
                            __METHOD__, 
                            (is_object($inputSpecification) ? get_class(
                                    $inputSpecification) : gettype(
                                    $inputSpecification))));
        }
        if ($inputSpecification instanceof Traversable) {
            $inputSpecification = ArrayUtils::iteratorToArray(
                    $inputSpecification);
        }
        ...
        if (! $input instanceof InputInterface) {
            throw new Exception\RuntimeException(
                    sprintf(
                            ...
                            'Zend\InputFilter\InputInterface', $class));
        }
        ...
        foreach ($inputSpecification as $key => $value) {
            switch ($key) {
                ...
                case 'continue_if_empty':
                    $input->setContinueIfEmpty(
                            $inputSpecification['continue_if_empty']);
                    break;
                case 'error_message':
                    ...
                    if (! is_array($value) && ! $value instanceof Traversable) {
                        throw new Exception\RuntimeException(
                                sprintf(
                                        ...
                                        __METHOD__, 
                                        (is_object($value) ? get_class($value) : gettype(
                                                $value))));
                    }
                ...
                case 'validators':
                    ...
                    $this->populateValidators($input->getValidatorChain(), 
                            $value);
                    break;
                default:
                    // ignore unknown keys
    ...
    public function createInputFilter ($inputFilterSpecification)
    {
        if (! is_array($inputFilterSpecification) &&
                 ! $inputFilterSpecification instanceof Traversable) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s expects an array or Traversable; received "%s"', 
                            __METHOD__, 
                            (is_object($inputFilterSpecification) ? get_class(
                                    $inputFilterSpecification) : gettype(
                                    $inputFilterSpecification))));
        }
        if ($inputFilterSpecification instanceof Traversable) {
            $inputFilterSpecification = ArrayUtils::iteratorToArray(
                    $inputFilterSpecification);
        }
        ...
        if (isset($inputFilterSpecification['type']) &&
                 is_string($inputFilterSpecification['type'])) {
