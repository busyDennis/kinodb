...
class FileInput extends Input
{
    ...
    public function isValid ($context = null)
    {
        ...
        // $value = $this->getValue(); // Do not run the filters yet for File
        // uploads (see getValue())
        $rawValue = $this->getRawValue();
        if (! is_array($rawValue)) {
            // This can happen in an AJAX POST, where the input comes across as
            // a string
            $rawValue = array(
                    'tmp_name' => $rawValue,
                    'name' => $rawValue,
        ...
        if (is_array($rawValue) && isset($rawValue['tmp_name'])) {
            ...
            $this->isValid = $validator->isValid($rawValue, $context);
                 ...
                 isset($rawValue[0]['tmp_name'])) {
            // Multi file input (multiple attribute set)
            $this->isValid = true;
            foreach ($rawValue as $value) {
                if (! $validator->isValid($value, $context)) {
                    $this->isValid = false;
                    break; // Do not continue processing files if validation
                               // fails
                }
            }
        }
