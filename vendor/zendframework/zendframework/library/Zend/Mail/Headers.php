...
class Headers implements Countable, Iterator
{
    ...
    public static function fromString ($string, $EOL = self::EOL)
    {
        ...
        foreach (explode($EOL, $string) as $line) {
            ...
            if (preg_match('/^(?P<name>[\x21-\x39\x3B-\x7E]+):.*$/', $line, 
                    $matches)) {
                if ($currentLine) {
        ...
        if (! is_string($headerFieldNameOrLine)) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            (is_object($headerFieldNameOrLine) ? get_class(
                                    $headerFieldNameOrLine) : gettype(
                                    $headerFieldNameOrLine))));
        }
        
        if ($fieldValue === null) {
            $this->addHeader(
                    Header\GenericHeader::fromString($headerFieldNameOrLine));
        } elseif (is_array($fieldValue)) {
            foreach ($fieldValue as $i) {
                $this->addHeader(
                        new Header\GenericMultiHeader($headerFieldNameOrLine, $i));
            }
        } else {
            $this->addHeader(
                    new Header\GenericHeader($headerFieldNameOrLine, $fieldValue));
        }
     ...
     *         is no headers with $name in this
     *         contain, an ArrayIterator if the header is a
     *         MultipleHeadersInterface instance and finally returns
