...
class Imap
{
    ...
    public function connect ($host, $port = null, $ssl = false)
    {
        ...
        if (! $this->socket) {
            throw new Exception\RuntimeException(
                    sprintf('cannot connect to host %s', 
                            ($error ? sprintf('; error = %s (errno = %d )', 
        ...
        if ($isTls) {
            ...
            $result = $result && stream_socket_enable_crypto($this->socket, 
    ...
    protected function _decodeLine ($line)
    {
        ...
        while (($pos = strpos($line, ' ')) !== false) {
            ...
            if ($stack && $token[strlen($token) - 1] == ')') {
                // closing braces are not separated by spaces, so we need to
                // count them
                $braces = strlen($token);
                $token = rtrim($token, ')');
     ...
     *         value)
     *         if items of messages are fetched it's returned as (msgno => (name
     *         => value))
     */
    ...
    public function store (array $flags, $from, $to = null, $mode = null, $silent = true)
    {
        ...
        $result = $this->requestAndResponse('STORE', 
                array(
                        $set,
                        $item,
                        $flags
