...
abstract class AbstractProtocol
{
    ...
    public function __construct ($host = '127.0.0.1', $port = null)
    {
        ...
        $this->validHost->attach(
                new Validator\Hostname(Validator\Hostname::ALLOW_ALL));
        
        if (! $this->validHost->isValid($host)) {
            throw new Exception\RuntimeException(
                    implode(', ', $this->validHost->getMessages()));
        }
    ...
    protected function _connect ($remote)
    {
        ...
        // open connection
        $this->socket = @stream_socket_client($remote, $errorNum, $errorStr, 
        ...
        if (($result = stream_set_timeout($this->socket, 
    ...
    protected function _send ($request)
    {
        if (! is_resource($this->socket)) {
            throw new Exception\RuntimeException(
                    'No connection has been established to ' . $this->host);
        }
        ...
        if ($result === false) {
            throw new Exception\RuntimeException(
                    'Could not send request to ' . $this->host);
        }
