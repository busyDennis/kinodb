...
class Maildir extends Folder\Maildir implements WritableInterface
{
    ...
    public static function initMaildir ($dir)
    {
        if (file_exists($dir)) {
            if (! is_dir($dir)) {
                throw new StorageException\InvalidArgumentException(
            ...
            if (! $test) {
                ...
                if (! file_exists($dir)) {
                    throw new StorageException\InvalidArgumentException(
                            "parent $dir not found", 0, $error);
                } elseif (! is_dir($dir)) {
                    throw new StorageException\InvalidArgumentException(
                            "parent $dir not a directory", 0, $error);
                } else {
                    throw new StorageException\RuntimeException(
                            'cannot create maildir', 0, $error);
                }
            }
        }
        ...
        foreach (array(
            ...
            if (! $test) {
                ...
                if (! file_exists($dir . DIRECTORY_SEPARATOR . $subdir)) {
                    throw new StorageException\RuntimeException(
                            'could not create subdir ' . $subdir, 0, $error);
                }
            }
        }
    }
    ...
    public function createFolder ($name, $parentFolder = null)
    {
        ...
        if (strpos($folder, DIRECTORY_SEPARATOR) !== false ||
            ...
            throw new StorageException\RuntimeException(
                    'invalid name - no directory separator allowed in folder name');
        }
        ...
        $this->getFolders($parent)->$localName = new Folder($localName, $folder, 
                true);
    ...
    public function removeFolder ($name)
    {
        ...
        $parent = strpos($name, $this->delim) ? substr($name, 0, 
                strrpos($name, $this->delim)) : null;
    ...
    {
        if ($folder == 'INBOX') {
            $tmpdir = $this->rootdir . DIRECTORY_SEPARATOR . 'tmp' .
                     DIRECTORY_SEPARATOR;
        } else {
            $tmpdir = $this->rootdir . '.' . $folder . DIRECTORY_SEPARATOR .
        ...
        if (! $fh) {
            throw new StorageException\RuntimeException(
                    "tried $maxTries unique ids for a temp file, but all were taken" .
                             ' - giving up');
        }
     ...
     * @throws \Zend\Mail\Storage\Exception\RuntimeException
     */
    // not yet * @param string|\Zend\Mail\Message|\Zend\Mime\Message $message
            ...
            $recent = false)
    {
        ...
        if (! link($tempFile['filename'], $newFilename)) {
            $exception = new StorageException\RuntimeException(
    ...
    public function copyMessage ($id, $folder)
    {
        ...
        $newFile = $tempFile['dirname'] . DIRECTORY_SEPARATOR . 'cur' .
                 DIRECTORY_SEPARATOR . $tempFile['uniq'] . $info;
        ...
        if ($folder->getGlobalName() == $this->currentFolder || ($this->currentFolder ==
    ...
    public function setFlags ($id, $flags)
    {
        ...
        $error = ErrorHandler::stop();
        if (! $test) {
            throw new StorageException\RuntimeException('cannot rename file', 0, 
                    $error);
        }
    ...
    public function getQuota ($fromStorage = false)
    {
        if ($fromStorage) {
            ...
            if (! $fh) {
                throw new StorageException\RuntimeException(
                        'cannot open maildirsize', 0, $error);
            }
            $definition = fgets($fh);
            fclose($fh);
    ...
    protected function _calculateMaildirsize ()
    {
        ...
        foreach ($folders as $folder) {
            ...
            foreach (array(
                ...
                $dirname = $this->rootdir . $subdir . DIRECTORY_SEPARATOR .
                ...
                // therefore not be fully enforced, but next request will fail
    ...
    protected function _calculateQuota ($forceRecalc = false)
    {
        ...
        $messages = 0;
        $maildirsize = '';
        if (! $forceRecalc && file_exists($this->rootdir . 'maildirsize') &&
        ...
        if ($overQuota && ($maildirsize ||
                 filemtime($this->rootdir . 'maildirsize') > time() - 900)) {
            $result = $this->_calculateMaildirsize();
            $totalSize = $result['size'];
            $messages = $result['count'];
            $quota = $result['quota'];
