...
class Mbox extends AbstractStorage
{
    ...
    public function getMessage ($id)
    {
        ...
        if (strtolower($this->messageClass) == '\zend\mail\storage\message\file' || is_subclass_of(
        ...
        if ($bodyLines) {
            $message .= "\n";
            while ($bodyLines -- &&
                     ftell($this->fh) < $this->positions[$id - 1]['end']) {
    ...
    protected function openMboxFile ($filename)
    {
        ...
        if (! $this->fh) {
            throw new Exception\RuntimeException('cannot open mbox file', 0, 
                    $error);
        }
        ...
        if (! $this->isMboxFile($this->fh, false)) {
            ...
            $error = ErrorHandler::stop();
            throw new Exception\InvalidArgumentException(
                    'file is not a valid mbox format', 0, $error);
        }
