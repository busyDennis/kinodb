...
class Collection extends Fieldset implements FieldsetPrepareAwareInterface
{
     ...
     * - allow_add: if set to true, elements can be added to the form
        ...
        if (! is_array($object) && ! $object instanceof Traversable) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, 
                            (is_object($object) ? get_class($object) : gettype(
                                    $object))));
        }
    ...
    public function populateValues ($data)
    {
        ...
        if (count($data) < $this->getCount()) {
            ...
            // If there are less data and that allowRemove is true, we remove
            // elements that are not presents
        ...
        // If there are still data, this means that elements or fieldsets were
        // dynamically added. If allowed by the user, add them
        ...
        if (! $elementOrFieldset instanceof ElementInterface) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, __NAMESPACE__ . '\ElementInterface', 
                            (is_object($elementOrFieldset) ? get_class(
                                    $elementOrFieldset) : gettype(
                                    $elementOrFieldset))));
        }
    ...
    /**
     * Prepare the collection by adding a dummy template element if the user
     * want one
     *
            ...
            if ($this->hydrator) {
                ...
                if (! $this->createNewObjects() && $this->has($key)) {
                    ...
                    if ($fieldset instanceof Fieldset &&
                             $fieldset->allowObjectBinding($value)) {
