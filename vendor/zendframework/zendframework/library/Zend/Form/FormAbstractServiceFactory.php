...
class FormAbstractServiceFactory implements AbstractFactoryInterface
{
            ...
            $name, $rName)
    {
        ...
        return (isset($config[$rName]) && is_array($config[$rName]) &&
                 ! empty($config[$rName]));
    }
     ...
     */
    protected function marshalInputFilter (array &$config, 
            ServiceLocatorInterface $services, Factory $formFactory)
    {
        ...
        if (is_string($config['input_filter']) &&
            ...
            if ($inputFilters->has($config['input_filter'])) {
                $config['input_filter'] = $inputFilters->get(
                        $config['input_filter']);
                return;
            }
        }
