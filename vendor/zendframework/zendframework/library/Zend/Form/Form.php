...
class Form extends Fieldset implements FormInterface
{
    ...
    /**
     * Whether or not to bind values to the bound object on successful
     * validation
     *
     * @var int
     */
    ...
    {
        // TODO: find a better solution than duplicating the factory code, the
        // problem being that if $elementOrFieldset is an array,
        // it is passed by value, and we don't get back the concrete
        // ElementInterface
        if (is_array($elementOrFieldset) || ($elementOrFieldset instanceof Traversable &&
    ...
    public function prepare ()
    {
        ...
        // If the user wants to, elements names can be wrapped by the form's
        // name
    ...
    /**
     * Ensures state is ready for use.
     * Here, we append the name of the fieldsets to every elements in order to
     * avoid
        ...
        if (! is_array($data)) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, 
                            (is_object($data) ? get_class($data) : gettype(
                                    $data))));
        }
        ...
        if (! in_array($flags, 
            ...
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            'Zend\Form\FormInterface::VALUES_RAW', $flags));
        }
    ...
    public function bindValues (array $values = array())
    {
        if (! is_object($this->object)) {
            if ($this->baseFieldset === null ||
                     $this->baseFieldset->allowValueBinding() == false) {
                return;
            }
        }
        ...
        // If there is a base fieldset, only hydrate beginning from the base
        // fieldset
        if ($this->baseFieldset !== null) {
    ...
    /**
     * Set flag indicating whether or not to bind values on successful
     * validation
     *
    ...
    {
        if (! in_array($bindOnValidateFlag, 
                array(
                        self::BIND_ON_VALIDATE,
                        self::BIND_MANUAL
                ))) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s expects the flag to be one of %s::%s or %s::%s', 
                            __METHOD__, get_class($this), 'BIND_ON_VALIDATE', 
                            get_class($this), 'BIND_MANUAL'));
        }
    ...
    public function setValidationGroup ()
    {
        ...
        if (0 === $argc) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s expects at least one argument; none provided', 
                            __METHOD__));
        }
     ...
     * function also handle the case where elements
     * could have been dynamically added or removed from a collection using
     * JavaScript)
     *
            ...
            array $data, array &$validationGroup)
    {
        foreach ($validationGroup as $key => &$value) {
            ...
            if ($fieldset instanceof Collection) {
                ...
                $this->prepareValidationGroup($fieldset, $data[$key], 
                        $validationGroup[$key]);
            }
        }
    }
            ...
            FieldsetInterface $fieldset)
    {
        ...
        if (! $fieldset instanceof Collection ||
            ...
            foreach ($elements as $element) {
                ...
                if (! $element instanceof InputProviderInterface) {
                    ...
                    // Create an input based on the specification returned from
                    // the element
        ...
        foreach ($fieldset->getFieldsets() as $childFieldset) {
            ...
            if (! $childFieldset instanceof InputFilterProviderInterface) {
                if (! $inputFilter->has($name)) {
                    // Add a new empty input filter if it does not exist (or the
                    // fieldset's object input filter),
                    // so that elements of nested fieldsets can be recursively
                    // added
                    if ($childFieldset->getObject() instanceof InputFilterAwareInterface) {
                        $inputFilter->add(
                                $childFieldset->getObject()
                                    ->getInputFilter(), $name);
                    } else {
                        if ($fieldset instanceof Collection &&
                                 $inputFilter instanceof CollectionInputFilter) {
                ...
                if (! $fieldsetFilter instanceof InputFilterInterface) {
                    // Input attached for fieldset, not input filter; nothing
                    // more to do.
                    continue;
                }
                ...
                $this->attachInputFilterDefaults($fieldsetFilter, 
                        $childFieldset);
                continue;
            }
            ...
            // Create an input filter based on the specification returned from
            // the fieldset
