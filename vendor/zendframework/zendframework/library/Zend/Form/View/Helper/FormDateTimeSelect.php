...
class FormDateTimeSelect extends FormDateSelectHelper
{
    ...
    public function __invoke (ElementInterface $element = null, 
            $dateType = IntlDateFormatter::LONG, $timeType = IntlDateFormatter::LONG, $locale = null)
    {
        if (! $element) {
            return $this;
        }
    ...
    public function render (ElementInterface $element)
    {
        ...
        $yearOptions = $this->getYearsOptions($element->getMinYear(), 
                $element->getMaxYear());
        ...
        $monthElement = $element->getMonthElement()->setValueOptions(
                $monthsOptions);
        ...
        $minuteElement = $element->getMinuteElement()->setValueOptions(
                $minuteOptions);
        $secondElement = $element->getSecondElement()->setValueOptions(
                $secondOptions);
    ...
    public function getPattern ()
    {
        if ($this->pattern === null) {
            $intl = new IntlDateFormatter($this->getLocale(), $this->dateType, 
                    $this->timeType);
        ...
        foreach ($pregResult as $value) {
            if (stripos($value, "'") === false && stripos($value, 'd') !== false) {
                $result['day'] = $value;
                     ...
                     strpos($value, 'M') !== false) {
                $result['month'] = $value;
            } elseif (stripos($value, "'") === false &&
                     stripos($value, 'y') !== false) {
                $result['year'] = $value;
            } elseif (stripos($value, "'") === false &&
                     stripos($value, 'h') !== false) {
                $result['hour'] = $value;
            } elseif (stripos($value, "'") === false &&
                     stripos($value, 'm') !== false) {
                $result['minute'] = $value;
            } elseif (stripos($value, "'") === false &&
    ...
    {
        $keyFormatter = new IntlDateFormatter($this->getLocale(), null, null, 
                null, null, 'HH');
        $valueFormatter = new IntlDateFormatter($this->getLocale(), null, null, 
                null, null, $pattern);
