...
class Factory
{
     ...
     * @return Factory
     */
    public function setInputFilterFactory (
     ...
     * @return Factory
     */
    public function setFormElementManager (
    ...
    public function create ($spec)
    {
        ...
        throw new Exception\DomainException(
                sprintf(
                        ...
                        __METHOD__, 'Zend\Form\ElementInterface', 
                        'Zend\Form\FieldsetInterface', 'Zend\Form\FormInterface', 
                        $type));
    }
     ...
     * - options: an array, Traversable, or ArrayAccess object of element
     * options
    ...
    public function configureElement (ElementInterface $element, $spec)
    {
        ...
        if (is_array($attributes) || $attributes instanceof Traversable ||
                 $attributes instanceof ArrayAccess) {
    ...
    public function configureFieldset (FieldsetInterface $fieldset, $spec)
    {
        ...
        if (isset($spec['object'])) {
            $this->prepareAndInjectObject($spec['object'], $fieldset, 
                    __METHOD__);
        }
        
        if (isset($spec['hydrator'])) {
            $this->prepareAndInjectHydrator($spec['hydrator'], $fieldset, 
                    __METHOD__);
        }
        
        if (isset($spec['elements'])) {
            $this->prepareAndInjectElements($spec['elements'], $fieldset, 
                    __METHOD__);
        }
        
        if (isset($spec['fieldsets'])) {
            $this->prepareAndInjectFieldsets($spec['fieldsets'], $fieldset, 
                    __METHOD__);
        }
    ...
    public function configureForm (FormInterface $form, $spec)
    {
        ...
        if (isset($spec['input_filter'])) {
            $this->prepareAndInjectInputFilter($spec['input_filter'], $form, 
                    __METHOD__);
        }
        
        if (isset($spec['validation_group'])) {
            $this->prepareAndInjectValidationGroup($spec['validation_group'], 
                    $form, __METHOD__);
        }
        ...
        if (! $spec instanceof ArrayAccess) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            $method, 
                            (is_object($spec) ? get_class($spec) : gettype(
                                    $spec))));
        }
     ...
     * @return void
     */
    protected function prepareAndInjectElements ($elements, 
            FieldsetInterface $fieldset, $method)
    {
    ...
    public function prepareAndInjectFieldsets ($fieldsets, 
            FieldsetInterface $masterFieldset, $method)
    {
     ...
     * @return void
     */
    protected function prepareAndInjectObject ($objectName, 
            FieldsetInterface $fieldset, $method)
    {
        if (! is_string($objectName)) {
            throw new Exception\DomainException(
                    sprintf('%s expects string class name; received "%s"', 
                            $method, 
                            (is_object($objectName) ? get_class($objectName) : gettype(
                                    $objectName))));
        }
     ...
     * try first to instantiates the class
     * by pulling it from service manager, and injects the hydrator instance
     * into the form.
     *
     ...
     * @param string $method            
     * @return void
     * @throws Exception\DomainException If $hydratorOrName is not a string,
     *         does not resolve to a known class, or
     *         the class does not implement Hydrator\HydratorInterface
     */
    protected function prepareAndInjectHydrator ($hydratorOrName, 
            FieldsetInterface $fieldset, $method)
    {
        ...
        if (! $hydrator instanceof Hydrator\HydratorInterface) {
            throw new Exception\DomainException(
                    sprintf(
                            ...
                            $method, $hydratorOrName));
        }
        
        if (! empty($hydratorOptions) &&
                 $hydrator instanceof Hydrator\HydratorOptionsInterface) {
     ...
     * @param string $method            
     * @return void
     * @throws Exception\DomainException for unknown InputFilter class or
     *         invalid InputFilter instance
     */
    protected function prepareAndInjectInputFilter ($spec, FormInterface $form, 
            $method)
    {
     ...
     * @param string $method            
     * @return void
     * @throws Exception\DomainException if validation group given is not an
     *         array
     */
    protected function prepareAndInjectValidationGroup ($spec, 
            FormInterface $form, $method)
    {
        if (! is_array($spec)) {
            if (! class_exists($spec)) {
                throw new Exception\DomainException(
