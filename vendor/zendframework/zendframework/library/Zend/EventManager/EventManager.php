...
class EventManager implements EventManagerInterface
{
     ...
     * @return EventManager
     */
    public function setSharedManager (
    ...
    {
        // "false" means "I do not want a shared manager; don't try and fetch
        // one"
    ...
    public function addIdentifiers ($identifiers)
    {
        if (is_array($identifiers) || $identifiers instanceof Traversable) {
            $this->identifiers = array_unique(
                    array_merge($this->identifiers, (array) $identifiers));
        } elseif ($identifiers !== null) {
            $this->identifiers = array_unique(
     ...
     *            Object calling emit, or symbol describing target (such as
     *            static method name)
     ...
     *            An event or array of event names. If a
     *            ListenerAggregateInterface, proxies to {@link
     *            attachAggregate()}.
     * @param callable|int $callback
     *            If string $event provided, expects PHP callback; for a
     *            ListenerAggregateInterface $event, this will be the priority
     * @param int $priority
    ...
    public function attach ($event, $callback = null, $priority = 1)
    {
        ...
        if (null === $callback) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s: expects a callback; none provided', __METHOD__));
        }
        
        // Array of events should be registered individually, and return an
        ...
        // Create a callback handler, setting the event and priority in its
        // metadata
        $listener = new CallbackHandler($callback, 
                array(
                        'event' => $event,
                        'priority' => $priority
                ));
     ...
     */
    public function attachAggregate (ListenerAggregateInterface $aggregate, 
            $priority = 1)
    {
        ...
        if (! $listener instanceof CallbackHandler) {
            throw new Exception\InvalidArgumentException(
                    sprintf(
                            ...
                            __METHOD__, 
                            (is_object($listener) ? get_class($listener) : gettype(
                                    $listener))));
        }
     ...
     * Trigger listeners
     *
     * Actual functionality for triggering listeners, to which both trigger()
     * and triggerUntil()
     * delegate.
     *
     * @param string $event
