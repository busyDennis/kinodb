...
class Di implements DependencyInjectionInterface
{
    ...
    public function __construct (DefinitionList $definitions = null, 
            InstanceManager $instanceManager = null, Config $config = null)
    {
     ...
     * @return array
     */
    protected function getCallParameters ($name, array $params, 
            $method = "__construct")
    {
        ...
        if ($this->definitions->hasClass($class)) {
            ...
            if ($this->definitions->hasMethod($class, $method)) {
                foreach ($this->definitions->getMethodParameters($class, 
                        $method) as $param) {
    ...
    {
        array_push($this->instanceContext, 
                array(
                        'GET',
                        $name,
                        null
    ...
    public function newInstance ($name, array $params = array(), $isShared = true)
    {
        ...
        if ($instantiator === '__construct') {
            ...
            if (is_array($instantiator)) {
                ...
                $msg = sprintf('Invalid instantiator of type "%s" for "%s".', 
                        gettype($instantiator), $name);
            }
        ...
        if ($isShared) {
            if ($callParameters = $this->getCallParameters($name, $params)) {
                $this->instanceManager->addSharedInstanceWithParameters(
                        $instance, $name, $callParameters);
            } else {
                $this->instanceManager->addSharedInstance($instance, $name);
            }
        }
        
        $this->handleInjectDependencies($instance, $injectionMethods, $params, 
                $class, $alias, $name);
    ...
    public function injectDependencies ($instance, array $params = array())
    {
        ...
        $injectionMethods = array(
                $class => ($definitions->hasClass($class)) ? $definitions->getMethods(
                        $class) : array()
        );
        $parent = $class;
        ...
        foreach (class_implements($class) as $interface) {
            if ($definitions->hasClass($interface)) {
                $injectionMethods[$interface] = $definitions->getMethods(
                        $interface);
            }
        }
        $this->handleInjectDependencies($instance, $injectionMethods, $params, 
     ...
     * @throws Exception\RuntimeException
     */
    protected function handleInjectDependencies ($instance, $injectionMethods, 
            $params, $instanceClass, $instanceAlias, $requestedName)
    {
        // localize dependencies
        $definitions = $this->definitions;
        $instanceManager = $this->instanceManager();
            ...
            foreach ($injectionMethods as $type => $typeInjectionMethods) {
                foreach ($typeInjectionMethods as $typeInjectionMethod => $methodRequirementType) {
                    if (! isset($calledMethods[$typeInjectionMethod])) {
                        if ($this->resolveAndCallInjectionMethodForInstance(
                                $instance, $typeInjectionMethod, $params, 
                                $instanceAlias, $methodRequirementType, $type)) {
                ...
                if ($instanceConfig['injections']) {
                    ...
                    foreach ($instanceConfig['injections'] as $injectName => $injectValue) {
                        if (is_int($injectName) && is_string($injectValue)) {
                            $objectsToInject[] = $this->get($injectValue, 
                                    $params);
                        } elseif (is_string($injectName) &&
                                 is_array($injectValue)) {
                    ...
                    if ($objectsToInject) {
                        foreach ($objectsToInject as $objectToInject) {
                            ...
                            foreach ($injectionMethods as $type => $typeInjectionMethods) {
                                foreach ($typeInjectionMethods as $typeInjectionMethod => $methodRequirementType) {
                                    if (! isset(
                                            $calledMethods[$typeInjectionMethod])) {
                                        $methodParams = $definitions->getMethodParameters(
                                                $type, $typeInjectionMethod);
                                        if ($methodParams) {
                                            foreach ($methodParams as $methodParam) {
                                                $objectToInjectClass = $this->getClass(
                                                        $objectToInject);
                                                if ($objectToInjectClass ==
                                                         $methodParam[1] ||
                                                         self::isSubclassOf(
                                                                $objectToInjectClass, 
                                                                $methodParam[1])) {
                    ...
                    if ($methodsToCall) {
                        foreach ($methodsToCall as $methodInfo) {
                            $this->resolveAndCallInjectionMethodForInstance(
                                    $instance, $methodInfo['method'], 
                                    $methodInfo['args'], $instanceAlias, 
                                    self::METHOD_IS_REQUIRED, $instanceClass);
                        }
                    }
                }
            }
            ...
            $alias = null)
    {
        ...
        if ($this->definitions->hasMethod($class, '__construct')) {
            $callParameters = $this->resolveMethodParameters($class, 
                    '__construct', $params, $alias, self::METHOD_IS_CONSTRUCTOR, 
                    true);
        }
    ...
    protected function createInstanceViaCallback ($callback, $params, $alias)
    {
        ...
        if ($this->definitions->hasMethod($class, $method)) {
            $callParameters = $this->resolveMethodParameters($class, $method, 
                    $params, $alias, self::METHOD_IS_INSTANTIATOR, true);
        }
     ...
     * @return bool
     */
    protected function resolveAndCallInjectionMethodForInstance ($instance, 
            $method, $params, $alias, $methodRequirementType, 
            $methodClass = null)
    {
        $methodClass = ($methodClass) ?  : $this->getClass($instance);
        $callParameters = $this->resolveMethodParameters($methodClass, $method, 
                $params, $alias, $methodRequirementType);
    ...
    protected function resolveMethodParameters ($class, $method, 
            array $callTimeUserParams, $alias, $methodRequirementType, 
            $isInstantiator = false)
    {
        ...
        if ($requestedClass != $class &&
                 $this->instanceManager->hasConfig($requestedClass)) {
            $iConfig['requestedClass'] = $this->instanceManager->getConfig(
                    $requestedClass);
            if ($requestedAlias) {
                $iConfig['requestedAlias'] = $this->instanceManager->getConfig(
                        $requestedAlias);
            }
        }
        
        // This is a 2 pass system for resolving parameters
        // first pass will find the sources, the second pass will order them and
        // resolve lookups if they exist
        // MOST methods will only have a single parameters to resolve, so this
        // should be fast
        
        foreach ($injectionMethodParameters as $fqParamPos => $info) {
            list ($name, $type, $isRequired) = $info;
            
            $fqParamName = substr_replace($fqParamPos, ':' . $info[0], 
                    strrpos($fqParamPos, ':'));
            ...
            // PRIORITY 2 -specific instance configuration (thisAlias) - this
            // alias
            // PRIORITY 3 -THEN specific instance configuration (thisClass) -
            // this class
            // PRIORITY 4 -THEN specific instance configuration (requestedAlias)
            // - requested alias
            // PRIORITY 5 -THEN specific instance configuration (requestedClass)
            // - requested class
                ...
                if (isset($iConfig[$thisIndex]['parameters'][$fqParamPos]) ||
                    ...
                    if (isset($iConfig[$thisIndex]['parameters'][$fqParamPos])) {
                        $iConfigCurValue = & $iConfig[$thisIndex]['parameters'][$fqParamPos];
                            ...
                            $iConfig[$thisIndex]['parameters'][$fqParamName])) {
            ...
            // use AwareInterface or specify the method requirement option
            // METHOD_IS_EAGER at ClassDefinition
            if ($methodRequirementType & self::RESOLVE_EAGER) {
                if ($alias && $this->instanceManager->hasTypePreferences($alias)) {
                    $pInstances = $this->instanceManager->getTypePreferences(
                            $alias);
            ...
            if ($type && $isRequired &&
                     ($methodRequirementType & self::RESOLVE_EAGER)) {
                $computedParams['retrieval'][$fqParamPos] = array(
                        $type,
                        $type
                );
        ...
        foreach ($injectionMethodParameters as $fqParamPos => $value) {
            ...
            if (isset($computedParams['value'][$fqParamPos])) {
                ...
                // detect circular dependencies! (they can only happen in
                // instantiators)
                if ($isInstantiator && in_array(
                        $computedParams['retrieval'][$fqParamPos][1], 
                ...
                $dConfig = $this->instanceManager->getConfig(
                        $computedParams['retrieval'][$fqParamPos][0]);
                
                try {
                    if ($dConfig['shared'] === false) {
                        $resolvedParams[$index] = $this->newInstance(
                                $computedParams['retrieval'][$fqParamPos][0], 
                                $callTimeUserParams, false);
                    } else {
                ...
                array_pop($this->currentDependencies);
            } elseif (! array_key_exists($fqParamPos, 
                    $computedParams['optional'])) {
                if ($methodRequirementType & self::RESOLVE_STRICT) {
     ...
     *           ServiceLocator\DependencyInjectorProxy class to interact with
     *           instances
     *           and is a hack to be used internally until a major refactor does
     *           not split the `resolveMethodParameters`. Do not
