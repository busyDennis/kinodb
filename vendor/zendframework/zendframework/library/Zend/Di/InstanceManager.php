...
class InstanceManager /* implements InstanceManagerInterface */
{
    ...
    public function addSharedInstance ($instance, $classOrAlias)
    {
        if (! is_object($instance)) {
            throw new Exception\InvalidArgumentException(
                    'This method requires an object to be shared. Class or Alias given: ' .
                             $classOrAlias);
        }
     ...
     * @return bool|string
     */
    public function hasSharedInstanceWithParameters ($classOrAlias, 
            array $params, $returnFastHashLookupKey = false)
    {
        ksort($params);
        ...
        if (isset($this->sharedInstancesWithParams['hashShort'][$hashKey])) {
            ...
            if (isset(
                    $this->sharedInstancesWithParams['hashLong'][$hashKey . '/' .
                             $hashValue])) {
     ...
     */
    public function getSharedInstanceWithParameters ($classOrAlias, 
            array $params, $fastHashFromHasLookup = null)
    {
        ...
        if (isset($this->sharedInstancesWithParams['hashShort'][$hashKey])) {
            ...
            if (isset(
                    $this->sharedInstancesWithParams['hashLong'][$hashKey . '/' .
                             $hashValue])) {
                return $this->sharedInstancesWithParams['hashLong'][$hashKey .
                 '/' . $hashValue];
    }
}
...
{
$key = ($this->hasAlias($aliasOrClass)) ? 'alias:' .
         $this->getBaseAlias($aliasOrClass) : $aliasOrClass;
...
$this->configurations[$key] = array_replace_recursive(
 ...
 */
public function setTypePreference ($interfaceOrAbstract, 
 ...
 * A type preference is a redirection to a preferred alias or type when an
 * abstract type
 ...
 * @return self
 */
public function addTypePreference ($interfaceOrAbstract, 
...
$key = ($this->hasAlias($interfaceOrAbstract)) ? 'alias:' . $interfaceOrAbstract : $interfaceOrAbstract;
if (! isset($this->typePreferences[$key]) ||
         ! in_array($preferredType, $this->typePreferences[$key])) {
    return false;
}
