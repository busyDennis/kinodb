...
class Number extends Line
{
     ...
     */
    public function __construct ($promptText = 'Please enter a number: ', $allowEmpty = false, 
            $allowFloat = false, $min = null, $max = null)
    {
        if ($promptText !== null) {
    ...
    public function show ()
    {
        ...
        do {
            ...
            if ($number === "" && ! $this->allowEmpty) {
                ...
                $this->getConsole()->writeLine(
                        "Please enter a number not greater than " . $this->max .
                                 "\n");
                $valid = false;
            } elseif ($this->min !== null && $number < $this->min) {
                $this->getConsole()->writeLine(
                        "Please enter a number not smaller than " . $this->min .
                                 "\n");
                $valid = false;
            }
        } while (! $valid);
