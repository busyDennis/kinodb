...
abstract class Console
{
     ...
     *            Optional adapter class name. Ccan be absolute namespace or
     *            class name
     *            relative to Zend\Console\Adapter\. If not provided, a best
     *            matching
     *            adapter will be automatically selected.
     * @param null|string $forceCharset
     *            optional charset name can be absolute namespace or class name
     *            relative to
     *            Zend\Console\Charset\. If not provided, charset will be
     *            detected
     *            automatically.
    ...
    public static function getInstance ($forceAdapter = null, $forceCharset = null)
    {
        ...
        if ($forceAdapter !== null) {
            ...
            if (! class_exists($className)) {
                throw new Exception\InvalidArgumentException(
                        sprintf('Cannot find Console adapter class "%s"', 
                                $className));
            }
        } else {
            ...
            if (! $className) {
                throw new Exception\RuntimeException(
                        'Cannot create Console adapter - am I running in a console?');
            }
        }
        ...
        if ($forceCharset !== null) {
            ...
            if (! class_exists($className)) {
                throw new Exception\InvalidArgumentException(
                        sprintf('Cannot find Charset class "%s"', $className));
            }
    ...
    {
        return (defined('PHP_OS') &&
                 (substr_compare(PHP_OS, 'win', 0, 3, true) === 0)) || (getenv(
                        'OS') != false &&
