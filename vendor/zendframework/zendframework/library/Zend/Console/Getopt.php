...
class Getopt
{
     ...
     *
     * @param array $argv            
     * @throws \Zend\Console\Exception\InvalidArgumentException When not given
        ...
        while (count($argv) > 0) {
            if ($argv[0] == '--') {
                ...
                if ($this->getoptConfig[self::CONFIG_DASHDASH]) {
                    $this->remainingArgs = array_merge($this->remainingArgs, 
                            $argv);
                    break;
                }
            }
            if (substr($argv[0], 0, 2) == '--') {
                ...
                /*
                 * We should put all other arguments in remainingArgs and stop
                 * parsing
    ...
    protected function _parseSingleOption ($flag, &$argv)
    {
        ...
        if (! isset($this->ruleMap[$flag])) {
            // Don't throw Exception for flag-like param in case when freeform
            // flags are allowed
    ...
    protected function _setNumericOptionValue ($value)
    {
        if (! $this->getoptConfig[self::CONFIG_NUMERIC_FLAGS]) {
            throw new Exception\RuntimeException(
                    "Using of numeric flags are deny by configuration");
        }
        
        if (empty($this->getoptConfig['numericFlagsOption'])) {
            throw new Exception\RuntimeException(
                    "Any option for handling numeric flags are specified");
        }
        
        return $this->_setSingleOptionValue(
                $this->getoptConfig['numericFlagsOption'], $value);
    }
    ...
    protected function _setSingleOptionValue ($flag, $value)
    {
        if (true === $value && $this->getoptConfig[self::CONFIG_CUMULATIVE_FLAGS]) {
            // For boolean values we have to create new flag, or increase number
            // of flags' usage count
        ...
        if (is_string($value) && ! empty($separator) && is_string($separator) &&
                 substr_count($value, $separator)) {
    ...
    {
        $this->options[$flag] = array_key_exists($flag, $this->options) ? (int) $this->options[$flag] +
                 1 : true;
    }
