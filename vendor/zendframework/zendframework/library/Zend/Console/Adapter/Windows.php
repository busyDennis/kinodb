...
class Windows extends Virtual
{
     ...
     * window. The command is fed through
     * standard input (with echo) to prevent Powershell from creating a
     * sub-thread and hanging PHP when run through
     * a debugger/IDE.
     *
     * @return void
     */
    ...
    public function getTitle ()
    {
        ...
        exec('powershell -command "write $Host.UI.RawUI.WindowTitle"', $output, 
                $result);
        if ($result || ! $output) {
            return '';
        }
    ...
    public function clear ()
    {
        ...
        exec(
                "powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Command Clear-Host", 
                $output, $return);
        
        if ($return) {
            // Could not run powershell... fall back to filling the buffer with
            // newlines
            echo str_repeat("\r\n", $this->getHeight());
        }
    }
        ...
        if ($mask === null) {
            exec(
                    'powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Command "' .
                             ...
                             'write $key.VirtualKeyCode;' . '"', $result, 
                            $return);
            ...
            exec(
                    'powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Command "' .
                             ...
                             'write $key;' . '"', $result, $return);
            
            $char = ! empty($result) ? trim(implode('', $result)) : null;
            
            if (! $return && $char &&
                     ($mask === null || in_array($char, $asciiMask))) {
                // Normalize CR to LF
                if ($char == 13) {
                    $char = 10;
                }
