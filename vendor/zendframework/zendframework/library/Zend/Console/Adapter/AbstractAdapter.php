...
abstract class AbstractAdapter implements AdapterInterface
{
     ...
     * distance from far right or bottom edge
     ...
     * @param int $fillStyle
     *            (optional) Box fill style or a single character to fill it
     *            with.
     * @param int $color
     ...
     */
    public function writeBox ($x1, $y1, $x2, $y2, $lineStyle = self::LINE_SINGLE, 
            $fillStyle = self::FILL_NONE, $color = null, $bgColor = null, $fillColor = null, 
            $fillBgColor = null)
    {
        ...
        if ($x1 < 0 || $y1 < 0 || $x2 < $x1 || $y2 < $y1) {
            throw new Exception\BadMethodCallException(
                    'Supplied X,Y coordinates are invalid.');
        }
        ...
        if ($lineStyle === static::LINE_NONE) {
            for ($y = $y1; $y <= $y2; $y ++) {
                $this->setPos($x1, $y);
                $this->write(str_repeat($fillChar, $width), $fillColor, 
                        $fillBgColor);
            }
        } else {
            switch ($lineStyle) {
            ...
            for ($y = $y1 + 1; $y < $y2; $y ++) {
                ...
                $this->write(str_repeat($fillChar, $width - 2), $fillColor, 
                        $fillBgColor);
                $this->write($lineChar, $color, $bgColor);
            }
        }
     ...
     * width and height.
     * In case a line of text does not fit desired width, it will be wrapped to
     * the next line.
     * In case the whole text does not fit in desired height, it will be
     * truncated.
     *
     * @param string $text
     *            Text to write
     * @param int $width
     *            Maximum block width. Negative value means distance from right
     *            edge.
     * @param int|null $height
     *            Maximum block height. Negative value means distance from
     *            bottom edge.
     * @param int $x
     ...
     */
    public function writeTextBlock ($text, $width, $height = null, $x = 0, $y = 0, 
    ...
    public function readChar ($mask = null)
    {
        ...
        do {
            $char = fread($f, 1);
                 ...
                 ($mask !== null && false === strstr($mask, $char)));
        fclose($f);
        return $char;
    }
}
