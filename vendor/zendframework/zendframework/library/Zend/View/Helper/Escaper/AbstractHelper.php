...
abstract class AbstractHelper extends Helper\AbstractHelper
{
     ...
     * @param mixed $value            
     * @param int $recurse
     *            Expects one of the recursion constants; used to decide whether
     *            or not to recurse the given value when escaping
     * @throws Exception\InvalidArgumentException
     * @return mixed Given a scalar, a scalar value is returned. Given an
     *         object, with the $recurse flag not allowing object recursion,
     *         returns a string. Otherwise, returns an array.
     */
    public function __invoke ($value, $recurse = self::RECURSE_NONE)
    {
        ...
        if (is_object($value)) {
            ...
            if (method_exists($value, 'toArray')) {
                return $this->__invoke($value->toArray(), 
                        $recurse | self::RECURSE_ARRAY);
            }
            return $this->__invoke((array) $value, 
                    $recurse | self::RECURSE_ARRAY);
        }
