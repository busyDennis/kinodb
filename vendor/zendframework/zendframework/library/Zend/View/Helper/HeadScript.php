...
class HeadScript extends Placeholder\Container\AbstractStandalone
{
     ...
     * @return HeadScript
     */
    public function __invoke ($mode = HeadScript::FILE, $spec = null, $placement = 'APPEND', 
    ...
    public function captureStart (
            $captureType = Placeholder\Container\AbstractContainer::APPEND, $type = 'text/javascript', $attrs = array())
    {
        if ($this->captureLock) {
    ...
    public function itemToString ($item, $indent, $escapeStart, $escapeEnd)
    {
        ...
        if (! empty($item->attributes)) {
            foreach ($item->attributes as $key => $value) {
                if ((! $this->arbitraryAttributesAllowed() &&
                         ! in_array($key, $this->optionalAttributes)) || in_array(
                                $key, 
                                array(
                                        'conditional',
                                        'noescape'
        ...
        $addScriptEscape = ! (isset($item->attributes['noescape']) && filter_var(
                $item->attributes['noescape'], FILTER_VALIDATE_BOOLEAN));
