...
class HeadMeta extends Placeholder\Container\AbstractStandalone
{
     ...
     * @return HeadMeta
     */
    public function __invoke ($content = null, $keyValue = null, $keyType = 'name', 
            $modifiers = array(), 
    ...
    public function toString ($indent = null)
    {
        ...
        return $indent .
                 implode($this->escape($this->getSeparator()) . $indent, $items);
    }
    ...
    public function itemToString (stdClass $item)
    {
        ...
        $meta = sprintf($tpl, $type, $this->escape($item->$type), 
                $this->escape($item->content), $modifiersString);
        
        if (isset($item->modifiers['conditional']) &&
                 ! empty($item->modifiers['conditional']) &&
                 is_string($item->modifiers['conditional'])) {
            $meta = '<!--[if ' . $this->escape($item->modifiers['conditional']) .
             ']>' . $meta . '<![endif]-->';
}
...
switch ($type) {
    ...
    default:
        throw new Exception\DomainException(
                sprintf('Invalid type "%s" passed to normalizeType', $type));
}
}
...
if (! isset($item->content) && (! $this->view->plugin('doctype')->isHtml5() || (! $this->view->plugin(
