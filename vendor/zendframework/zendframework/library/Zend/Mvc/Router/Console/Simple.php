...
class Simple implements RouteInterface
{
    ...
    public function __construct ($route, array $constraints = array(), 
            array $defaults = array(), array $aliases = array(), $filters = null, 
            $validators = null)
    {
        ...
        if ($filters !== null) {
            if ($filters instanceof FilterChain) {
                ...
                $this->filters = new FilterChain(
                        array(
                                'filters' => ArrayUtils::iteratorToArray(
                                        $filters, false)
                        ));
                ...
                throw new InvalidArgumentException(
                        'Cannot use ' . gettype($filters) . ' as filters for ' .
                                 __CLASS__);
            }
        }
        ...
        if ($validators !== null) {
            if ($validators instanceof ValidatorChain) {
            ...
            } elseif ($validators instanceof Traversable || is_array(
                    $validators)) {
                ...
                throw new InvalidArgumentException(
                        'Cannot use ' . gettype($validators) .
                                 ' as validators for ' . __CLASS__);
            }
        }
    ...
    public static function factory ($options = array())
    {
        ...
        return new static($options['route'], $options['constraints'], 
                $options['defaults'], $options['aliases'], $options['filters'], 
                $options['validators']);
    }
    ...
    protected function parseRouteDefinition ($def)
    {
        ...
        while ($pos < $length) {
            ...
            elseif (preg_match(
                ...
                $options = preg_split('/ *\| */', trim($m['options']), 0, 
                        PREG_SPLIT_NO_EMPTY);
                ...
                // prepare item
                $item = array(
                        'name' => isset($m['groupName']) ? $m['groupName'] : 'unnamedGroup' .
                                 $unnamedGroupCounter ++,
                                'literal' => true,
                                'required' => false,
                                'positional' => true,
            ...
            elseif (preg_match(
                ...
                // prepare item
                $item = array(
                        'name' => isset($m['groupName']) ? $m['groupName'] : 'unnamedGroupAt' .
                                 $unnamedGroupCounter ++,
                                'literal' => true,
                                'required' => true,
                                'positional' => true,
             ...
             */
            elseif (preg_match(
                    '/\G\[(?P<name>[a-z0-9][a-zA-Z0-9\_\-]*?)\](?: +|$)/s', $def, 
                    $m, 0, $pos)) {
                $item = array(
             ...
             * SOMETHING
             */
            elseif (preg_match('/\G(?P<name>[A-Z][a-zA-Z0-9\_\-]*?)(?: +|$)/s', 
                    $def, $m, 0, $pos)) {
                $item = array(
                        'name' => strtolower($m['name']),
                        'literal' => false,
             ...
             */
            elseif (preg_match(
                    '/\G(?P<name>[a-z0-9][a-zA-Z0-9\_\-]*?)(?: +|$)/s', $def, $m, 
                    0, $pos)) {
                $item = array(
                        'name' => $m['name'],
                        'literal' => true,
    ...
    public function match (Request $request, $pathOffset = null)
    {
        ...
        foreach ($positional as &$part) {
            ...
            if ($part['literal']) {
                if ((isset($part['alternatives']) &&
                         ! in_array($value, $part['alternatives'])) || (! isset(
            ...
            if ($part['hasValue']) {
                ...
                // from all alternativesm set matching parameter to TRUE and the
                // rest to FALSE
                foreach ($part['alternatives'] as $alt) {
                    if ($alt == $value) {
                        $matches[$alt] = isset($this->defaults[$alt]) ? $this->defaults[$alt] : true;
