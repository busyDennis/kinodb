...
class Segment implements RouteInterface
{
    ...
    protected static $urlencodeCorrectionMap = array(
            ...
            '%2B' => "+", // sub-delims
            '%2C' => ",", // sub-delims
                          // '%2D' => "-", // unreserved - not touched by
                          // rawurlencode
                          // '%2E' => ".", // unreserved - not touched by
                          // rawurlencode
            '%3A' => ":", // pchar
            '%3B' => ";", // sub-delims
            '%3D' => "=", // sub-delims
    ...
    protected function parseRouteDefinition ($def)
    {
        ...
        while ($currentPos < $length) {
            preg_match('(\G(?P<literal>[^:{\[\]]*)(?P<token>[:{\[\]]|$))', $def, 
            ...
            if ($matches['token'] === ':') {
                ...
                if (! preg_match('(\G(?P<literal>[^}]+)\})', $def, $matches, 0, 
                        $currentPos)) {
                    throw new Exception\RuntimeException(
                ...
                $levelParts[$level + 1] = &$levelParts[$level][count(
                        $levelParts[$level]) - 1][1];
            ...
            $hasChild, array $options)
    {
        ...
        foreach ($parts as $part) {
            switch ($part[0]) {
                ...
                case 'parameter':
                    ...
                    if (! isset($mergedParams[$part[1]])) {
                        ...
                        return '';
                             ...
                             ! isset($this->defaults[$part[1]]) || $this->defaults[$part[1]] !==
                             $mergedParams[$part[1]]) {
                        $skip = false;
                    }
                ...
                case 'optional':
                    $skippable = true;
                    $optionalPart = $this->buildPath($part[1], $mergedParams, 
                            true, $hasChild, $options);
                ...
                case 'translated-literal':
                    $path .= $translator->translate($part[1], $textDomain, 
                            $locale);
                    break;
            }
        }
        ...
        if ($this->translationKeys) {
            ...
            foreach ($this->translationKeys as $key) {
                $regex = str_replace('#' . $key . '#', 
                        $translator->translate($key, $textDomain, $locale), 
                        $regex);
            }
        }
        
        if ($pathOffset !== null) {
            $result = preg_match('(\G' . $regex . ')', $path, $matches, null, 
                    $pathOffset);
        } else {
    ...
    public function assemble (array $params = array(), array $options = array())
    {
        ...
        return $this->buildPath($this->parts, 
                array_merge($this->defaults, $params), false, 
    ...
    protected function encode ($value)
    {
        if (! isset(static::$cacheEncode[$value])) {
            ...
            static::$cacheEncode[$value] = strtr(static::$cacheEncode[$value], 
