...
class Part extends TreeRouteStack implements RouteInterface
{
     ...
     */
    public function __construct ($route, $mayTerminate, 
            RoutePluginManager $routePlugins, array $childRoutes = null, 
    ...
    public static function factory ($options = array())
    {
        ...
        if (! isset($options['route_plugins'])) {
            throw new Exception\InvalidArgumentException(
                    'Missing "route_plugins" in options array');
        }
        ...
        if ($options['child_routes'] instanceof Traversable) {
            $options['child_routes'] = ArrayUtils::iteratorToArray(
                    $options['child_routes']);
        }
        
        return new static($options['route'], $options['may_terminate'], 
                $options['route_plugins'], $options['child_routes'], 
                $options['prototypes']);
    }
        ...
        if ($match !== null && method_exists($request, 'getUri')) {
            ...
            foreach ($this->routes as $name => $route) {
                if (($subMatch = $route->match($request, $nextOffset, $options)) instanceof RouteMatch) {
                    if ($match->getLength() + $subMatch->getLength() +
                             $pathOffset === $pathLength) {
                        return $match->merge($subMatch)->setMatchedRouteName(
                                $name);
                    }
                }
            }
    ...
    public function assemble (array $params = array(), array $options = array())
    {
        ...
        $params = array_diff_key($params, 
                array_flip($this->route->getAssembledParams()));
