...
class Wildcard implements RouteInterface
{
    ...
    public static function factory ($options = array())
    {
        ...
        return new static($options['key_value_delimiter'], 
                $options['param_delimiter'], $options['defaults']);
    }
    ...
    public function match (Request $request, $pathOffset = null)
    {
        ...
        if ($this->keyValueDelimiter === $this->paramDelimiter) {
            ...
            for ($i = 1; $i < $count; $i += 2) {
                if (isset($params[$i + 1])) {
                    $matches[rawurldecode($params[$i])] = rawurldecode(
                            $params[$i + 1]);
                }
            }
        } else {
    ...
    public function assemble (array $params = array(), array $options = array())
    {
        ...
        if ($mergedParams) {
            foreach ($mergedParams as $key => $value) {
                $elements[] = rawurlencode($key) . $this->keyValueDelimiter .
                         rawurlencode($value);
            ...
            return $this->paramDelimiter .
                     implode($this->paramDelimiter, $elements);
        }
