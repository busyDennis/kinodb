...
class Forward extends AbstractPlugin
{
    ...
    public function getListenersToDetach ()
    {
        ...
        if (null === $this->listenersToDetach) {
            // We need to detach the InjectViewModelListener to prevent
            // templates
            // from getting attached to the ViewModel twice when a calling
            // action
     ...
     *            Listener information; see getListenersToDetach() for details
     *            on format.
     * @return void
     */
     ...
     *
     * @param string $name
     *            Controller name; either a class name or an alias used in the
     *            controller manager
     * @param null|array $params
     *            Parameters with which to seed a custom RouteMatch object for
     *            the new controller
     * @return mixed
     * @throws Exception\DomainException if composed controller does not define
     *         InjectApplicationEventInterface
     *         or Locator aware; or if the discovered controller is not
     *         dispatchable
     */
    public function dispatch ($name, array $params = null)
    {
        ...
        // Allow passing parameters to seed the RouteMatch with & copy matched
        // route name
        if ($params !== null) {
    ...
    /**
     * Detach problem listeners specified by getListenersToDetach() and return
     * an array of information that will
    ...
    {
        // Convert the problem list from two-dimensional array to more
        // convenient id => event => class format:
        ...
        // Loop through the class blacklist, detaching problem events and
        // remembering their CallbackHandlers
     ...
     * @return void
     */
    protected function reattachProblemListeners (SharedEvents $sharedEvents, 
            array $listeners)
    {
        foreach ($listeners as $id => $eventArray) {
            foreach ($eventArray as $eventName => $callbacks) {
                foreach ($callbacks as $current) {
                    $sharedEvents->attach($id, $eventName, 
                            $current->getCallback(), 
                            $current->getMetadatum('priority'));
                }
            }
        }
    }
    ...
    protected function getEvent ()
    {
        ...
        if (! $controller instanceof InjectApplicationEventInterface) {
            throw new Exception\DomainException(
                    'Forward plugin requires a controller that implements InjectApplicationEventInterface');
        }
