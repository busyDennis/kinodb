...
class FilePostRedirectGet extends AbstractPlugin
{
     ...
     */
    public function __invoke (FormInterface $form, $redirect = null, 
            $redirectToUrl = false)
    {
     ...
     * @return Response
     */
    protected function handlePostRequest (FormInterface $form, $redirect, 
            $redirectToUrl)
    {
        ...
        $this->traverseInputs($inputFilter, $previousFiles, 
        ...
        $postFiles = array_merge_recursive($prevFileData ?  : array(), 
                $newFileData ?  : array());
        $post = array_merge_recursive($postOther, $postFiles);
    ...
    protected function handleGetRequest (FormInterface $form)
    {
        ...
        $inputFilter = $form->getInputFilter();
        $this->traverseInputs($inputFilter, $post, 
                function  ($input, $value)
                {
                    if ($input instanceof FileInput) {
        ...
        $this->setProtectedFormProperty($form, 'isValid', $isValid); // force
                                                                     // previous
                                                                     // state
     ...
     * @return FilePostRedirectGet
     */
    protected function setProtectedFormProperty (FormInterface $form, $property, 
            $value)
    {
     ...
     * @return array|null
     */
    protected function traverseInputs (InputFilterInterface $inputFilter, 
            $values, $callback)
    {
        $returnValues = null;
     ...
     * @return array
     */
    protected function getNonEmptyUploadData (InputFilterInterface $inputFilter, 
            $data)
    {
        return $this->traverseInputs($inputFilter, $data, 
                function  ($input, $value)
                {
                    $messages = $input->getMessages();
                    if (is_array($value) && $input instanceof FileInput &&
                             empty($messages)) {
                        $rawValue = $input->getRawValue();
                        if ((isset($rawValue['error']) &&
                                 $rawValue['error'] !== UPLOAD_ERR_NO_FILE) ||
                                 (isset($rawValue[0]['error']) &&
                                 $rawValue[0]['error'] !== UPLOAD_ERR_NO_FILE)) {
                            return $value;
                        }
                    }
                    return null;
     ...
     * @return array
     */
    protected function getEmptyUploadData (InputFilterInterface $inputFilter, 
            $data)
    {
        return $this->traverseInputs($inputFilter, $data, 
                function  ($input, $value)
