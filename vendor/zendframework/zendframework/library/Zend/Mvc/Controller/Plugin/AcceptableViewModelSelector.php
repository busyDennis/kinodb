...
/**
 * Controller Plugin to assist in selecting an appropriate View Model type based
 * on the
...
class AcceptableViewModelSelector extends AbstractPlugin
{
     ...
     */
    public function __invoke (array $matchAgainst = null, $returnDefault = true, 
            & $resultReference = null)
    {
        return $this->getViewModel($matchAgainst, $returnDefault, 
                $resultReference);
    }
     ...
     * @return ModelInterface|null
     */
    public function getViewModel (array $matchAgainst = null, $returnDefault = true, 
            & $resultReference = null)
    {
        $name = $this->getViewModelName($matchAgainst, $returnDefault, 
                $resultReference);
        ...
        if (! class_exists($name)) {
            throw new InvalidArgumentException(
                    'The supplied View Model could not be found');
        }
     ...
     *         match could be made
     */
    public function getViewModelName (array $matchAgainst = null, 
            $returnDefault = true, & $resultReference = null)
    {
    ...
    public function match (array $matchAgainst = null)
    {
        ...
        if ((! $matchAgainst && ! $this->defaultMatchAgainst) ||
                 ! $headers->has('accept')) {
            return null;
        }
        ...
        foreach ($matchAgainst as $modelName => $modelStrings) {
            foreach ((array) $modelStrings as $modelString) {
                $matchAgainstString .= $this->injectViewModelName($modelString, 
                        $modelName);
            }
        }
    ...
    protected function injectViewModelName ($modelAcceptString, $modelName)
    {
        ...
        return $modelAcceptString . '; ' . self::INJECT_VIEWMODEL_NAME . '="' .
                 $modelName . '", ';
    }
