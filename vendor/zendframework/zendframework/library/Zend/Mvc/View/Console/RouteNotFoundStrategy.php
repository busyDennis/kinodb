...
class RouteNotFoundStrategy extends AbstractListenerAggregate
{
    ...
    public function handleRouteNotFoundError (MvcEvent $e)
    {
        ...
        try {
            ...
            // The application does not have or use module manager, so we cannot
            // use it
        }
        ...
        try {
            ...
            throw new RuntimeException(
                    'Cannot access Console adapter - is it defined in ServiceManager?');
        }
     ...
     * @return string
     */
    protected function getConsoleBanner (ConsoleAdapter $console, 
     ...
     * @throws RuntimeException
     */
    protected function getConsoleUsage (ConsoleAdapter $console, $scriptName, 
            ModuleManagerInterface $moduleManager = null)
    {
        /*
        ...
        if ($moduleManager !== null) {
            foreach ($moduleManager->getLoadedModules(false) as $name => $module) {
                ...
                // We prepend the usage by the module name (printed in red), so
                // that each module is
                // clearly visible by the user
                $moduleName = sprintf("%s\n%s\n%s\n", 
                        str_repeat('-', $console->getWidth()), $name, 
                        str_repeat('-', $console->getWidth()));
                
                $moduleName = $console->colorize($moduleName, 
                        ColorInterface::RED);
        ...
        /*
         * Transform arrays in usage info into columns, otherwise join
         * everything together
         */
        $result = '';
        $table = false;
        ...
        foreach ($usageInfo as $moduleName => $usage) {
            if (! is_string($usage) && ! is_array($usage)) {
                throw new RuntimeException(
                        sprintf('Cannot understand usage info for module "%s"', 
                                $moduleName));
            }
            ...
            foreach ($usage as $a => $b) {
                ...
                if (is_string($a) && is_string($b)) {
                    if (($tableCols !== 2 || $tableType != 1) && $table !== false) {
                        // render last table
                        $result .= $this->renderTable($table, $tableCols, 
                                $console->getWidth());
                        $table = false;
                    ...
                    $a = $console->colorize($scriptName . ' ' . $a, 
                 ...
                 */
                if (is_array($b)) {
                    if ((count($b) != $tableCols || $tableType != 2) &&
                             $table !== false) {
                        // render last table
                        $result .= $this->renderTable($table, $tableCols, 
                                $console->getWidth());
    ...
    protected function renderTable ($data, $cols, $consoleWidth)
    {
        ...
        for ($x = 1; $x <= $cols; $x += 1) {
            ...
            foreach ($data as $row) {
                $maxW[$x] = max($maxW[$x], 
                        $strWrapper->strlen($row[$x - 1]) + $padding * 2);
            }
        }
