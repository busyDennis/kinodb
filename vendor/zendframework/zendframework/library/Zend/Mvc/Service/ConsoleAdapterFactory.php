...
class ConsoleAdapterFactory implements FactoryInterface
{
     ...
     * object.
     *
     * In order to disable adapter auto-detection and use a specific adapter
     * (and charset),
    ...
    public function createService (ServiceLocatorInterface $serviceLocator)
    {
        ...
        if (! Console::isConsole()) {
            // SM factory cannot currently return null, so we return dummy
            // object
            return new stdClass();
        }
        ...
        if (! empty($config['console']) && ! empty(
                $config['console']['adapter'])) {
        ...
        if (! $adapter instanceof AdapterInterface) {
            // SM factory cannot currently return null, so we convert it to
            // dummy object
            return new stdClass();
        }
