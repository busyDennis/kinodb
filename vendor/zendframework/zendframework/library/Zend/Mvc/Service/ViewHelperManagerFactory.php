...
class ViewHelperManagerFactory extends AbstractPluginManagerFactory
{
    ...
    public function createService (ServiceLocatorInterface $serviceLocator)
    {
        ...
        $plugins->setFactory('url', 
                function  ($sm) use( $serviceLocator)
                {
        ...
        $plugins->setFactory('basepath', 
                function  ($sm) use( $serviceLocator)
                {
                    $config = $serviceLocator->has('Config') ? $serviceLocator->get(
                            'Config') : array();
                    $basePathHelper = new ViewHelper\BasePath();
                    if (isset($config['view_manager']) &&
                             isset($config['view_manager']['base_path'])) {
                        $basePath = $config['view_manager']['base_path'];
                    } else {
                        $basePath = $serviceLocator->get('Request')
                            ->getBasePath();
                    }
        ...
        /**
         * Configure doctype view helper with doctype from configuration, if
         * available.
         *
         * Other view helpers depend on this to decide which spec to generate
         * their tags
         * based on. This is why it must be set early instead of later in the
         * layout phtml.
         */
        $plugins->setFactory('doctype', 
                function  ($sm) use( $serviceLocator)
                {
                    $config = $serviceLocator->has('Config') ? $serviceLocator->get(
                            'Config') : array();
