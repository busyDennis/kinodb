...
class DiStrictAbstractServiceFactory extends Di implements 
        AbstractFactoryInterface
{
    ...
    {
        $this->allowedServiceNames = array_flip(
                array_values($allowedServiceNames));
    }
    ...
    public function get ($name, array $params = array())
    {
        ...
        if (self::USE_SL_BEFORE_DI === $this->useServiceLocator &&
        ...
        try {
            ...
            if (self::USE_SL_AFTER_DI === $this->useServiceLocator &&
                     $this->serviceLocator->has($name)) {
                return $this->serviceLocator->get($name);
            }
            
            throw new Exception\ServiceNotFoundException(
                    sprintf('Service %s was not found in this DI instance', 
                            $name), null, $e);
        }
    }
    ...
    {
        // won't check if the service exists, we are trusting the user's
        // whitelist
