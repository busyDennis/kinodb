...
class ArrayOfTypeSequence extends DefaultComplexType
{
    ...
    public function addComplexType ($type)
    {
        ...
        if ($nestedCounter > 0) {
            ...
            for ($i = 1; $i <= $nestedCounter; $i ++) {
                $complexType = $this->_getTypeBasedOnNestingLevel($singularType, 
                        $i);
                $complexTypePhp = $singularType . str_repeat('[]', $i);
                $childType = $this->_getTypeBasedOnNestingLevel($singularType, 
                        $i - 1);
                
                $this->_addSequenceType($complexType, $childType, 
                        $complexTypePhp);
            }
    ...
    protected function _getTypeBasedOnNestingLevel ($singularType, $level)
    {
        ...
        return Wsdl::TYPES_NS . ':' . str_repeat('ArrayOf', $level) .
