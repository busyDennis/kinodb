...
class Wsdl
{
    ...
    public function __construct ($name, $uri, 
            ComplexTypeStrategy $strategy = null, array $classMap = array())
    {
        ...
        $this->setComplexTypeStrategy(
                $strategy ?  : new Wsdl\ComplexTypeStrategy\DefaultComplexType());
    }
    ...
    protected function getDOMDocument ($name, $uri = null)
    {
        ...
        $this->setAttributeWithSanitization($definitions, 'targetNamespace', 
                $uri);
        
        $definitions->setAttributeNS(self::XML_NS_URI, 'xmlns:' . self::WSDL_NS, 
                self::WSDL_NS_URI);
        $definitions->setAttributeNS(self::XML_NS_URI, 
                'xmlns:' . self::TYPES_NS, $uri);
        $definitions->setAttributeNS(self::XML_NS_URI, 
                'xmlns:' . self::SOAP_11_NS, self::SOAP_11_NS_URI);
        $definitions->setAttributeNS(self::XML_NS_URI, 'xmlns:' . self::XSD_NS, 
                self::XSD_NS_URI);
        $definitions->setAttributeNS(self::XML_NS_URI, 
                'xmlns:' . self::SOAP_ENC_NS, self::SOAP_ENC_URI);
        $definitions->setAttributeNS(self::XML_NS_URI, 
                'xmlns:' . self::SOAP_12_NS, self::SOAP_12_NS_URI);
    ...
    public function setUri ($uri)
    {
        ...
        if ($this->dom instanceof DOMDocument) {
            ...
            // explicitly set on root element
            // xmlns:tns = $uri
            $this->dom->documentElement->setAttributeNS(self::XML_NS_URI, 
                    self::XML_NS . ':' . self::TYPES_NS, $uri);
            ...
            $attributeNodes = $xpath->query(
                    '//attribute::*[contains(., "' . $oldUri . '")]');
            
            foreach ($attributeNodes as $node) {
                $attributeValue = $this->dom->createTextNode(
                        str_replace($oldUri, $uri, $node->nodeValue));
    ...
    public function addBindingOperation ($binding, $name, $input = false, 
            $output = false, $fault = false, $soapVersion = SOAP_1_1)
    {
        $operation = $this->dom->createElementNS(self::WSDL_NS_URI, 'operation');
        ...
        if (is_array($input) && ! empty($input)) {
            ...
            $soapNode = $this->dom->createElementNS(
                    $this->getSoapNamespaceUriByVersion($soapVersion), 'body');
    ...
    public function addSoapBinding ($binding, $style = 'document', 
            $transport = 'http://schemas.xmlsoap.org/soap/http', $soapVersion = SOAP_1_1)
    {
        $soapBinding = $this->dom->createElementNS(
                $this->getSoapNamespaceUriByVersion($soapVersion), 'binding');
            ...
            $soapVersion = SOAP_1_1)
    {
        ...
        $soapOperation = $this->dom->createElementNS(
                $this->getSoapNamespaceUriByVersion($soapVersion), 'operation');
        $operation->insertBefore($soapOperation, $operation->firstChild);
        
        $this->setAttributeWithSanitization($soapOperation, 'soapAction', 
                $soapAction);
            ...
            $soapVersion = SOAP_1_1)
    {
        ...
        $soapAddress = $this->dom->createElementNS(
                $this->getSoapNamespaceUriByVersion($soapVersion), 'address');
    ...
    public function addDocumentation ($inputNode, $documentation)
    {
        ...
        $docCData = $this->dom->createTextNode(
                str_replace(
                        array(
                                "\r\n",
                                "\r"
    ...
    public function addSchemaTypeSection ()
    {
        if ($this->schema === null) {
            ...
            $this->schema = $this->dom->createElementNS(WSDL::XSD_NS_URI, 
                    'schema');
            $types->appendChild($this->schema);
            
            $this->setAttributeWithSanitization($this->schema, 
                    'targetNamespace', $this->getUri());
        }
    ...
    protected function _parseElement ($element)
    {
        ...
        foreach ($element as $key => $value) {
            if (in_array($key, 
                ...
                if (is_array($value)) {
                    $complexType = $this->dom->createElementNS(self::XSD_NS_URI, 
                            'complexType');
                    if (count($value) > 0) {
                        $container = $this->dom->createElementNS(
                                self::XSD_NS_URI, $key);
     ...
     */
    protected function arrayToAttributes (\DOMNode $node, array $attributes, 
            $withSanitizer = true)
    {
        foreach ($attributes as $attributeName => $attributeValue) {
            if ($withSanitizer) {
                $this->setAttributeWithSanitization($node, $attributeName, 
                        $attributeValue);
            } else {
     ...
     */
    protected function setAttributeWithSanitization (\DOMNode $node, 
            $attributeName, $attributeValue)
    {
        $attributeValue = $this->sanitizeAttributeValueByName($attributeName, 
                $attributeValue);
        $this->setAttribute($node, $attributeName, $attributeValue);
    }
     ...
     * @param mixed $attributeValue            
     */
    protected function setAttribute (\DOMNode $node, $attributeName, 
            $attributeValue)
    {
        $attributeNode = $node->ownerDocument->createAttribute($attributeName);
        $node->appendChild($attributeNode);
        
        $attributeNodeValue = $node->ownerDocument->createTextNode(
                $attributeValue);
    ...
    protected function getSoapNamespaceUriByVersion ($soapVersion)
    {
        if ($soapVersion != SOAP_1_1 and $soapVersion != SOAP_1_2) {
            throw new Exception\InvalidArgumentException(
                    'Invalid SOAP version, use constants: SOAP_1_1 or SOAP_1_2');
        }
