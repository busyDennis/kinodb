...
class AutoDiscover
{
    ...
    public function setClassMap ($classMap)
    {
        if (! is_array($classMap)) {
            throw new Exception\InvalidArgumentException(
                    sprintf('%s expects an array; received "%s"', __METHOD__, 
                            (is_object($classMap) ? get_class($classMap) : gettype(
                                    $classMap))));
        }
    ...
    public function setServiceName ($serviceName)
    {
        ...
        // first character must be letter or underscore {@see
        // http://www.w3.org/TR/wsdl#_document-n}
        $i = preg_match('/^[a-z\_]/ims', $serviceName, $matches);
        if ($i != 1) {
            throw new Exception\InvalidArgumentException(
                    'Service Name must start with letter or _');
        }
    ...
    public function setWsdlClass ($wsdlClass)
    {
        if (! is_string($wsdlClass) &&
                 ! is_subclass_of($wsdlClass, '\Zend\Soap\Wsdl')) {
            throw new Exception\InvalidArgumentException(
    ...
    {
        return $this->_generateWsdl(
                $this->reflection->reflectClass($this->class)
                    ->getMethods());
    }
    ...
    protected function _generateWsdl (array $reflectionMethods)
    {
        ...
        $wsdl = new $this->wsdlClass($serviceName, $uri, $this->strategy, 
                $this->classMap);
        ...
        $binding = $wsdl->addBinding($serviceName . 'Binding', 
                Wsdl::TYPES_NS . ':' . $serviceName . 'Port');
    ...
    protected function _addFunctionToWsdl ($function, $wsdl, $port, $binding)
    {
        ...
        if ($this->bindingStyle['style'] == 'document') {
            ...
            foreach ($prototype->getParameters() as $param) {
                $sequenceElement = array(
                        'name' => $param->getName(),
                        'type' => $wsdl->getType(
                                $this->discoveryStrategy->getFunctionParameterType(
                                        $param))
                );
                if ($param->isOptional()) {
        ...
        $isOneWayMessage = $this->discoveryStrategy->isFunctionOneWay($function, 
                $prototype);
            ...
            if ($this->bindingStyle['style'] == 'document') {
                ...
                if ($prototype->getReturnType() != "void") {
                    $sequence[] = array(
                            ...
                            'type' => $wsdl->getType(
                                    $this->discoveryStrategy->getFunctionReturnType(
                                            $function, $prototype))
                    );
                }
        ...
        if ($this->bindingStyle['style'] == 'rpc' &&
                 ! isset($operationBodyStyle['namespace'])) {
