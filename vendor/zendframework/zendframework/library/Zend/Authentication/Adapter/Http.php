...
class Http implements AdapterInterface
{
    ...
    public function __construct (array $config)
    {
        ...
        $this->acceptSchemes = array_intersect($schemes, 
        ...
        if (in_array('digest', $this->acceptSchemes)) {
            ...
            if (isset($config['algorithm']) &&
                     in_array($config['algorithm'], $this->supportedAlgos)) {
    ...
    public function authenticate ()
    {
        ...
        if (! in_array($clientScheme, $this->supportedSchemes)) {
            ...
            return new Authentication\Result(
                    Authentication\Result::FAILURE_UNCATEGORIZED, array(), 
                    array(
        ...
        switch ($clientScheme) {
            ...
            default:
                throw new Exception\RuntimeException(
                        'Unsupported authentication scheme: ' . $clientScheme);
        }
    ...
    protected function _challengeClient ()
    {
        ...
        return new Authentication\Result(
                Authentication\Result::FAILURE_CREDENTIAL_INVALID, array(), 
                array(
    ...
    {
        $wwwauth = 'Digest realm="' . $this->realm . '", ' . 'domain="' .
                 $this->domains . '", ' . 'nonce="' . $this->_calcNonce() . '", ' .
                 ($this->useOpaque ? 'opaque="' . $this->_calcOpaque() . '", ' : '') .
                 'algorithm="' . $this->algo . '", ' . 'qop="' .
                 implode(',', $this->supportedQops) . '"';
    ...
    protected function _basicAuth ($header)
    {
        ...
        $result = $this->basicResolver->resolve($creds[0], $this->realm, 
                $creds[1]);
        ...
        if (! $result instanceof Authentication\Result && ! is_array($result) &&
                 CryptUtils::compareStrings($result, $creds[1])) {
            $identity = array(
                    'username' => $creds[0],
                    'realm' => $this->realm
            );
            return new Authentication\Result(Authentication\Result::SUCCESS, 
                    $identity);
    ...
    protected function _parseDigestAuth ($header)
    {
        ...
        $ret = preg_match('/algorithm="?(' . $this->algo . ')"?/', $header, 
                $temp);
        if ($ret && ! empty($temp[1]) &&
