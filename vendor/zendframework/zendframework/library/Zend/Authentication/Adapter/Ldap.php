...
class Ldap extends AbstractAdapter
{
     ...
     */
    public function __construct (array $options = array(), $identity = null, 
            $credential = null)
    {
    ...
    public function authenticate ()
    {
        ...
        foreach ($this->options as $options) {
            ...
            try {
                 ...
                 * optionally
                 * "password" in the Zend\Ldap\Ldap options is able to retrieve
                 * the required
                 * information.
                 */
                $requireRebind = false;
                ...
                $dn = $ldap->getCanonicalAccountName($canonicalName, 
                        ZendLdap\Ldap::ACCTNAME_FORM_DN);
                
                $groupResult = $this->checkGroupMembership($ldap, 
                ...
                if ($groupResult === true) {
                    ...
                    return new AuthenticationResult(
                            AuthenticationResult::SUCCESS, $canonicalName, 
                            $messages);
                } else {
                ...
                if ($err == LdapException::LDAP_X_DOMAIN_MISMATCH) {
                    ...
                    $messages[] = preg_replace(
                            '/\b' . preg_quote(substr($password, 0, 15), '/') .
                                     '\b/', '*****', $zle->getTraceAsString());
     ...
     * @return string|true
     */
    protected function checkGroupMembership (ZendLdap\Ldap $ldap, $canonicalName, 
            $dn, array $adapterOptions)
    {
        ...
        $groupName = ZendLdap\Filter::equals($adapterOptions['groupAttr'], 
                $adapterOptions['group']);
        $membership = ZendLdap\Filter::equals($adapterOptions['memberAttr'], 
                $user);
     ...
     * Zend\Authentication\Adapter\DbTable::getResultRowObject()}.
     * Closes ZF-6813
     *
            ...
            array $omitAttribs = array())
    {
        ...
        $entry = $this->getLdap()->getEntry($this->authenticatedDn, 
                $returnAttribs, true);
