    ...
    if ('-' == $output) {
        ...
        if (! $opts->getOption('w') && ! $appending) {
            echo "Plugin map file already exists at '$output'," . PHP_EOL .
                     "but 'overwrite' flag was not specified; aborting." .
...
if (! $usingStdout) {
    if ($appending) {
        echo "Appending to plugin class map '$output' for classes in '$path'..." .
                 PHP_EOL;
    } else {
...
// Iterate over each element in the path, and create a map of pluginname =>
// classname
$map = new \stdClass();
foreach ($l as $file) {
...
if ($appending) {
    ...
    // necessary
    $content = '<' . "?php\n\n" . "// plugin class map\n" .
             "// auto-generated using " . basename($_SERVER['argv'][0]) . ', ' .
             date('Y-m-d H:i:s') . "\n\n" . 'return ' .
