...
if (isset($opts->o)) {
    ...
    if ('-' == $output) {
        ...
        // created in the template map file.
        $mapPath = str_replace(DIRECTORY_SEPARATOR, '/', 
        ...
        if (strpos($libraryPath, $mapPath) === 0) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                if (! isset($libraryPathParts[$i]) ||
                         $libraryPathParts[$i] != $mapPathParts[$i]) {
                    // Common part end
                    break;
                }
            }
...
if (! $usingStdout) {
    if ($appending) {
        echo "Appending to template file map '$output' for library in '$libraryPath'..." .
                 PHP_EOL;
    } else {
        echo "Creating template file map for library in '$libraryPath'..." .
                 PHP_EOL;
    }
}

$dirOrIterator = new RecursiveDirectoryIterator($viewPath, 
...
foreach ($l as $file) {
    ...
    $mapName = str_replace($libraryPath . '/', '', 
            str_replace(DIRECTORY_SEPARATOR, '/', $file->getPath()) . '/' .
                     $baseName);
